...
pub fn content_requires_genesis(content: &str, force_genesis: bool) -> Result<bool, String> {
    if force_genesis {
        return Ok(true);
    }
...
pub fn capture_vault_unlock_from_content(
    content: &str,
) -> Result<(VaultUnlock, Vec<crate::PasswordUnlockEntry>, Option<String>, u64), String> {
...
mod tests {
    use super::*;
    use crate::{generate_store_id, generate_vault_keys, genesis_auth_record, genesis_members_records, serialize_stored_yaml_with_unlock};

    #[test]
    fn empty_content_requires_genesis() -> Result<(), String> {
    ...
    #[test]
    fn genesis_yaml_reports_ready_for_enrolled_device() -> Result<(), String> {
