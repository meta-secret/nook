...
test("remote compile preserves cache secrets and rejects registry access failures", () => {
  const bake = read("nook-app/docker-bake.hcl");
  const compile = read(".github/scripts/compile-remote.sh");
  ...
  const action = read(".github/actions/nook-docker-setup/action.yml");
...
test("two-phase compile keeps its publish and read-only timeout budgets", () => {
  const batch = read(".github/scripts/remote-task-batch.sh");
  const workflow = read(".github/workflows/remote.yml");
  for (const entry of [
