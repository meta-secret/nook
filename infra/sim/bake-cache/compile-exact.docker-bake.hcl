...
target "compile-exact-source" {
  ...
  cache-to = COMPILE_CACHE_WRITE_ENABLED != "" && GHA_CACHE_EXACT_BUILD_COMPILE_AVAILABLE == "" ? [
    "type=registry,ref=${compile_source_cache_ref},mode=min,compression=zstd,force-compression=true,timeout=5m",
  ] : []
...
target "compile-exact-dependencies" {
  ...
  cache-to = COMPILE_DEPS_CACHE_WRITE_ENABLED != "" ? [
