...
fn compile_web_creates_package_directories_before_dependency_symlinks() -> anyhow::Result<()> {
    ...
    let source_copy = dockerfile.required_offset(
        ...
        "compile-web must copy the bounded web workspace source",
    )?;
...
-> anyhow::Result<()> {
    ...
    for (stage_name, stage, crate_groups) in [
        ...
        for crates in crate_groups {
            ...
            let crate_paths = crates.join(" ");
            assert!(
                invalidation.contains(&format!(
                    "find {crate_paths} -type f -name '*.rs' -exec touch {{}} +"
                )),
                "{stage_name} stage must refresh every Rust source under {crate_paths} after copying real sources and before building"
            );
