...
use nook_preflight::source_size::{
    SOURCE_SIZE_REMEDIATION, UNIT_TEST_COLOCATION_REMEDIATION, external_rust_unit_test_modules,
...
fn critical_architecture_rule_stays_wired_to_agent_guidance() -> anyhow::Result<()> {
    ...
    for (name, source) in [
        ...
        assert!(
            source.contains("1,500") && source.contains("1,000"),
            "{name} must preserve both hard limits"
        );
        assert!(
