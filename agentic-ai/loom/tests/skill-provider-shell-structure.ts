...
export class SkillProviderShellStructureScenario {
  private constructor(private readonly request: ShellStructureInspection) {}

  static shellStructure(inspection: ShellStructureInspection): ShellStructure {
    return new SkillProviderShellStructureScenario(inspection).execute();
  }

  private execute(): ShellStructure {
    const inspection = this.request;
    const heredocs = SkillProviderShellStructureScenario.stripHeredocs(
      inspection.source,
    );
    ...
    const source =
      SkillProviderShellStructureScenario.extractFunctions(functionInspection);
    return {
      source,
      substitutions: [
        ...heredocs.substitutions,
        ...SkillProviderShellStructureScenario.compoundSubstitutions(
          heredocs.source,
        ),
      ],
    };
  ...
  static stripHeredocs(source: string): StrippedHeredocs {
    const output: string[] = [];
    ...
    for (const line of lines) {
      ...
      if (active) {
        ...
        if (candidate === active.delimiter) {
          ...
          substitutions.push(
            ...SkillProviderShellStructureScenario.shellSubstitutionBodies(
              line,
            ),
          );
      ...
      const declaration = SkillProviderShellStructureScenario.heredocLine(line);
  ...
  static heredocLine(source: string): HeredocLine {
    ...
    for (let index = 0; index < source.length; index += 1) {
      ...
      const compoundEnd =
        SkillProviderShellStructureScenario.shellCompoundEnd(compoundRequest);
      ...
      if (
        SkillProviderShellTokenizerScenario.isShellCommentStart([
          source,
          index,
          wordActive,
        ])
      ...
      const parsed =
        SkillProviderShellStructureScenario.delimiterWord(delimiterRequest);
  ...
  static compoundSubstitutions(source: string): readonly string[] {
    const substitutions: string[] = [];
    let quote = '';
    for (let index = 0; index < source.length; index += 1) {
      const [character = ''] = [source[index]];
      ...
      if (
        !['[[', '((', '$(('].some((opening) =>
          source.startsWith(opening, index),
        )
      )
        continue;
      const request: DelimiterRequest = { source, start: index };
      const end = SkillProviderShellStructureScenario.shellCompoundEnd(request);
  ...
  static shellCompoundEnd(request: DelimiterRequest): number | false {
    const opening =
  ...
  static delimiterWord(request: DelimiterRequest): DelimiterWord | false {
    let value = '';
    let quote = '';
    let quoted = false;
  ...
  static extractFunctions(inspection: ShellStructureInspection): string {
    ...
    for (let index = 0; index < inspection.source.length; index += 1) {
      ...
      const end =
        SkillProviderShellStructureScenario.findClosing(closingRequest);
  ...
  static findClosing(request: ClosingRequest): number {
    let depth = 1;
    let quote = '';
    let wordActive = false;
  ...
  static shellSubstitutionBodies(source: string): readonly string[] {
    const bodies: string[] = [];
    let quote = '';
    for (let index = 0; index < source.length; index += 1) {
      const [character = ''] = [source[index]];
      ...
      if (character === '`') {
        ...
        if (end < 0)
          throw new Error('Unterminated shell backtick substitution.');
