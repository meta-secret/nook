...
export class SkillProviderSubprocessScenario {
  ...
  static repositorySubprocessEntrypoints(
    inspection: RepositorySubprocessInspection,
  ): RepositorySubprocessDiscovery {
    const context =
      SkillProviderSubprocessScenario.createSubprocessTypeContext(inspection);
    const sourceFile = context.sourceFile;
    const initializerInspection: InitializerInspection = {
      checker: context.checker,
      sourceFile,
    };
    const initializerCollection =
      SkillProviderSubprocessScenario.collectInitializers(
        initializerInspection,
      );
    const bindings =
      SkillProviderSubprocessScenario.collectSubprocessBindings(sourceFile);
    ...
    const visit = (node: ts.Node): void => {
      if (ts.isCallExpression(node)) {
        ...
        const api =
          SkillProviderSubprocessScenario.subprocessApi(subprocessInspection);
        if (api !== false) {
          const resolution: SubprocessCallResolution = { api, call: node };
          discoveries.push(resolution);
        }
        if (
          SkillProviderSubprocessScenario.isBunSubprocessCall(node.expression)
        ) {
    ...
    for (const discovery of discoveries) {
      ...
      for (const argument of discovery.call.arguments) {
        ...
        SkillProviderSubprocessScenario.collectLiterals(collection);
      }
      ...
      const candidateDiscovery =
        SkillProviderSubprocessScenario.candidateScriptLiterals(
          candidateInspection,
        );
      for (const literal of candidateDiscovery.literals) {
        if (
          ...
          !literal.includes('/') &&
          !SkillProviderSubprocessScenario.candidateUsesScriptRuntime(
            candidateInspection,
          )
        ) {
          continue;
        ...
        const resolved =
          SkillProviderSubprocessScenario.resolveRepositoryScript(
            scriptResolution,
          );
  ...
  static createSubprocessTypeContext(
    inspection: RepositorySubprocessInspection,
  ): SubprocessTypeContext {
    return new SkillProviderSubprocessScenario(inspection).execute();
  }

  private execute(): SubprocessTypeContext {
  ...
  static subprocessApi(
    inspection: SubprocessExpressionInspection,
  ): SubprocessApi | false {
    const expression = SkillProviderSubprocessScenario.unwrapExpression(
    ...
    const member = SkillProviderSubprocessScenario.asSubprocessApi(
      expression.name.text,
    );
    if (member === false) return false;
    const root = SkillProviderSubprocessScenario.unwrapExpression(
    ...
    return SkillProviderSubprocessScenario.isChildProcessRequire(root)
      ? member
      : false;
  }

  static collectSubprocessBindings(
    ...
    SkillProviderSubprocessScenario.collectImportedBindings(collection);
    let priorSize = -1;
    while (
      priorSize !==
      ...
      priorSize =
        bindings.apiIdentifiers.size + bindings.namespaceIdentifiers.size;
      SkillProviderSubprocessScenario.collectAliasedBindings(collection);
    }
    return bindings;
  }

  static collectImportedBindings(collection: BindingCollection): void {
    const visit = (node: ts.Node): void => {
      if (
        ...
        if (bindings && ts.isNamespaceImport(bindings)) {
          ...
          for (const element of bindings.elements) {
            ...
            const api =
              SkillProviderSubprocessScenario.asSubprocessApi(imported);
            if (api !== false) {
      ...
      if (
        ...
        SkillProviderSubprocessScenario.isChildProcessSpecifier(
          node.moduleReference.expression,
        )
      ) {
  ...
  static collectAliasedBindings(collection: BindingCollection): void {
    const visit = (node: ts.Node): void => {
      if (ts.isVariableDeclaration(node) && node.initializer) {
        const bindingInspection: VariableBindingInspection = {
          bindings: collection.bindings,
          initializer: SkillProviderSubprocessScenario.unwrapExpression(
            node.initializer,
          ),
          name: node.name,
        };
        SkillProviderSubprocessScenario.collectVariableBinding(
          bindingInspection,
        );
      }
      ts.forEachChild(node, visit);
  ...
  static collectVariableBinding(inspection: VariableBindingInspection): void {
    if (ts.isIdentifier(inspection.name)) {
      ...
      if (
        SkillProviderSubprocessScenario.isNamespaceExpression(
      ...
      const api =
        SkillProviderSubprocessScenario.aliasedApi(expressionInspection);
      if (api !== false)
    ...
    for (const element of inspection.name.elements) {
      ...
      const imported =
        element.propertyName && ts.isIdentifier(element.propertyName)
          ...
          : element.name.text;
      const api = SkillProviderSubprocessScenario.asSubprocessApi(imported);
      if (api !== false)
  ...
  static aliasedApi(
    ...
    return SkillProviderSubprocessScenario.subprocessApi(inspection);
  }

  static isNamespaceExpression(
    inspection: SubprocessExpressionInspection,
  ): boolean {
    const expression = SkillProviderSubprocessScenario.unwrapExpression(
      inspection.expression,
    ...
    return (
      SkillProviderSubprocessScenario.isChildProcessRequire(expression) ||
      (ts.isIdentifier(expression) &&
        inspection.bindings.namespaceIdentifiers.has(expression.text))
    );
  }

  static isChildProcessRequire(node: ts.Expression): boolean {
    return (
  ...
  static isChildProcessSpecifier(node: ts.Expression): boolean {
    return (
  ...
  static isBunSubprocessCall(expression: ts.Expression): boolean {
    const candidate =
      SkillProviderSubprocessScenario.unwrapExpression(expression);
    return (
  ...
  static collectInitializers(
    ...
    const mutableSymbols =
      SkillProviderSubprocessScenario.collectMutableSymbols(inspection);
    const visit = (node: ts.Node): void => {
      if (
  ...
  static collectMutableSymbols(
    ...
    const recordMutation = (expression: ts.Expression): void => {
      const root =
        SkillProviderSubprocessScenario.mutationRootIdentifier(expression);
      const symbol = root
        ? inspection.checker.getSymbolAtLocation(root)
        : false;
    ...
    const visit = (node: ts.Node): void => {
      if (
        ...
        const sourceRoot =
          SkillProviderSubprocessScenario.mutationRootIdentifier(
            node.initializer,
          );
  ...
  static mutationRootIdentifier(
    expression: ts.Expression,
  ): ts.Identifier | false {
    let current = SkillProviderSubprocessScenario.unwrapExpression(expression);
    while (
      ts.isPropertyAccessExpression(current) ||
      ts.isElementAccessExpression(current)
    ) {
      current = SkillProviderSubprocessScenario.unwrapExpression(
        current.expression,
      );
    }
  ...
  static collectLiterals(collection: LiteralCollection): void {
    ...
    if (ts.isIdentifier(collection.node)) {
      ...
      if (initializer) {
        ...
        SkillProviderSubprocessScenario.collectLiterals(nestedCollection);
        return;
      }
      if (
    ...
    if (ts.isArrayLiteralExpression(collection.node)) {
      for (const element of collection.node.elements) {
        ...
        SkillProviderSubprocessScenario.collectNestedLiterals(nestedInspection);
      }
      return;
    }
    if (ts.isObjectLiteralExpression(collection.node)) {
      for (const property of collection.node.properties) {
        if (ts.isPropertyAssignment(property)) {
          ...
          SkillProviderSubprocessScenario.collectNestedLiterals(
            nestedInspection,
          );
  ...
  static collectNestedLiterals(inspection: NestedLiteralCollection): void {
    const nestedCollection: LiteralCollection = {
  ...
  static candidateScriptLiterals(
    ...
    if (
      ...
      if (SCRIPT_RUNTIMES.has(posix.basename(command))) {
        ...
        return SkillProviderSubprocessScenario.runtimeScriptLiterals(
          runtimeInspection,
        );
      }
      return {
        literals: SkillProviderSubprocessScenario.isScriptLikeLiteral(command)
          ? [command]
          : [],
        unresolved: false,
      };
    ...
    return {
      literals: SkillProviderSubprocessScenario.isScriptLikeLiteral(first)
        ? [first]
        : [],
      unresolved: false,
    };
  ...
  static candidateUsesScriptRuntime(
    inspection: CandidateScriptInspection,
  ): boolean {
    const [first = false] = [inspection.literals[0]];
  ...
  static runtimeScriptLiterals(
    ...
    while (index < inspection.arguments.length) {
      ...
      if (bunRun && !SCRIPT_EXTENSION.test(argument)) {
        ...
        return SkillProviderSubprocessScenario.resolveBunPackageScript(
      ...
      return {
        literals: SkillProviderSubprocessScenario.isScriptLikeLiteral(literal)
          ? [literal]
          : [],
        unresolved:
          !SkillProviderSubprocessScenario.isScriptLikeLiteral(literal),
      };
    }
    return { literals: [], unresolved: true };
  }

  static resolveBunPackageScript(
  ...
  static isScriptLikeLiteral(value: string): boolean {
    return SCRIPT_EXTENSION.test(value) || posix.extname(value) === '';
  }

  static resolveRepositoryScript(
  ...
  static unwrapExpression(expression: ts.Expression): ts.Expression {
    if (
      ...
      return SkillProviderSubprocessScenario.unwrapExpression(
        expression.expression,
      );
    }
    return expression;
