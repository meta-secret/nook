...
export class SkillProviderDynamicEvaluatorScenario {
  private constructor(private readonly request: DynamicEvaluatorInspection) {}

  static isAmbientDynamicEvaluator(
    inspection: DynamicEvaluatorInspection,
  ): boolean {
    return new SkillProviderDynamicEvaluatorScenario(inspection).execute();
  }
  ...
  private execute(): boolean {
    ...
    if (
      ...
      SkillProviderDynamicEvaluatorScenario.isDirectlyInvokedExpression(node)
    ) {
      ...
      const recovered =
        SkillProviderDynamicEvaluatorScenario.runtimeReceiverExpression(
          resolution,
        );
      if (ts.isCallExpression(recovered)) {
        ...
        const member =
          SkillProviderDynamicEvaluatorScenario.dynamicEvaluatorMember(
            memberInspection,
          );
        if (member !== false) {
          ...
          if (
            SkillProviderDynamicEvaluatorScenario.isAmbientRecovery(
              recoveryInspection,
            )
          )
            return true;
    ...
    if (
      SkillProviderDynamicEvaluatorScenario.isConstructorBinding(
        bindingInspection,
      )
    )
      return true;
    if (
      SkillProviderDynamicEvaluatorScenario.isUnboundedAmbientCapabilityRoot(
        inspection,
      )
    )
      return true;
    if (ts.isIdentifier(node) && AMBIENT_DYNAMIC_EVALUATORS.has(node.text)) {
      if (
        SkillProviderDynamicEvaluatorScenario.isNonRuntimeIdentifierPosition(
          node,
        )
      )
        return false;
    ...
    if (member === false) {
      if (
        SkillProviderDynamicEvaluatorScenario.isUnboundedAmbientElementAccess(
          inspection,
        )
      )
        return true;
      ...
      return SkillProviderDynamicEvaluatorScenario.isComputedCallableElementAccess(
        computedInspection,
      );
    }
  ...
  static isAmbientRecovery(request: AmbientRecoveryInspection): boolean {
    if (
    ...
    if (
      !SkillProviderDynamicEvaluatorScenario.isAmbientNamedRoot(rootInspection)
    )
      return false;
    if (
      request.member === ReflectEvaluatorMember.Get &&
      ...
      return SkillProviderDynamicEvaluatorScenario.isDirectlyInvokedExpression(
        request.node.parent,
      );
    }
    return true;
  ...
  static dynamicEvaluatorMember(
    ...
    if (ts.isElementAccessExpression(inspection.node)) {
      ...
      return SkillProviderDynamicEvaluatorScenario.staticString(resolution);
    }
    return false;
  }

  static isUnboundedAmbientElementAccess(
    inspection: DynamicEvaluatorInspection,
  ): boolean {
    if (!ts.isElementAccessExpression(inspection.node)) return false;
    const root = SkillProviderDynamicEvaluatorScenario.unwrapExpression(
  ...
  static isAmbientNamedRoot(request: AmbientNamedRootInspection): boolean {
    const root = SkillProviderDynamicEvaluatorScenario.unwrapExpression(
      request.expression,
    );
  ...
  static isUnboundedAmbientCapabilityRoot(
    inspection: DynamicEvaluatorInspection,
  ): boolean {
    const node = inspection.node;
    let ambient = false;
  ...
  static isComputedCallableElementAccess(
    ...
    if (
      ...
      !SkillProviderDynamicEvaluatorScenario.isDirectlyInvokedExpression(
        inspection.node,
      )
    ) {
      return false;
    ...
    return (
      SkillProviderDynamicEvaluatorScenario.typeCanExposeEvaluator(
        receiverType,
      ) ||
      SkillProviderDynamicEvaluatorScenario.typeCanExposeEvaluator(
        runtimeReceiverType,
      ) ||
      SkillProviderDynamicEvaluatorScenario.typeCanExposeEvaluator(resultType)
    );
  }

  static isDirectlyInvokedExpression(expression: ts.Expression): boolean {
    ...
    return (
      ...
      (ts.isTaggedTemplateExpression(parent) &&
        parent.tag === runtimeExpression)
    );
  }

  static runtimeReceiverExpression(
    resolution: RuntimeReceiverResolution,
  ): ts.Expression {
    const expression = SkillProviderDynamicEvaluatorScenario.unwrapExpression(
    ...
    const initializer =
      SkillProviderDynamicEvaluatorScenario.runtimeReceiverInitializer(
    ...
    return SkillProviderDynamicEvaluatorScenario.runtimeReceiverExpression(
      nested,
    );
  }

  static runtimeReceiverInitializer(
    ...
    for (const declaration of defaulted1) {
      ...
      if (ts.isBindingElement(declaration)) {
        ...
        const initializer =
          SkillProviderDynamicEvaluatorScenario.bindingElementInitializer(
            bindingResolution,
          );
  ...
  static bindingElementInitializer(
    resolution: BindingElementInitializerResolution,
  ): ts.Expression | false {
    const defaultInitializer =
      SkillProviderDynamicEvaluatorScenario.callableBindingDefault(resolution);
    ...
    const patternInitializer =
      SkillProviderDynamicEvaluatorScenario.bindingPatternInitializer(
        patternResolution,
      );
    ...
    const propertyName =
      SkillProviderDynamicEvaluatorScenario.bindingElementPropertyName(
        resolution.binding,
      );
    ...
    for (const property of initializer.properties) {
      if (
        ...
        SkillProviderDynamicEvaluatorScenario.propertyNameText(
          property.name,
        ) === propertyName
      ) {
  ...
  static callableBindingDefault(
    ...
    const defaultType =
      resolution.checker.getTypeAtLocation(defaultInitializer);
    return SkillProviderDynamicEvaluatorScenario.typeCanExposeEvaluator(
      defaultType,
    )
      ? defaultInitializer
      : false;
  ...
  static bindingPatternInitializer(
    ...
    if (ts.isBindingElement(parent) && parent.name === resolution.pattern) {
      ...
      return SkillProviderDynamicEvaluatorScenario.bindingElementInitializer(
        parentResolution,
      );
    }
    return false;
  ...
  static bindingElementPropertyName(
    binding: ts.BindingElement,
  ): string | false {
    if (binding.propertyName)
      return SkillProviderDynamicEvaluatorScenario.propertyNameText(
        binding.propertyName,
      );
    return ts.isIdentifier(binding.name) ? binding.name.text : false;
  }

  static propertyNameText(name: ts.PropertyName): string | false {
  ...
  static typeCanExposeEvaluator(type: ts.Type): boolean {
    if (
      type.flags &
  ...
  static isConstructorBinding(
    inspection: ConstructorBindingInspection,
  ): boolean {
    if (
      !ts.isBindingElement(inspection.node) ||
      !inspection.node.propertyName
    ) {
      return false;
    }
    if (
    ...
    return (
      SkillProviderDynamicEvaluatorScenario.staticString(resolution) ===
      DynamicEvaluatorMember.Constructor
    );
  }

  static staticString(resolution: StaticStringResolution): string | false {
    const expression = SkillProviderDynamicEvaluatorScenario.unwrapExpression(
      resolution.expression,
    );
    ...
    return SkillProviderDynamicEvaluatorScenario.staticString(nested);
  }
  ...
  static unwrapExpression(expression: ts.Expression): ts.Expression {
    if (
      ...
      return SkillProviderDynamicEvaluatorScenario.unwrapExpression(
  ...
  static isNonRuntimeIdentifierPosition(node: ts.Identifier): boolean {
    const parent = node.parent;
