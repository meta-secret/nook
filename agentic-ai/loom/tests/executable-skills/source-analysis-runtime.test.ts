...
describe('sealed source analysis runtime', () => {
  ...
  test('bounds pending requests before serializing their source', async () => {
    let releaseFirst = (): void => {};
    const firstWait = new Promise<void>((resolve) => {
      releaseFirst = resolve;
    });
    ...
    const first = runSourceAnalysisWithDependencies(firstExecution);
    await Bun.sleep(5);
    const capacity = { length: 8 };
    const pending = Array.from(capacity, () => {
      const execution = {
        dependencies,
        request: analysisRequest('\0'.repeat(MAXIMUM_SEALED_SOURCE_BYTES)),
      };
      return runSourceAnalysisWithDependencies(execution);
    });
    await Bun.sleep(5);
    const overflowExecution = {
      dependencies,
      request: analysisRequest('\0'.repeat(MAXIMUM_SEALED_SOURCE_BYTES)),
    };
    await expect(
      runSourceAnalysisWithDependencies(overflowExecution),
    ).rejects.toThrow('queue reached its capacity');
