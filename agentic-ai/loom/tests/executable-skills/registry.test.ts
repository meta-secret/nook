...
import {
  auditExecutableSkillCatalogWithDependencies,
  auditExecutableSkillRegistry,
  ExecutableSkillRegistryInspectionKind,
  type ExecutableSkillRegistryAuditRequest,
...
async function auditExecutableSkillCatalog(
  request: AuditExecutableSkillCatalogRequest,
): Promise<readonly ExecutableSkillRegistryFinding[]> {
  const dependencies = { planClosure: planExecutableSkillClosure };
...
describe('executable skill registry trust boundary', () => {
  ...
  test('deadline interrupts bounded frozen-tree discovery', async () => {
    ...
    try {
      ...
      const request: ExecutableSkillRegistryAuditRequest = {
