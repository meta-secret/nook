...
export class SkillProviderConfigRuntimeScenario {
  ...
  static isRunnableConfiguration(path: string): boolean {
    return (
  ...
  static isActionManifest(path: string): boolean {
    return /(^|\/)action\.ya?ml$/u.test(path);
  }

  static actionSourceRequiresContent(path: string): boolean {
    return new SkillProviderConfigRuntimeScenario(path).execute();
  }

  private execute(): boolean {
    const path = this.request;
    return (
      SkillProviderConfigRuntimeScenario.isActionManifest(path) ||
  ...
  static configurationRootWorkingDirectory(path: string): string {
    if (
  ...
  static resolutionCandidates(
    ...
    return [
      ...new Set(
        [
          ...bases,
          ...SkillProviderConfigRuntimeScenario.tsconfigAliasBases(request),
        ].flatMap((path) =>
          SkillProviderConfigRuntimeScenario.moduleCandidates([
            path,
            request.exactFirst,
          ]),
        ),
  ...
  static tsconfigAliasBases(
    ...
    while (true) {
      ...
      if (source && source.length > 0) {
        ...
        return SkillProviderConfigRuntimeScenario.aliasesFromTsconfig(
          aliasRequest,
        );
      }
      if (!directory) return [];
  ...
  static aliasesFromTsconfig(
    ...
    for (const [alias, targets] of Object.entries(mappings)) {
      ...
      return targets.map((target) => {
        if (
          ...
          target.includes('*') !== wildcard >= 0
        )
          throw new Error(
            `Runnable tsconfig alias target is invalid: ${alias}`,
          );
        ...
        if (candidate.startsWith('../') || candidate.startsWith('/'))
          throw new Error(
            `Runnable tsconfig alias escapes repository: ${alias}`,
          );
        return candidate;
      });
    }
    ...
    for (const parent of [...parentNames].reverse()) {
      ...
      const aliases =
        SkillProviderConfigRuntimeScenario.aliasesFromTsconfig(parentRequest);
      if (aliases.length > 0) return aliases;
    }
    return [];
  }

  static isRepositoryBackedPackageSpecifier(
  ...
  static moduleCandidates([path, exactFirst]: readonly [
    string,
    boolean,
  ]): readonly string[] {
  ...
  static specializePositionalArguments(
  ...
  static normalizeConfigurationShellSource([source, sourcePath]: readonly [
    string,
    string,
  ]): string {
    ...
    const normalized = source
      ...
      .replace(/\bnode (?:-e|--eval) (?:"[^"]*"|'[^']*')/gu, (command) => {
        const expectedDigest =
          AUDITED_NODE_EVAL_COMMAND_DIGESTS.get(sourcePath);
