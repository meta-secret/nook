...
export class SkillProviderShellAliasScenario {
  private constructor(private readonly request: AliasRequest) {}

  static applyAliasMutation(request: AliasRequest): boolean {
    if (
      request.command.value !== 'alias' &&
      request.command.value !== 'unalias'
    )
      return false;
    ...
    if (request.command.value === 'unalias') {
      ...
      for (const alias of aliases) {
        if (alias.value === '-p' || /^[A-Za-z_]\w*$/u.test(alias.value))
          continue;
  ...
  static aliasInvocationSource(request: AliasRequest): string | false {
    return new SkillProviderShellAliasScenario(request).execute();
  }
