...
export class CortexArticleAdapterBoundaryScenario {
  private constructor(
    private readonly request: CortexArticleAdapterInspection,
  ) {}

  static cortexArticleAdapterViolatesBoundary(
    inspection: CortexArticleAdapterInspection,
  ): boolean {
    return new CortexArticleAdapterBoundaryScenario(inspection).execute();
  }
  ...
  private execute(): boolean {
    ...
    const visit = (node: ts.Node): void => {
      if (ts.isImportDeclaration(node)) {
        if (!CortexArticleAdapterBoundaryScenario.isAllowedImport(node))
          invalid = true;
        else {
          const eraseRequest: EraseNodeRequest = { node, target: retained };
          CortexArticleAdapterBoundaryScenario.eraseNode(eraseRequest);
        }
        return;
      }
      if (ts.isExportDeclaration(node) && node.moduleSpecifier) {
        if (!CortexArticleAdapterBoundaryScenario.isAllowedExport(node))
          invalid = true;
        else {
          const eraseRequest: EraseNodeRequest = { node, target: retained };
          CortexArticleAdapterBoundaryScenario.eraseNode(eraseRequest);
    ...
    try {
      ExecutableSkillSource.analyze(analysisRequest);
      return false;
    } catch {
      return true;
    }
  ...
  static isAllowedImport(node: ts.ImportDeclaration): boolean {
    ...
    if (specifier === 'remark-gfm' || specifier === 'remark-parse') {
      return Boolean(
        clause.name && !clause.namedBindings && !clause.isTypeOnly,
      );
    }
    if (specifier === 'unified') {
      return Boolean(
        ...
        clause.namedBindings.elements.every(
          (element) =>
            (element.propertyName ? element.propertyName : element.name)
              .text === 'unified',
        ),
      );
    }
  ...
  static isAllowedExport(node: ts.ExportDeclaration): boolean {
  ...
  static eraseNode(request: EraseNodeRequest): void {
    request.target.fill(
      ' ',
      request.node.getFullStart(),
      request.node.getEnd(),
    );
  }
}
