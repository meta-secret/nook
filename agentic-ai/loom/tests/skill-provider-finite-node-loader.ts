...
export class SkillProviderFiniteNodeLoaderScenario {
  private constructor(private readonly request: FiniteNodeLoaderInspection) {}

  static specializeClosedFiniteNodeLoaders(
    inspection: FiniteNodeLoaderInspection,
  ): string {
    return new SkillProviderFiniteNodeLoaderScenario(inspection).execute();
  }
  ...
  private execute(): string {
    ...
    const visit = (node: ts.Node): void => {
      if (ts.isImportDeclaration(node)) {
        ...
        const capabilityReplacement =
          SkillProviderFiniteNodeLoaderScenario.finiteNodeCapabilityImportReplacement(
      ...
      if (ts.isVariableStatement(node)) {
        ...
        const processReplacements =
          SkillProviderFiniteNodeLoaderScenario.closedProcessViewReplacements(
      ...
      if (ts.isFunctionDeclaration(node)) {
        const candidate =
          SkillProviderFiniteNodeLoaderScenario.finiteNodeLoaderCandidate(node);
        if (candidate !== false) {
          ...
          const modules =
            SkillProviderFiniteNodeLoaderScenario.candidateUsesAmbientEvaluatorBindings(
              validation,
            )
              ? SkillProviderFiniteNodeLoaderScenario.finiteNodeLoaderModules(
                  validation,
                )
              : false;
          if (modules !== false) {
            ...
            const replacement: SourceReplacement = {
              ...
              replacement:
                SkillProviderFiniteNodeLoaderScenario.finiteNodeLoaderBody(
                  bodyRequest,
                ),
    ...
    const orderedReplacements =
      SkillProviderFiniteNodeLoaderScenario.descendingNonOverlappingReplacements(
        replacements,
      );
  ...
  static candidateUsesAmbientEvaluatorBindings(
  ...
  static descendingNonOverlappingReplacements(
  ...
  static finiteNodeCapabilityImportReplacement(
    ...
    for (const specifier of clause.namedBindings.elements) {
      ...
      bindings.push(
        `const ${specifier.name.text} = safeProcessChangeDirectory;`,
      );
    }
    return {
  ...
  static closedProcessViewReplacements(
    ...
    if (
      ...
      !SkillProviderFiniteNodeLoaderScenario.isGlobalProcessAccess(
        declaration.initializer,
      )
    ) {
      return false;
    ...
    const scope = SkillProviderFiniteNodeLoaderScenario.nearestFunctionScope(
    ...
    const visit = (node: ts.Node): void => {
      ...
      if (ts.isIdentifier(node) && node.text === binding.text) {
        ...
        const access =
          SkillProviderFiniteNodeLoaderScenario.outermostAccessExpression(node);
  ...
  static isGlobalProcessAccess(expression: ts.Expression): boolean {
    const candidate =
      SkillProviderFiniteNodeLoaderScenario.unwrapTransparentExpression(
        expression,
      );
    if (
  ...
  static outermostAccessExpression(node: ts.Identifier): ts.Expression {
  ...
  static finiteNodeLoaderCandidate(
    ...
    if (
      ...
      !SkillProviderFiniteNodeLoaderScenario.hasAsyncModifier(declaration) ||
      adapterInspection === false ||
      !SkillProviderFiniteNodeLoaderScenario.isExactEvaluatorAdapter(
        adapterInspection,
      )
    ) {
      return false;
  ...
  static hasAsyncModifier(declaration: ts.FunctionDeclaration): boolean {
    return Boolean(
      ts
  ...
  static isExactEvaluatorAdapter(
    ...
    const loader =
      SkillProviderFiniteNodeLoaderScenario.evaluatorLoaderBinding(
        loaderInspection,
      );
    ...
    return (
      ...
      SkillProviderFiniteNodeLoaderScenario.isLoaderReturn(returnInspection) &&
      SkillProviderFiniteNodeLoaderScenario.isIndirectEvalReturn(
        fallbackInspection,
      )
    );
  }

  static evaluatorLoaderBinding(
    ...
    if (declaration === false || !ts.isIdentifier(declaration.name))
      return false;
  ...
  static isLoaderReturn(inspection: LoaderReturnInspection): boolean {
    const expression = SkillProviderFiniteNodeLoaderScenario.returnedExpression(
      inspection.statement,
    );
  ...
  static finiteNodeLoaderModules(
    validation: FiniteNodeLoaderValidation,
    ...
    const visit = (node: ts.Node): void => {
      ...
      if (
        ts.isIdentifier(node) &&
        node.text === validation.candidate.name.text
      ) {
  ...
  static finiteNodeLoaderBody(body: FiniteNodeLoaderBody): string {
