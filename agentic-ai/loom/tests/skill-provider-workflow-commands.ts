...
export class SkillProviderWorkflowCommandsScenario {
  ...
  static workflowCommandSources(
    ...
    return new SkillProviderWorkflowCommandsScenario(request).execute();
  }

  private execute(): readonly string[] {
    const request = this.request;
    const root = SkillProviderWorkflowCommandsScenario.mapping(
      request.document,
    );
    const commands: string[] = [];
    if (request.action) {
      const [runsNode = false] = [root.runs];
      const runs = SkillProviderWorkflowCommandsScenario.mapping(runsNode);
      if (runs.using === 'composite') {
        ...
        const stepRequest: StepRunRequest = {
          defaultDirectory: '',
          defaultShell:
            SkillProviderWorkflowCommandsScenario.workflowDefaultShell(root),
          environment:
            SkillProviderWorkflowCommandsScenario.staticEnvironment(
              environmentRequest,
            ),
          steps,
          target: commands,
        };
        SkillProviderWorkflowCommandsScenario.collectStepRuns(stepRequest);
      }
      return commands;
    }
    const workflowDirectory =
      SkillProviderWorkflowCommandsScenario.defaultWorkingDirectory(root);
    const workflowShell =
      SkillProviderWorkflowCommandsScenario.workflowDefaultShell(root);
    ...
    for (const job of Object.values(
      SkillProviderWorkflowCommandsScenario.mapping(jobs),
    )) {
      const jobNode = SkillProviderWorkflowCommandsScenario.mapping(job);
      const jobDirectory =
        SkillProviderWorkflowCommandsScenario.defaultWorkingDirectory(jobNode);
      const environmentRequest: StaticEnvironmentRequest = {
      ...
      const stepRequest: StepRunRequest = {
        ...
        defaultShell:
          SkillProviderWorkflowCommandsScenario.workflowDefaultShell(jobNode) ||
          workflowShell ||
          SkillProviderWorkflowCommandsScenario.workflowRunnerDefaultShell(
            jobNode,
          ),
        environment:
          SkillProviderWorkflowCommandsScenario.staticEnvironment(
  ...
  static workflowGithubScriptSources(
    request: WorkflowCommandRequest,
  ): readonly string[] {
    const root = SkillProviderWorkflowCommandsScenario.mapping(
      request.document,
    ...
    if (request.action) {
      ...
      const runs = SkillProviderWorkflowCommandsScenario.mapping(runsNode);
      const [steps = false] = [runs.steps];
      return SkillProviderWorkflowCommandsScenario.githubScriptSources(steps);
    }
    const [jobs = false] = [root.jobs];
    return Object.values(
      ...
      const [steps = false] = [
        SkillProviderWorkflowCommandsScenario.mapping(job).steps,
      ];
      return SkillProviderWorkflowCommandsScenario.githubScriptSources(steps);
    });
  }
  ...
  static githubScriptSources(steps: ConfigurationNode): readonly string[] {
    ...
    return steps.flatMap((step) => {
      ...
      const source =
        SkillProviderWorkflowCommandsScenario.mapping(withNode).script;
  ...
  static collectStepRuns(request: StepRunRequest): void {
    ...
    for (const step of request.steps) {
      ...
      SkillProviderWorkflowCommandsScenario.assertSafeWorkflowShell(
        defaulted10,
      );
      SkillProviderWorkflowCommandsScenario.assertNoProtectedCommandFileMutation(
        node.run,
      );
      const directory =
  ...
  static assertNoProtectedCommandFileMutation(source: string): void {
    const words = SkillProviderShellTokenizerScenario.tokenizeShell(
      source,
    ).filter((token) => typeof token !== 'string');
    if (
      !words.some(
        (word) =>
          word.value === '$GITHUB_ENV' || word.value === '${GITHUB_ENV}',
      )
    )
      return;
  ...
  static assertSafeWorkflowShell(shell: ConfigurationNode | false): void {
    if (shell === false)
      throw new Error(
  ...
  static workflowRunnerDefaultShell(
  ...
  static staticEnvironment(
    ...
    for (const [name, value] of Object.entries(
      SkillProviderWorkflowCommandsScenario.mapping(environment),
    ))
      if (
  ...
  static defaultWorkingDirectory(node: ConfigurationMapping): string {
    const [defaults = false] = [node.defaults];
    const [runNode = false] = [
      SkillProviderWorkflowCommandsScenario.mapping(defaults).run,
    ];
    const run = SkillProviderWorkflowCommandsScenario.mapping(runNode);
    return typeof run['working-directory'] === 'string'
      ? run['working-directory']
      : '';
  }

  static workflowDefaultShell(node: ConfigurationMapping): string | false {
    const [defaults = false] = [node.defaults];
    const [runNode = false] = [
      SkillProviderWorkflowCommandsScenario.mapping(defaults).run,
    ];
    const runDefaults = SkillProviderWorkflowCommandsScenario.mapping(runNode);
    if (!('shell' in runDefaults)) return false;
    const shell = runDefaults.shell;
    SkillProviderWorkflowCommandsScenario.assertSafeWorkflowShell(shell);
