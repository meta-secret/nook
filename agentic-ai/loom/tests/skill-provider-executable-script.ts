...
import { SkillProviderFiniteNodeLoaderScenario } from './skill-provider-finite-node-loader.ts';

export class SkillProviderExecutableScriptScenario {
  private constructor(
    private readonly request: ExecutableProviderReferenceInspection,
  ) {}

  static executableScriptViolatesBoundary(
    ...
    const finiteNodeLoaderSource =
      SkillProviderFiniteNodeLoaderScenario.specializeClosedFiniteNodeLoaders(
  ...
  static executableSourceReferencesProvider(
    inspection: ExecutableProviderReferenceInspection,
  ): boolean {
    return new SkillProviderExecutableScriptScenario(inspection).execute();
  }
  ...
  private execute(): boolean {
    ...
    if (TYPESCRIPT_JAVASCRIPT_SOURCE.test(inspection.path)) {
      ...
      return BOUNDARY_TRANSPILER.scanImports(importSource).some((imported) =>
        SkillProviderBoundaryScenario.referencesSkillProvider(imported.path),
      );
    }
  ...
  static shellExecutableLaunchesUnprovenScript(source: string): boolean {
