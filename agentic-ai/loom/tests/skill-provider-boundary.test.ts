...
test('rejects every unbounded ambient module loader route', () => {
  const unboundedLoaders = [
    ...
    "const load = (process as NodeJS.Process)['getBuiltinModule'];",
    ...
    '(globalThis.process as NodeJS.Process).getBuiltinModule(moduleName);',
    '(import.meta as ImportMeta).require(modulePath);',
    "const load = (import.meta as ImportMeta)['require'];",
    ...
    'const importMetaRoot = import.meta as ImportMeta;',
    'const processRoot = condition ? process : fallback;',
    'const moduleRoot = (noop(), module);',
...
test('resolves globalThis bindings in their exact lexical scopes', () => {
  for (const source of [
    ...
    'for (const globalThis in values) { globalThis.require(path); }',
    'try {} catch (globalThis) { globalThis.require(path); }',
    'switch (value) { case 1: { const globalThis = local; globalThis.require(path); } }',
    ...
    'async function inspect() { await using globalThis = local; globalThis.require(path); }',
    'namespace Local { const globalThis = local; globalThis.require(path); }',
    'namespace Local { var globalThis = local; globalThis.require(path); }',
  ]) {
    const inspection: SkillProviderImportInspection = {
