...
export class SkillProviderConfigApplicationScenario {
  private constructor(
    private readonly request: ConfigurationSourceBoundaryRequest,
  ) {}

  static isApplicationDependency(path: string): boolean {
    return (
  ...
  static isConfigurationProviderPackage(path: string): boolean {
    return path === PROVIDER_PACKAGE || path === HOST_PACKAGE;
  }

  static expectedProviderReferences(allPaths: readonly string[]): string[] {
    return [
      PROVIDER_PACKAGE,
      HOST_PACKAGE,
      '.task/agentic-ai.yml',
  ...
  static isAuthorizedApplicationEdge(edge: ApplicationConsumerEdge): boolean {
    ...
    if (
      ...
      (edge.dependency === ARTICLE_ACTION ||
  ...
  static assertConfigurationSourceBoundary(
    request: ConfigurationSourceBoundaryRequest,
  ): void {
    return new SkillProviderConfigApplicationScenario(request).execute();
  }
  ...
  private execute(): void {
    ...
    if (
      SkillProviderConfigApplicationScenario.mightUseRuntimeLoader(
        request.source,
      ) &&
  ...
  static mightUseRuntimeLoader(source: string): boolean {
