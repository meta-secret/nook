...
const SHARED_AUTHORITY_PATH = '.cortex/shared/AGENTS.md';
...
const EXECUTION_LIST_DIRECTION =
  /^(?:agents?|team agents?|workers?|subagents?|gizmo(?: prime)?|ai|sre|security|development core|web development)\s+(?:may|can|should|must|shall|need to|are allowed to|is allowed to|are permitted to|is permitted to|are required to|is required to)(?:\s+not)?(?:\s+ask\s+(?:an?\s+)?(?:team\s+)?(?:agent|worker)s?\s+to)?\s*:$/iu;
const QUALIFIED_AUTHORITY_DIRECTION =
  /(\b(?:agents?|team agents?|workers?|subagents?|gizmo(?: prime)?|ai|sre|security|development core|web development)\s+(?:may|can|should|must|shall|need to|are allowed to|is allowed to|are permitted to|is permitted to|are required to|is required to))\s*,\s*(?:(?:only\s+)?when|while|during|for|if|unless|after|before)\b[^,.;!?]*,\s*/iu;
const AUTHORITY_ACTOR =
  /^(?:agents?|team agents?|workers?|subagents?|gizmo(?: prime)?|ai|sre|security|development core|web development)\b/iu;
...
const ELLIPTICAL_AUTHORITY_DIRECTION =
  /^(?:may|can|should|must|shall|need to|are allowed to|is allowed to|are permitted to|is permitted to|are required to|is required to)\b/iu;
...
export function auditTeamAuthorities(
  ...
  for (const path of [REPOSITORY_AUTHORITY_PATH, SHARED_AUTHORITY_PATH]) {
    const authorityPath = join(request.repoRoot, path);
    if (existsSync(authorityPath)) {
      appendLocalExecutionGrantFindings({
        findings,
        source: readFileSync(authorityPath, 'utf8'),
        path,
        authorityName: CortexAuthorityName.Team,
      });
    }
...
function appendLocalExecutionGrantFindings(
  ...
  for (const statement of markdownStatements(request.source)) {
    ...
    const affirmativeLocalExecution =
    ...
    const localExecutionDirection =
      ...
      affirmativeLocalExecution;
    if (
      (commandDirection &&
        (affirmativeLocalExecution || !HOSTED_EXECUTION.test(statement))) ||
...
function inspectAuthorityStatements(
  ...
  for (const node of request.nodes) {
    ...
    if (node.type === 'html') {
      appendAuthoritySentences({
        statements: request.statements,
        text: visibleHtmlText(node.value).replace(/\s+/gu, ' ').trim(),
      });
    }
    listGrant = false;
  }
...
function appendAuthoritySentences(
  ...
  for (const sentence of request.text.split(/(?<=[.!?])\s+/u)) {
    const clauses = sentence
      .split(
        /\s*;\s*|\s*,?\s+(?:but|yet)\s+|\s+and\s+(?=(?:may|can|should|must|shall|need to|are allowed to|is allowed to|are permitted to|is permitted to|are required to|is required to)\b)/iu,
      )
