...
export class ModuleResourceContainment {
  private constructor(private readonly request: EvidenceCoverageRequest) {}

  static resourceClaimListsOverlap(request: ResourceClaimListPair): boolean {
  ...
  static moduleDeliveryNodesConflict(pair: ModuleDeliveryNodePair): boolean {
    ...
    return (
      ModuleResourceContainment.resourceClaimListsOverlap(writeWrite) ||
      ModuleResourceContainment.resourceClaimListsOverlap(firstWriteRead) ||
      ModuleResourceContainment.resourceClaimListsOverlap(secondWriteRead)
    );
  }

  static moduleDeliveryNodesAreOrdered(
  ...
  static buildModuleDeliveryReachability(
  ...
  static sortedModuleDeliveryPrecedence(
    ...
    for (const value of values)
      byIdentity.set(
        ModuleResourceContainment.moduleDeliveryPrecedenceIdentity(value),
        value,
      );
    return [...byIdentity.keys()].sort().map((identity) => {
      const value = byIdentity.get(identity);
      if (!value)
        throw new Error(`Execution constraint ${identity} is missing.`);
      return value;
    });
  }

  static uncoveredEvidenceClaims(
    request: EvidenceCoverageRequest,
  ): readonly string[] {
    return new ModuleResourceContainment(request).execute();
  }
  ...
  private execute(): readonly string[] {
    ...
    return request.evidenceSurface.filter((evidenceClaim) => {
      ...
      return !request.read.some((readClaim) => {
        ...
        return ModuleResourceContainment.resourceClaimContainsClaim(
          containmentRequest,
        );
      });
    });
  ...
  private static resourceClaimContainsClaim(
    ...
    if (request.coveringClaim.endsWith('/**')) {
      ...
      return (
        coveredRoot === coveringRoot ||
        coveredRoot.startsWith(`${coveringRoot}/`)
      );
    }
    if (request.coveringClaim.startsWith('**/')) {
      ...
      return ModuleResourceContainment.basenamePatternContains(basenameRequest);
    }
  ...
  private static basenamePatternContains(
    request: BasenameContainmentRequest,
  ): boolean {
    ...
    if (!request.coveredBasename.startsWith('*')) {
      ...
      return request.coveredBasename.endsWith(
        request.coveringBasename.slice(1),
      );
    }
    return request.coveringBasename === request.coveredBasename;
  }
