...
export class LoomFailure extends Error {
  ...
  constructor(args: LoomFailureArgs) {
    ...
    super(
      detail.kind === LoomFailureDetailKind.Text
        ? detail.text
        : LoomFailure.defaultMessage(code),
      { cause },
    );
  ...
  private static defaultMessage(code: LoomFailureCode): string {
    switch (code) {
  ...
  static fromCause(args: LoomFailureFromCauseRequest): LoomFailure {
  ...
  static async withCode<T>(args: WithLoomFailureCodeRequest<T>): Promise<T> {
    try {
      return await args.action();
    } catch (cause) {
      throw LoomFailure.fromCause({
        code: args.code,
  ...
  static raise(code: LoomFailureCode): never {
  ...
  static detail(args: LoomFailureDetailArgs): never {
