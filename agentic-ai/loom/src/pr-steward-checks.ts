...
export class PrStewardChecksReader implements PrStewardCompletionReader {
  ...
  async read(
    ...
    const result = await this.command.run({
      ...
      arguments: [
        ...
        'graphql',
        '-f',
        `query=${PrStewardChecksReader.#query}`,
        '-F',
    ...
    if (
      result.kind !== PrStewardCommandResultKind.Success ||
    ...
    try {
      ...
      const repository = this.#object(
        this.#field({ record: data, key: 'repository' }),
      );
      const pr = this.#object(
        this.#field({ record: repository, key: 'pullRequest' }),
      );
      ...
      if (this.#field({ record: commit, key: 'oid' }) !== headSha)
        throw new PrStewardGithubUnavailableError({ cause: false });
      const summary = this.#rollup(
        this.#field({ record: commit, key: 'statusCheckRollup' }),
      );
      return {
        headSha,
        url,
        ...
        unknownConclusions: summary.unknown,
        state:
          state === 'MERGED'
            ? PrStewardCompletionState.Merged
            : state === 'CLOSED'
              ? PrStewardCompletionState.Closed
  ...
  #rollup(value: UntrustedYamlNode): CheckSummary {
    ...
    const contexts = this.#object(
      this.#field({ record: this.#object(value), key: 'contexts' }),
    );
  ...
  #counts(request: {
    ...
    const expected = this.#field({
      record: request.contexts,
      key: `${request.kind}Count`,
    });
    const groups = this.#field({
      record: request.contexts,
      key: `${request.kind}CountsByState`,
    });
    if (
