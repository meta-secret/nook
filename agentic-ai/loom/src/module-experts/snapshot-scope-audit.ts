...
export class ModuleExpertSnapshotScope {
  private constructor(
    private readonly request: AuditModuleExpertSnapshotScopesArgs,
  ) {}
  static audit(
    args: AuditModuleExpertSnapshotScopesArgs,
  ): readonly ModuleExpertSnapshotScopeFinding[] {
    return new ModuleExpertSnapshotScope(args).execute();
  }
  private execute(): readonly ModuleExpertSnapshotScopeFinding[] {
    ...
    for (const profile of args.profiles) {
      ...
      const contextMatches = this.sameOrderedPaths(contextComparison);
      if (!contextMatches) {
      ...
      if (profile.name === WEB_EXPERT_NAME) {
        ...
        if (!this.sameOrderedPaths(authorityComparison)) {
          const finding: ModuleExpertSnapshotScopeFinding = {
        ...
        if (!this.sameOrderedPaths(scopeComparison)) {
          const finding: ModuleExpertSnapshotScopeFinding = {
        ...
        if (!this.sameOrderedPaths(allowedContextComparison)) {
          const finding: ModuleExpertSnapshotScopeFinding = {
        ...
        if (!this.sameOrderedPaths(skillComparison)) {
          const finding: ModuleExpertSnapshotScopeFinding = {
    ...
    if (!this.sameOrderedPaths(boundaryComparison)) {
      const finding: ModuleExpertSnapshotScopeFinding = {
  ...
  private sameOrderedPaths(paths: OrderedSnapshotPaths): boolean {
