...
export class ModuleExpertRuntimeRouting {
  private constructor(
    private readonly request: AuditModuleExpertRuntimeRoutingArgs,
  ) {}
  static audit(
    args: AuditModuleExpertRuntimeRoutingArgs,
  ): readonly ModuleExpertRuntimeRoutingFinding[] {
    return new ModuleExpertRuntimeRouting(args).execute();
  }
  private execute(): readonly ModuleExpertRuntimeRoutingFinding[] {
    const args = this.request;
    const findings: ModuleExpertRuntimeRoutingFinding[] = [];
    const moduleExpertRuntimeNames = this.constructedRuntimeNames(
      args.moduleExpertCliSource,
    );
    const moduleExpertCallNames = this.calledFunctionNames(
      args.moduleExpertCliSource,
    );
    const trustedRuntimeCallNames = this.calledFunctionNames(
      args.trustedRuntimeSource,
    );
    const trustedRuntimeNames = this.constructedRuntimeNames(
      args.trustedRuntimeSource,
    );
    if (
  ...
  private constructedRuntimeNames(source: string): readonly string[] {
  ...
  private calledFunctionNames(source: string): readonly string[] {
    const sourceFile = ts.createSourceFile(
      'runtime-routing.ts',
      source,
      ts.ScriptTarget.Latest,
