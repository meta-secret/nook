...
import { CortexIdentifierSyntax } from '../agent-workflow/cortex-references.ts';

import { CortexDocumentStructure } from '../../../../.cortex/teams/ai/dynamic-skills/cortex-document-map/scripts/src/cortex-document-structure.ts';

export class CortexIdentifierCatalog {
  private constructor(private readonly request: string) {}

  static auditCortexIdentifierRegistry(
    repoRoot: string,
  ): CortexIdentifierAudit {
    return new CortexIdentifierCatalog(repoRoot).execute();
  }

  private execute(): CortexIdentifierAudit {
    ...
    if (!existsSync(registryPath)) {
      findings.push(
        CortexIdentifierCatalog.finding(
          'Cortex identifier registry is missing.',
        ),
      );
    ...
    try {
      ...
      findings.push(
        CortexIdentifierCatalog.finding(
          'Cortex identifier registry is not valid JSON.',
        ),
      );
      return { registry: false, findings };
    }
    ...
    for (const [index, candidate] of value.entries.entries()) {
      ...
      if (!decoded) {
        findings.push(
          CortexIdentifierCatalog.finding(
            `Cortex identifier entry ${index + 1} is invalid.`,
          ),
        );
        continue;
      }
    ...
    CortexIdentifierCatalog.validateEntries(validationArgs);
  ...
  static auditCortexIdentifierStability(
  ...
  static registeredCortexIdentifiers(repoRoot: string): ReadonlySet<string> {
    const audit =
      CortexIdentifierCatalog.auditCortexIdentifierRegistry(repoRoot);
    return audit.registry && audit.findings.length === 0
      ? CortexIdentifierCatalog.cortexIdentifierSet(audit.registry)
      : new Set<string>();
  }

  static registeredCortexIdentifiersAtCommit(
    ...
    try {
      ...
      return CortexIdentifierCatalog.registeredCortexIdentifiers(args.repoRoot);
    } catch {
      return new Set<string>();
    }
  }

  static publishedCortexIdentifiersAtCommit(
    args: RegisteredCortexIdentifiersAtCommitArgs,
  ): ReadonlySet<string> {
  ...
  private static validateEntries(args: ValidateEntriesArgs): void {
    const ids = new Set<string>();
    ...
    for (const entry of args.entries) {
      if (!CortexIdentifierCatalog.validEntryIdentifier(entry)) {
        args.findings.push(
          CortexIdentifierCatalog.finding(
            `Cortex identifier ${entry.id} has an invalid shape.`,
          ),
        );
      }
      if (ids.has(entry.id)) {
        args.findings.push(
          CortexIdentifierCatalog.finding(
            `Cortex identifier ${entry.id} is duplicated.`,
          ),
        );
      }
      if (locators.has(entry.locator)) {
        args.findings.push(
          CortexIdentifierCatalog.finding(
            `Cortex locator ${entry.locator} is duplicated.`,
          ),
        );
      }
      if (authorities.has(entry.authority)) {
        args.findings.push(
          CortexIdentifierCatalog.finding(
            `Cortex authority ${entry.authority} is duplicated.`,
          ),
        );
      }
      ids.add(entry.id);
      ...
      if (
        ...
        args.findings.push(
          CortexIdentifierCatalog.finding(
            `Cortex identifier ${entry.id} has an invalid category.`,
          ),
        );
      }
      ...
      CortexIdentifierCatalog.validateLocator(locatorArgs);
    }
  }

  private static validEntryIdentifier(entry: CortexIdentifierEntry): boolean {
    return entry.kind === CortexIdentifierKind.Category
      ? CortexIdentifierSyntax.validCortexCategoryIdentifier(entry.id)
      : CortexIdentifierSyntax.validCortexScopedIdentifier(entry.id);
  }

  private static validateLocator(args: ValidateLocatorArgs): void {
    ...
    if (
      ...
      args.findings.push(
        CortexIdentifierCatalog.finding(
          `Cortex locator ${args.entry.locator} is invalid.`,
        ),
      );
      return;
    }
    ...
    if (path.posix.normalize(relativePath) !== relativePath) {
      args.findings.push(
        CortexIdentifierCatalog.finding(
          `Cortex locator ${args.entry.locator} is not canonical.`,
        ),
      );
      return;
    }
    if (!absolutePath.startsWith(cortexRoot) || !existsSync(absolutePath)) {
      args.findings.push(
        CortexIdentifierCatalog.finding(
          `Cortex locator ${args.entry.locator} does not exist.`,
        ),
      );
      return;
    }
    ...
    if (args.entry.kind === CortexIdentifierKind.Item && !fragment) {
      args.findings.push(
        CortexIdentifierCatalog.finding(
          `Cortex item ${args.entry.id} must name a heading fragment.`,
        ),
      );
      return;
    }
    ...
    if (
      ...
      args.findings.push(
        CortexIdentifierCatalog.finding(
          `Cortex locator ${args.entry.locator} has no matching heading.`,
        ),
      );
    }
  }

  private static finding(message: string): CortexIdentifierFinding {
