...
export class CortexMarkdownInventory {
  ...
  static listPersistentCortexMarkdownFiles(root: string): string[] {
    return CortexMarkdownInventory.listCortexMarkdownFiles(root).filter(
      (filePath) => {
  ...
  static listCortexMarkdownFiles(root: string): string[] {
    const out: string[] = [];
    const stack = [root];
    ...
    while (stack.length > 0) {
      ...
      for (const entry of readdirSync(current, directoryReadOptions)) {
        ...
        if (entry.isDirectory()) {
          ...
          if (
            CortexMarkdownInventory.isExecutableSkillScriptsDirectory(
              scriptsDirectoryArgs,
            )
          )
            continue;
          stack.push(full);
  ...
  private static nestedSkillCards(scriptsRoot: string): readonly string[] {
  ...
  private static isExecutableSkillScriptsDirectory(
    ...
    if (!CortexMarkdownInventory.isRegularFile(skillPath)) return false;
    ...
    return (
      CortexMarkdownInventory.EXECUTABLE_SKILL_PROJECT_FILES.every((name) =>
        CortexMarkdownInventory.isRegularFile(path.join(args.candidate, name)),
      ) &&
      ['src', 'tests'].every((name) =>
        CortexMarkdownInventory.isRegularDirectory(
          path.join(args.candidate, name),
        ),
      ) &&
      CortexMarkdownInventory.nestedSkillCards(args.candidate).length === 0
    );
  }

  private static isRegularFile(filePath: string): boolean {
    try {
  ...
  private static isRegularDirectory(directoryPath: string): boolean {
    try {
