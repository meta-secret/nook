...
- **Heavy work runs remotely on GitHub Actions:** Heavy builds and tests do not run locally. Trusted same-repository pull requests and every explicit Main job use disposable ARC Pods with a persistent node-local rootless BuildKit shard. Fork pull requests, Dependabot, releases, and emergency fallbacks retain GitHub-hosted capacity. See [dynamic-skills/github-actions-only-validation.md](dynamic-skills/github-actions-only-validation.md) and [workflows/remote-execution.md](workflows/remote-execution.md).
