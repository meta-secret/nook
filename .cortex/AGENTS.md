...
* [design-docs/hive-isolated-agent-platform.md](design-docs/hive-isolated-agent-platform.md) — **Stateful isolated AI-agent platform**: k0s/Kata topology, Neo4j task DAG, brokered credentials, disposable workers, complete Main-repair delivery, caching, and Taskfile operations.
...
* **Line coverage threshold (90%):** `task rust:coverage:check` measures
  `nook-core + nook-auth2 + nook-replication + nook-event-log` and fails below
