...
3. Push the branch; open a PR with summary and test plan. `pr.yml` runs `task ci:pr:publish` plus a Cloudflare preview; `main.yml` runs `task ci:main:publish` (verify ‖ build ‖ e2e in parallel, GitHub Pages) after squash merge.
