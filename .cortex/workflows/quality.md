...
14. **Never pipe long-running commands through `| tail`/`| head`/`| grep`.** These buffer until the command exits, so a multi-minute `task setup` / `task check` / bake looks hung with zero live output — you cannot tell progress from a stall. Run the command directly (its full output streams and is captured to the terminal file automatically), or redirect to a log file (`... > /tmp/x.log 2>&1`) and inspect the file. Filtering pipes are never a performance optimization here; they only add uncertainty. Filter *after* the run completes, on the saved output.
