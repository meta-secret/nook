...
10. **CI policy:** `.github/workflows/pr.yml` — `task ci:pr` (prepare, verify ‖ web build, **full local-provider e2e**, Cloudflare preview). `.github/workflows/main.yml` — prepare, verify ‖ build, **full local-provider e2e**, push toolchain to GHCR (`ci:main:publish`). `.github/workflows/release-v1.yml` — main-equivalent verify/e2e, then Cloudflare Pages deploy for `v1.nokey.sh` from `release/v1`. Nightly: `ci:nightly:e2e` (sync-live). `.github/workflows/runner-cleanup.yml` runs daily Nook-runner Docker cleanup with `docker system prune --force --volumes`. E2e uses `127.0.0.1:5173` inside each container — no host `-p 5173`. All jobs use self-hosted `nook` runners. Agents: follow [pull-requests.md § Agent pipeline](pull-requests.md#agent-pipeline).
11. **Local CI strategy:** When functionality for the current iteration is complete, agents push/open/update the PR before the long final local gate, then run `task check` (minimum) locally while remote CI runs. Local `task ci:pr` mirrors PR gates and is much faster on warm Docker caches than repeated cold remote runs; use it for high-risk web changes and after broad remote CI failures before merge/handoff. See [coding-bro.md](coding-bro.md), [pull-requests.md § Local checks](pull-requests.md#4-local-checks), and [ci-pipeline.md § Local vs remote CI](ci-pipeline.md#local-vs-remote-ci).
12. Verify the latest pushed head locally with `task check` before merge or handoff; escalate to `task ci:pr` after remote failure or for high-risk web changes.
