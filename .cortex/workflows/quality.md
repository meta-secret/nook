...
10. **CI policy:** `.github/workflows/pr.yml` — `toolchain` build (image cached for the run), parallel `check` (`task ci:verify`) and `preview` (`task web:build` + Cloudflare; loads image from cache, not GHCR). `.github/workflows/main.yml` — one job (`ci`): `task check`, web build, e2e; then GitHub Pages deploy.
