...
10. **CI policy:** `.github/workflows/pr.yml` — `task ci:pr` (prepare, verify ‖ web build, **full stub e2e**, Cloudflare preview). `.github/workflows/main.yml` — prepare, verify ‖ build, **full stub e2e**, push toolchain to GHCR (`ci:main:publish`). Nightly: `ci:nightly:e2e` (sync-live). `.github/workflows/runner-cleanup.yml` runs daily Nook-runner Docker cleanup with `docker system prune --force --volumes`. E2e uses `127.0.0.1:5173` inside each container — no host `-p 5173`. All jobs use self-hosted `nook` runners. Agents: follow [pull-requests.md § Agent pipeline](pull-requests.md#agent-pipeline).
