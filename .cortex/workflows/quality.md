...
10. **CI policy:** `.github/workflows/pr.yml` — `task ci:pr` (prepare, verify ‖ web build, **full stub e2e**, buildcache refresh, Cloudflare preview). `.github/workflows/main.yml` — prepare, verify ‖ build, then **full stub e2e** in one container and push `:latest` after green CI. Nightly: `ci:nightly:e2e` (sync-live). E2e uses `127.0.0.1:5173` inside each container — no host `-p 5173`. All jobs use GitHub-hosted `ubuntu-latest` runners (no Blacksmith). Agents: follow [pull-requests.md § Agent pipeline](pull-requests.md#agent-pipeline).
