...
10. **CI policy:** `.github/workflows/pr.yml` — `task ci:pr:publish` (prepare, then verify ‖ web build in parallel, buildx push `:latest`). `.github/workflows/main.yml` — `task ci:main:publish` (prepare, verify ‖ web build in parallel, then e2e, buildx push `:latest`).
