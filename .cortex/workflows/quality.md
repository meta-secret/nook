...
10. **CI policy:** `.github/workflows/pr.yml` runs `task check` and Cloudflare previews on pull requests. `.github/workflows/main.yml` runs `task check`, production web build, Playwright e2e, and GitHub Pages deploy on push to `main` after squash merge.
