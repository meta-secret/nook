...
10. **CI policy:** `.github/workflows/pr.yml` — `task ci:pr` (prepare, verify ‖ web build, **full stub e2e**, Cloudflare preview). `.github/workflows/main.yml` — prepare, verify ‖ build, **full stub e2e**, push toolchain to GHCR (`ci:main:publish`). Nightly: `ci:nightly:e2e` (sync-live). E2e uses `127.0.0.1:5173` inside each container — no host `-p 5173`. All jobs use self-hosted `nook` runners. Agents: follow [pull-requests.md § Agent pipeline](pull-requests.md#agent-pipeline).
...
19. **Coverage reporting:** `task rust:coverage:export` exports `nook-core` coverage artifacts locally (`summary.txt`, `summary.json`, `lcov.info`, and `coverage-floor.json`). PR CI reuses the already-built PR image via `task docker:extract:coverage`, builds the base branch image for comparison, uploads both reports as `nook-core-coverage`, and posts a sticky PR comment comparing PR line coverage with the base branch. The Docker image build remains the enforcement point for the 90% floor; the PR comment is the readable trend/diff surface.
