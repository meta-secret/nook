...
Choose Gizmo Prime for mission planning, delegation, integration, review
...
- Feature-slice Gizmos are non-team controllers created by Gizmo Prime. Each
  coordinates Team Agents for exactly one semantic PR slice and returns a typed
  handoff; it does not own worker lifecycle or delivery state.

## Team routes

Gizmo Prime chooses the team agent that owns the requested functionality.
...
1. Gizmo Prime keeps delivery-control work in its own context.
2. Gizmo Prime recursively discovers bounded worker-executable team and provider task
   records. Parent-owned control operations remain outside that graph.
3. Gizmo Prime assigns exactly one team identity to each worker-executable team or
   provider task.
4. Loom/Nook computes eligible candidates, conflicts, capacity, leases, and
   exact frontier data; Gizmo Prime validates the batch, selects task records,
   admission-authorizes one exact attempt ID per selection, and freezes and owns
   those attempts' starting frontiers.
5. Gizmo Prime supplies each authorized contract to the active harness. The harness
   alone creates and operates attempts and never selects or admits records or
   snapshots or changes frontiers.
6. An unknown provider is a late mutation: it returns to Gizmo Prime and invalidates
...
8. Cycles fail closed and return the blocked dependency to Gizmo Prime.
9. Wave selection includes claims in every unreleased lease.
10. Worker termination does not release a lease.
11. Gizmo Prime records each conclusive output disposition; Loom/Nook then releases
...
17. Return foreign-team requirements to Gizmo Prime.
...
For a multi-team request, Gizmo Prime loads only its own graph. Every reached
...
owned Gizmo Prime control operations remain outside the worker graph and receive
