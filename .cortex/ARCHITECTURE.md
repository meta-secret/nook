...
- **Rust/WASM lineage**: `rust-base` + manifest-only chef cooking exposes a lightweight WASM dependency boundary, while native verification extends it with nextest/clippy/coverage profiles. Hosted PR CI runs native coverage and WASM clippy/build/Node tests on independent runners. Multi-GB source-sensitive `target/` snapshots stay local to each solve; hosted CI persists the stable native and WASM dependency boundaries in `nook-rust-deps-v2` and `nook-rust-wasm-deps-v1`, avoiding slower source-snapshot transfers. Explicit `task rust:*` / `task wasm:*` commands load the source-sealed `nook-rust:local` image on demand; browser-only WASM tests and mounted Vite development use `nook-rust-browser:local`.
...
- **BuildKit caching crosses hosted VMs through GHA only:** local commands keep using the selected builder's local content store with `GHA_CACHE_ENABLED` empty. Hosted CI exports stable native and WASM dependency/tool boundaries to `nook-rust-deps-v2` and `nook-rust-wasm-deps-v1` with `mode=max`, because cargo-chef and profile warmups are intermediate stages that `mode=min` cannot reliably restore on another VM. Source branches run on separate runners and are deliberately not exported because transferring their multi-GB snapshots is slower than executing them. Web dependencies, browser-free web, and e2e web use distinct scopes and `mode=max`. Cache export errors are non-fatal because cache is an optimization. Registry cache manifests remain forbidden.
