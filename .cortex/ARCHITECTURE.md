...
- **CI parity:** `.github/actions/nook-docker-setup` raises Linux watcher limits, creates and exports the hosted `docker-container` Buildx builder for both wrapped and direct Task callers, exposes the Actions cache-service runtime, enables the Bake GHA scopes, and—when the repository cache key is available—opens a strict-host-key-checked SSH tunnel from the Docker bridge to the persistent Redis service. Forks and other no-secrets contexts retain the job-local Redis fallback. Delivery does not depend on the daemon's default image store and never rewrites daemon configuration or restarts Docker.
...
- **Rust compiler fallback cache:** every Rust/WASM compile is wrapped by pinned `sccache`. Local Task runs idempotently keep one Docker-host-only Redis container running; GitHub-hosted runners set `SCCACHE_REDIS_MODE=external` after tunneling to the server deployment in `infra/`. Short-lived sccache daemons use per-container Unix sockets and share only Redis storage. The endpoint remains `host.docker.internal:${SCCACHE_REDIS_PORT:-6380}` inside build/runtime containers. `nook-app/docker/resolve-docker-host-ip.sh` resolves a numeric Docker-host IPv4 address and Task passes it to Bake and every Rust-capable runtime container; no magic gateway token is used. Local Redis publishes only on `127.0.0.1` under Docker Desktop and only on the default Docker bridge gateway under Linux Engine. Server Redis publishes only on server loopback and is reached through authenticated SSH forwarding. The server cache uses AOF persistence, a 12 GiB memory ceiling, and `allkeys-lru`. This cache survives Docker-layer invalidation and dependency graph changes, but Cargo downloads, compiler version, target, profile, flags, coverage instrumentation, and non-cacheable link outputs still produce misses.
...
persistent server cache through SSH. A no-secrets job starts an empty local
...
| Redis compiler cache | `task sccache:ensure` either creates/starts local `nook-sccache-redis` (macOS loopback / Linux bridge gateway, AOF, 8 GiB default, LRU) or verifies the external SSH tunnel selected by CI. `task sccache:stats` reports keys, memory, hits, misses, and evictions for either mode. `task infra:redis:stats` inspects the server's AOF-backed 12 GiB cache. |
