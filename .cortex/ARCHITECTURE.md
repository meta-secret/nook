...
- **Rust compiler cache:** Rust/WASM compiles are wrapped by pinned `sccache`. Authorized local builds and Hive use authenticated SeaweedFS S3 at `https://sccache.dev.nokey.sh` (path-style, bucket `nook-sccache`) when `.nook/cache/sccache-access-key` and `sccache-secret-key` exist. Traefik terminates publicly trusted TLS on `:443` and forwards only to loopback SeaweedFS S3 (`127.0.0.1:8333`). Runtime Docker commands mount the S3 keys as optional secrets, and short-lived sccache daemons use per-container Unix sockets. Hosted delivery jobs deliberately receive no S3 credentials and bypass sccache, so their BuildKit vertices match the secret-free vertices restored by PRs.

**Main seeds BuildKit cache visibility.** Main alone refreshes the shared Rust, WASM, web, and e2e registry refs after lane verification; every PR job is read-only so branch generations cannot poison the authoritative Main lineage. Main's compiler layers remain S3-secret-free so PR restores match. Clients authenticate to `registry.dev.nokey.sh` with repository secrets (`NOOK_REGISTRY_USERNAME` / `NOOK_REGISTRY_PASSWORD`).
...
exception is SeaweedFS S3-backed `sccache`: authorized local runtime builds and
Hive can use the authenticated HTTPS endpoint. Hosted delivery jobs bypass
...
| Rust crate dependencies | **cargo-chef** release WASM cooks plus a manifest-keyed release test warm-up, then a source-sensitive `cargo build --tests --release` sibling that matches `wasm-pack test`'s unit graph (dev-dependencies differ from `wasm-pack build --lib`). Dummy-root warm-ups cover native nextest/clippy/coverage. Hosted Main publishes these secret-free layers for read-only PR restores; authorized local builds can additionally reuse compatible compiler outputs through SeaweedFS S3 sccache. |
| SeaweedFS S3 compiler cache | `task sccache:ensure` verifies authenticated S3 when `.nook/cache/sccache-access-key` and `sccache-secret-key` exist and otherwise reports that sccache is disabled. Hosted delivery jobs never receive those keys because secret-dependent compiler vertices cannot seed the PR cache. `task sccache:stats` reports object presence on the bucket. `task infra:sccache:check` verifies anonymous deny and signed access on `sccache.dev.nokey.sh`. |
