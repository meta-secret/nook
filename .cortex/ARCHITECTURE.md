...
| CI Docker builds        | **`task ci:pr`** (normal PR verify/build without browser e2e, then internal-harness plus isolated Cloudflare aliases and a `github-pages` deployment status); `ci:full-e2e` PRs additionally run **`task ci:pr:e2e`**; **`task ci:main`** runs full browser e2e and isolated Cloudflare deploys for `dev.nokey.sh`, `simple.dev.nokey.sh`, and `sentinel.dev.nokey.sh`. Main also publishes commit-keyed Rust coverage; PRs reuse it, with `task docker:coverage:export` as a coverage-only fallback. |
