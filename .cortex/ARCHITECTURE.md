...
- **Rust/WASM lineage**: `rust-base` + manifest-only chef cooking exposes a lightweight WASM dependency boundary, while native verification extends it with nextest/clippy/coverage profiles. Hosted PR CI runs native coverage independently and verifies WASM once on a dedicated producer; web verification and opt-in browser jobs download that producer's small run-stable artifact instead of rebuilding Rust/WASM. `Verify and preview` starts in parallel, prepares its runner, then polls the current WASM job attempt and reports producer failure explicitly. Hosted CI persists the toolchain, stable native/WASM dependency boundaries, and separate source-sensitive native/WASM snapshots in `nook-rust-base-v1`, `nook-rust-deps-v2`, `nook-rust-wasm-deps-v1`, `nook-rust-native-source-v2`, and `nook-rust-wasm-source-v2`. Every PR job reads only Main's complete lineage and does not export branch-local caches. Explicit `task rust:*` / `task wasm:*` commands load the source-sealed `nook-rust:local` image on demand; browser-only WASM tests and mounted Vite development use `nook-rust-browser:local`.
...
- **CI parity:** `.github/actions/nook-docker-setup` raises Linux watcher limits, creates and exports the hosted `docker-container` Buildx builder for both wrapped and direct Task callers, exposes the Actions cache-service runtime, and enables the Bake GHA scopes. Trusted default-branch and nightly jobs also materialize the direct TLS Redis credential as a mode-`0600` runner file. PR-head, arbitrary-ref, dependency-update, agent-authored, fork, and other no-secrets contexts compile without sccache. Delivery does not depend on the daemon's default image store and never rewrites daemon configuration or restarts Docker.
...
- **Rust compiler cache:** Rust/WASM compiles are wrapped by pinned `sccache`. Authorized local, Main, and nightly builds connect directly to `rediss://redis-ovh-borg-1.bynull.link:6380`; Traefik terminates publicly trusted TLS and forwards only to the internal Redis service, which also requires a generated 256-bit password. BuildKit mounts the password as an optional secret, and short-lived sccache daemons use per-container Unix sockets. Secret-free jobs bypass sccache, so untrusted source cannot read or poison the shared cache. The server cache uses AOF persistence, a 12 GiB memory ceiling, and `allkeys-lru`. This cache survives source-layer invalidation and dependency graph changes, but Cargo downloads, compiler version, target, profile, flags, coverage instrumentation, and non-cacheable link outputs still produce misses.

**Main seeds BuildKit cache visibility.** GitHub lets PR workflows restore the default-branch cache read-only. Main alone refreshes the shared Rust, WASM, web, and e2e scopes; every PR job is read-only so branch generations cannot consume the repository cache quota and displace the authoritative Main lineage. The separate Redis compiler-output cache is shared across authorized workflows. Registry manifests, commit images, and registry authentication remain unwired until the future OCI-cache design is implemented explicitly.
...
exception is Redis-backed `sccache`: authorized local and trusted CI builds use
the direct password-protected TLS server cache. A no-secrets job bypasses
sccache without affecting correctness.
...
| Rust crate dependencies | **cargo-chef** release WASM cooks plus manifest-keyed dummy-root warm-ups for native nextest/clippy/coverage and the debug/test WASM artifacts consumed by `wasm-pack test --node`. Authorized builds also reuse compatible compiler outputs through direct TLS Redis; secret-free builds compile normally. |
| Redis compiler cache | `task sccache:ensure` verifies the direct TLS endpoint when `.nook/cache/redis-password` exists and otherwise reports that sccache is disabled. Trusted default-branch and nightly CI receive the same password; PR-head, arbitrary-ref, dependency-update, and agent-authored jobs never do. `task sccache:stats` reports keys, memory, hits, misses, and evictions. `task infra:redis:stats` inspects the server's AOF-backed 12 GiB cache. |
