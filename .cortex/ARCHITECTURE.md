...
- **Web lineage**: `web-base` contains Bun, Node, Task, and Playwright Chromium; `web-deps` adds `node_modules`. Its independent GHCR cache uses `:web-*` refs and never contains Cargo or `target/`.
...
`task setup` has two solves. The first builds Rust/WASM validation and web dependencies in parallel and exports the scratch `web-artifacts` target to a temporary host directory. That directory contains only generated WASM and coverage files and is guarded at 256 MiB. The second solve supplies it as a named host context to `nook-web`; it never passes `builder-wasm` as a Docker context or parent, so copying a few MB cannot materialize the multi-GB Rust snapshot. The final Dockerfile also asserts that `/usr/local/cargo` and `nook-app/target` are absent.
...
| Web wasm pkg + coverage | Generated in `builder-wasm`, exported from a scratch target to a temporary host directory, then consumed as a small named context by the web solve. |
...
| Playwright Chromium     | Pre-installed in `web-base`; absent from the normal Rust lineage. Browser-only WASM tasks use the on-demand Rust browser image. |
| CI Docker builds        | **`task ci:pr`** (PR verify/build without browser e2e, then in-container Cloudflare deploy from the already-prepared image, plus GitHub `github-pages` deployment status for the PR head SHA) / **`task ci:main:publish`** (main — full browser e2e, then `toolchain-push` after green verify and Cloudflare Pages deploy for `dev.nokey.sh` from the same prepared image). Main also publishes commit-keyed Rust coverage; PRs reuse it, with `task docker:coverage:export` as a coverage-only fallback. |
...
- **`dist` hand-off.** Cloudflare (PR) deploys from inside the container, then PR CI records the Cloudflare URL as a successful GitHub `github-pages` deployment for ruleset enforcement. Main deploys the development artifact to Cloudflare Pages and keeps `dev.nokey.sh` attached there. GitHub Pages release deploys extract `dist` to the runner with `task docker:extract:dist` before `upload-pages-artifact`.
