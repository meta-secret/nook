...
- **BuildKit caching crosses hosted VMs through GHA only:** local commands keep using the selected builder's local content store with `GHA_CACHE_ENABLED` empty. Hosted CI first stabilizes the shared Rust toolchain parent in `nook-rust-base-v1`, then exports native and WASM dependency boundaries to `nook-rust-deps-v2` and `nook-rust-wasm-deps-v1` with `mode=max`. Source-sensitive native coverage and WASM outputs use separate `nook-rust-native-source-v2` and `nook-rust-wasm-source-v2` scopes, so a workflow-only or web-only push does not recompile unchanged Rust on a fresh hosted runner. PR Rust/WASM producers restore only the default-branch scopes, disable export, and source the WASM dependency boundary from the complete `nook-rust-deps-v2` ancestor lineage; exact handoffs accelerate identical inputs. The explicit base and dependency scopes remain required: rebuilding apt/tool installers on each ephemeral VM changes the parent snapshot and invalidates reusable source layers. Web dependencies, browser-free web, and e2e web also use distinct scopes and `mode=max`. Cache export errors are non-fatal because cache is an optimization. The deployed loopback-only OCI registry is reserved for a future authenticated BuildKit cache design and has no current CI consumer.
...
**Main seeds BuildKit cache visibility.** GitHub lets PR workflows restore the default-branch cache read-only. Main refreshes the shared Rust scopes; PR Rust/WASM producers never write branch-local copies, while web/e2e jobs retain isolated PR generations. The separate Redis compiler-output cache is shared across authorized workflows. Registry manifests, commit images, and registry authentication remain unwired until the future OCI-cache design is implemented explicitly.
