...
- **Wasm:** generated by `wasm-pack build nook-wasm` from the `nook-app` workspace root in the `builder-wasm` stage into `nook-app/nook-web/src/lib/nook-wasm` (COPY'd into the nook-web image; mounted local-iteration paths `task web:dev`, `task web:dev:fast`, and `task wasm:build:fast` regenerate it when sources change). Chef-cached `nook-app/target/` at the default in-tree path.
