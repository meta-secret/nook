...
The old combined `nook-web` filesystem was about 9 GB because it inherited warm Rust `target/`, the compiler, Cargo registry, web dependencies, and Playwright. The split keeps those caches in independent BuildKit lineages. Only the WASM package and coverage outputs cross from Rust to web through the commit-scoped, invocation-isolated host directory, and the common runtime image contains no Rust toolchain or `target/`. The normal **`docker` driver** builder writes the web result directly to the containerd image store, avoiding an extra archive/import cycle. PR validation deliberately chooses isolation over that optimization: one fresh `docker-container` builder owns the complete `task ci:pr` invocation and is deleted on exit.

- **Builder selection:** normal `task setup` callers pass the active Docker-context builder (`desktop-linux` on Docker Desktop, `default` on plain Linux/CI), and may override the internal `BUILDX_BUILDER` selection. `task ci:pr` is the exception: `.github/scripts/with-isolated-buildkit.sh` ignores any caller-selected builder, creates a unique `docker-container` builder, runs repository preflight and both app solve phases through it, and removes the builder container/state on every exit. The final slim web image therefore pays one explicit export/import per PR invocation, but no PR build can inherit corrupt leases or daemon state from an earlier build.
...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rust/web/browser layers | Normal commands reuse the selected builder's local store; each `task ci:pr` invocation uses a fresh store and removes it afterward. Nothing is imported from or exported to a registry. |
