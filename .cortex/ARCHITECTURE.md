...
| `nook-app/nook-web/nook-web-app/node_modules` | Installed directly in the `web-deps` Dockerfile layer (parallel branch, local immutable layer like `builder-core-deps`), with no host/daemon cache mount. `web:dev` (mounted) runs `bun install` in its command. |
