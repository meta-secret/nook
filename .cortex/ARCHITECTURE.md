...
| Rust crate dependencies | **cargo-chef** release WASM cooks shared by the web build and `wasm-pack test --node --release`, plus manifest-keyed dummy-root warm-ups for native nextest/clippy/coverage. Hosted Main publishes these secret-free layers for read-only PR restores; authorized local builds can additionally reuse compatible compiler outputs through direct TLS Redis. |
