...
- **Registry push is split:** main updates `rust-buildcache`, `web-buildcache`, and `web-e2e-buildcache` independently. It does not publish duplicate commit-tagged images, and there is no combined registry image.

**Shared caches are pull-always, push-main-only.** Each lineage imports exactly one registry cache manifest: Rust reads `:rust-buildcache`, web reads `:web-buildcache`, and browser e2e reads `:web-e2e-buildcache`. Retired combined and commit-image fallbacks must not return: resolving overlapping histories caused cold runners to transfer the same multi-GB Rust lineage unnecessarily. Publishing is gated on `TOOLCHAIN_PUSH`, which only **main** CI sets; PR CI and local development never push. A missed or unauthenticated pull falls back to a cold build, so the registry remains a cache rather than a build dependency.
...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rust/web cache images   | Each branch imports one max-mode GHCR cache per lineage. **Main** updates Rust, web, and browser-e2e caches through the legacy-named `toolchain-push` bake group; the group never publishes duplicate commit images or merges their filesystems. |
