...
The old combined `nook-web` filesystem was about 9 GB because it inherited warm Rust `target/`, the compiler, Cargo registry, web dependencies, and Playwright. The split keeps those caches in independent BuildKit lineages. Only the WASM package and coverage outputs cross from Rust to web through the commit-scoped, invocation-isolated host directory, and the common runtime image contains no Rust toolchain or `target/`. The normal **`docker` driver** builder writes the web result directly to the containerd image store, avoiding an extra archive/import cycle. PR validation uses a dedicated persistent `docker-container` builder so it can retain expensive Rust/WASM/coverage layers while remaining independently replaceable when unhealthy.

- **Builder selection:** normal `task setup` callers pass the active Docker-context builder (`desktop-linux` on Docker Desktop, `default` on plain Linux/CI), and may override the internal `BUILDX_BUILDER` selection. `task ci:pr` is the exception: `.github/scripts/with-healthy-buildkit.sh` selects the stable dedicated `${NOOK_PR_BUILDX_BUILDER:-nook-pr}` `docker-container` builder. Before each invocation it gives `buildx inspect --bootstrap` plus a minimal `FROM scratch` solve one shared hard `${NOOK_BUILDKIT_HEALTH_TIMEOUT_SECONDS:-60}` second deadline. A healthy daemon and its cache are reused; a missing, failed, or timed-out daemon is force-killed, its state is removed, and a replacement is bootstrapped before repository preflight and both app solve phases begin.
...
- **BuildKit caching never crosses machines:** normal build commands reuse the selected Docker-context builder's local content store, while `task ci:pr` reuses its dedicated health-checked builder on the same Docker host. Registry cache imports and exports remain forbidden because lazy cache blobs still create large, failure-prone network transfers.
