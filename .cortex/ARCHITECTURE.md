...
- **Rust/WASM lineage**: `rust-base` + manifest-only chef cooking exposes a lightweight WASM dependency boundary, while native verification extends it with nextest/clippy/coverage profiles. Hosted PR CI runs native coverage independently and verifies WASM once on a dedicated producer; web verification and opt-in browser jobs download that producer's small run-stable artifact instead of rebuilding Rust/WASM. `Verify and preview` starts in parallel, prepares its runner, then polls for the current producer attempt's clippy-clean built artifact while the producer finishes required Node tests. The overall gate still requires both jobs, producer failures are reported explicitly, and preview deployment remains blocked until the Node-test producer succeeds. Hosted CI persists the toolchain, stable native/WASM dependency boundaries, and separate source-sensitive native/WASM snapshots in `nook-rust-base-v1`, `nook-rust-deps-v2`, `nook-rust-wasm-deps-v1`, `nook-rust-native-source-v2`, and `nook-rust-wasm-source-v2`. Every PR job reads only Main's complete lineage and does not export branch-local caches. Explicit `task rust:*` / `task wasm:*` commands load the source-sealed `nook-rust:local` image on demand; browser-only WASM tests and mounted Vite development use `nook-rust-browser:local`.
- **Web lineage**: `web-base` contains Bun, Node, and Task; `web-deps` adds `node_modules`. PR unit/preview builds use this browser-free lineage. The CI-only web target runs format/lint/check/tests as a sibling of the production web/extension build and joins both successful branches into the same sealed image, instead of serializing verification after the build. `web-e2e-base` adds Playwright Chromium only for main/nightly/manual e2e and uses a separate `:web-e2e-*` cache, so PR cache imports never pull the browser layer. Neither lineage contains Cargo or `target/`.
...
- **Builder selection:** normal local `task setup` and optional local `task ci:*` callers use the active Docker-context daemon builder (`desktop-linux` on Docker Desktop or `default` on plain Linux). They never share a `nook-pr` docker-container. GitHub Actions creates an ephemeral job-scoped `docker-container` builder with `docker/setup-buildx-action` and passes that unique name through the bounded health wrapper before repository preflight and app solve phases begin; warm layers come from GHA cache scopes, not from reusing one host BuildKit container across deployments.
- **CI parity:** `.github/actions/nook-docker-setup` raises Linux watcher limits, creates and exports the hosted `docker-container` Buildx builder for both wrapped and direct Task callers, exposes the Actions cache-service runtime, and enables the Bake GHA scopes. Every hosted delivery build is secret-free and compiles without Redis sccache, including Main and nightly: this keeps Main's exported compiler layers byte-for-byte reusable by read-only PR jobs. Delivery does not depend on the daemon's default image store and never rewrites daemon configuration or restarts Docker.
- **BuildKit caching crosses hosted VMs through GHA only:** local commands keep using the selected builder's local content store with `GHA_CACHE_ENABLED` empty. Hosted CI first stabilizes the shared Rust toolchain parent in `nook-rust-base-v1`, then exports native and WASM dependency boundaries to `nook-rust-deps-v2` and `nook-rust-wasm-deps-v1` with `mode=max`. Source-sensitive native coverage and WASM outputs use separate `nook-rust-native-source-v2` and `nook-rust-wasm-source-v2` scopes, so a workflow-only or web-only push does not recompile unchanged Rust on a fresh hosted runner. Every PR job restores only the default-branch scopes and disables export; WASM restores the dedicated `nook-rust-wasm-deps-v1` lineage, and exact handoffs accelerate identical inputs. The explicit base and dependency scopes remain required: rebuilding apt/tool installers on each ephemeral VM changes the parent snapshot and invalidates reusable source layers. Web dependencies, browser-free web, and e2e web also use distinct scopes and `mode=max`. Cache export errors are non-fatal because cache is an optimization. The deployed loopback-only OCI registry is reserved for a future authenticated BuildKit cache design and has no current CI consumer.
- **Rust compiler cache:** Rust/WASM compiles are wrapped by pinned `sccache`. Authorized local builds can connect directly to `rediss://redis-ovh-borg-1.bynull.link:6380`; Traefik terminates publicly trusted TLS and forwards only to the internal Redis service, which also requires a generated 256-bit password. Runtime Docker commands mount the password as an optional secret, and short-lived sccache daemons use per-container Unix sockets. Hosted delivery jobs deliberately receive no Redis credential and bypass sccache, so their GHA BuildKit vertices match the secret-free vertices restored by PRs. The server cache uses AOF persistence, a 12 GiB memory ceiling, and `allkeys-lru`.

**Main seeds BuildKit cache visibility.** GitHub lets PR workflows restore the default-branch cache read-only. Main alone refreshes the shared Rust, WASM, web, and e2e scopes; every PR job is read-only so branch generations cannot consume the repository cache quota and displace the authoritative Main lineage. Main's publisher is secret-free because a Redis-secret compiler vertex is not a valid seed for the no-secrets PR solve. Registry manifests, commit images, and registry authentication remain unwired until the future OCI-cache design is implemented explicitly.
...
exception is Redis-backed `sccache`: authorized local runtime builds can use
the direct password-protected TLS server cache. Hosted delivery jobs bypass
sccache so Main and PR BuildKit vertices remain identical.
...
| Rust crate dependencies | **cargo-chef** release WASM cooks plus a manifest-keyed release test warm-up, then a source-sensitive `cargo build --tests --release` sibling that matches `wasm-pack test`'s unit graph (dev-dependencies differ from `wasm-pack build --lib`). Dummy-root warm-ups cover native nextest/clippy/coverage. Hosted Main publishes these secret-free layers for read-only PR restores; authorized local builds can additionally reuse compatible compiler outputs through direct TLS Redis. |
| Redis compiler cache | `task sccache:ensure` verifies the direct TLS endpoint when `.nook/cache/redis-password` exists and otherwise reports that sccache is disabled. Hosted delivery jobs never receive that password because secret-dependent compiler vertices cannot seed the PR cache. `task sccache:stats` reports keys, memory, hits, misses, and evictions. `task infra:redis:stats` inspects the server's AOF-backed 12 GiB cache. |
...
| CI Docker builds        | PR CI builds verified WASM once and uploads its small generated package for parallel preview and `ci:full-e2e` consumers. Independent web and extension e2e consumers each build only the Chromium web image from that artifact and run on separate hosted runners; the extension job owns the shared e2e cache export while web restores it without duplicating that upload. Changed PR demo specs and Main's complete demo project retain 90-day Actions artifacts; the 10 largest WebMs per run are best-effort published into one private Linear `nook-ui` issue per PR. Main mirrors that split: native Rust ‖ WASM producers seed GHA scopes, then parallel web verify / web e2e / extension e2e / UI demo consumers rebuild from the WASM handoff; deploy waits on web verify + web e2e only. Main also publishes commit-keyed Rust coverage; PRs reuse it, with `task docker:coverage:export` as a coverage-only fallback. |
