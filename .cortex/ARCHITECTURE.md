...
├── infra/
│   ├── Taskfile.yml      (inline Redis, tunnel, and registry operations)
...
- **CI parity:** `.github/actions/nook-docker-setup` raises Linux watcher limits, creates and exports the hosted `docker-container` Buildx builder for both wrapped and direct Task callers, exposes the Actions cache-service runtime, enables the Bake GHA scopes, and—when repository cache credentials are available—starts a local Cloudflare Access TCP proxy to the persistent Redis endpoint. Forks and other no-secrets contexts retain the job-local Redis fallback. Delivery does not depend on the daemon's default image store and never rewrites daemon configuration or restarts Docker.
...
- **Rust compiler fallback cache:** every Rust/WASM compile is wrapped by pinned `sccache`. Local Task runs idempotently keep one Docker-host-only Redis container running; GitHub-hosted runners set `SCCACHE_REDIS_MODE=external` after a service-token-authenticated `cloudflared access tcp` proxy is healthy. GitHub supplies secrets only as masked inputs to the dedicated Node action, which materializes mode-`0600` runner files and removes the secret input variables before invoking Task. The Redis password enters BuildKit/runtime containers only through read-only secret mounts. The narrowly scoped Access token is present only in the client process environment required by `cloudflared`; no secret is written to a step `env` block, `GITHUB_ENV`, a command argument, container metadata, or logs. Short-lived sccache daemons use per-container Unix sockets and share only Redis storage. The endpoint remains `host.docker.internal:${SCCACHE_REDIS_PORT:-6380}` inside build/runtime containers. `nook-app/docker/resolve-docker-host-ip.sh` resolves a numeric Docker-host IPv4 address and Task passes it to Bake and every Rust-capable runtime container; no magic gateway token is used. Local Redis publishes only on `127.0.0.1` under Docker Desktop and only on the default Docker bridge gateway under Linux Engine. Server Redis publishes only on server loopback and is reached through an outbound-only Cloudflare Tunnel protected by Access plus a generated 256-bit Redis password. The server cache uses AOF persistence, a 12 GiB memory ceiling, and `allkeys-lru`. This cache survives Docker-layer invalidation and dependency graph changes, but Cargo downloads, compiler version, target, profile, flags, coverage instrumentation, and non-cacheable link outputs still produce misses.
...
| Redis compiler cache | `task sccache:ensure` either creates/starts local `nook-sccache-redis` (macOS loopback / Linux bridge gateway, AOF, 8 GiB default, LRU) or authenticates to the runner-local Cloudflare Access proxy selected by CI. `task sccache:stats` reports keys, memory, hits, misses, and evictions for either mode. `task infra:redis:stats` inspects the server's AOF-backed 12 GiB cache. |
...
| CI Docker builds        | PR CI builds verified WASM once and uploads its small generated package for parallel preview and `ci:full-e2e` consumers. Independent web and extension e2e consumers each build only the Chromium web image from that artifact and run on separate hosted runners; the extension job owns the shared e2e cache export while web restores it without duplicating that upload. Main keeps **`task ci:main`** for full verification, browser e2e, and isolated development deploys. Main also publishes commit-keyed Rust coverage; PRs reuse it, with `task docker:coverage:export` as a coverage-only fallback. |
