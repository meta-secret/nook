...
| CI Docker builds        | **`task ci:pr`** (PR verify, in-container Cloudflare deploy, GitHub `github-pages` deployment status for the PR head SHA) / **`task ci:main:publish`** (main — `toolchain-push` after green verify, then Cloudflare Pages deploy for `dev.nokey.sh`). Main also publishes commit-keyed Rust coverage; PRs reuse it, with `task docker:coverage:export` as a coverage-only fallback. |
