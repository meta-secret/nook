...
- **CI parity:** `.github/actions/nook-docker-setup` raises Linux watcher limits, creates and exports the hosted `docker-container` Buildx builder for both wrapped and direct Task callers, logs into `registry.nokey.sh`, and enables Bake registry cache refs. Every hosted delivery build compiles without Redis sccache, including Main and manual e2e: this keeps Main's exported compiler layers byte-for-byte reusable by read-only PR jobs. Delivery does not depend on the daemon's default image store and never rewrites daemon configuration or restarts Docker.
- **BuildKit caching crosses hosted VMs through `registry.nokey.sh`:** local commands keep using the selected builder's local content store with `GHA_CACHE_ENABLED` empty. Hosted CI first stabilizes the shared Rust toolchain parent in `nook-rust-base-v1`, then exports native and WASM dependency boundaries to `nook-rust-deps-v2` and the fingerprinted WASM deps ref with `mode=max`. Source-sensitive native coverage and WASM outputs use separate `nook-rust-native-source-v2` and `nook-rust-wasm-source-v2` refs, so a workflow-only or web-only push does not recompile unchanged Rust on a fresh hosted runner. Every PR job restores only Main's refs and disables export. Web dependencies, browser-free web, and e2e web also use distinct refs and `mode=max`. Cache export errors on web lineages are non-fatal because cache is an optimization. Zot is reached only through Traefik HTTPS at `registry.nokey.sh` with htpasswd auth; there is no host `:5000` listener and no `kubectl port-forward`.
- **Rust compiler cache:** Rust/WASM compiles are wrapped by pinned `sccache`. Authorized local builds can connect directly to `rediss://redis-ovh-borg-1.bynull.link:6380`; Traefik terminates publicly trusted TLS and forwards only to loopback Redis, which also requires a generated 256-bit password. Runtime Docker commands mount the password as an optional secret, and short-lived sccache daemons use per-container Unix sockets. Hosted delivery jobs deliberately receive no Redis credential and bypass sccache, so their BuildKit vertices match the secret-free vertices restored by PRs. The server cache uses AOF persistence, a 12 GiB memory ceiling, and `allkeys-lru`.

**Main seeds BuildKit cache visibility.** Main alone refreshes the shared Rust, WASM, web, and e2e registry refs after lane verification; every PR job is read-only so branch generations cannot poison the authoritative Main lineage. Main's compiler layers remain Redis-secret-free so PR restores match. Clients authenticate to `registry.nokey.sh` with repository secrets (`NOOK_REGISTRY_USERNAME` / `NOOK_REGISTRY_PASSWORD`).
...
| OCI registry (`registry.nokey.sh`) | Zot in k0s with retained `/var/lib/hive/zot`, ClusterIP `10.96.90.10:5000`, Traefik HTTPS + htpasswd. Hosted CI and Hive use it for BuildKit cache and image publish/pull. No host `:5000` mapping and no `kubectl port-forward`. |
