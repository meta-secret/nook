...
- **CI parity:** `.github/actions/nook-docker-setup` requires the runner-provided containerd image store, raises Linux watcher limits, creates and exports the hosted Buildx builder for both wrapped and direct Task callers, exposes the Actions cache-service runtime, and enables the Bake GHA scopes. Delivery never rewrites daemon configuration or restarts Docker; runner-image drift fails fast.
