...
| Rust crate dependencies | **cargo-chef** release WASM cooks plus manifest-keyed dummy-root warm-ups for native nextest/clippy/coverage and the debug/test WASM artifacts consumed by `wasm-pack test --node`. Delivery layers use the job-local Redis-backed sccache without credential mounts, so Main and PRs share identical BuildKit keys and Main can seed every hosted PR. |
