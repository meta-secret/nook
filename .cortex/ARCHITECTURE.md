...
| Rust crate dependencies | **cargo-chef** release WASM cooks plus a manifest-keyed release test warm-up shared by `wasm-pack test --node --release`, followed by dummy-root warm-ups for native nextest/clippy/coverage. Hosted Main publishes these secret-free layers for read-only PR restores; authorized local builds can additionally reuse compatible compiler outputs through direct TLS Redis. |
