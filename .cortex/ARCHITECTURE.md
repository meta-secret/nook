...
| CI Docker builds        | **`task ci:pr`** (PR verify/build without browser e2e, in-container Cloudflare deploy, GitHub `github-pages` deployment status for the PR head SHA) / **`task ci:main:publish`** (main — full browser e2e, then `toolchain-push` after green verify and Cloudflare Pages deploy for `dev.nokey.sh`).                                   |
