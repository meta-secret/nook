...
The old combined `nook-web` filesystem was about 9 GB because it inherited warm Rust `target/`, the compiler, Cargo registry, web dependencies, and Playwright. The split keeps those caches in independent BuildKit lineages. Only the WASM package and coverage outputs cross from Rust to web through the commit-scoped, invocation-isolated host directory, and the common runtime image contains no Rust toolchain or `target/`. The normal **`docker` driver** builder writes the web result directly to the containerd image store, avoiding an extra archive/import cycle. Self-hosted PR, main, and release validation use a dedicated persistent `docker-container` builder so they can retain expensive Rust/WASM/coverage layers while remaining independently replaceable when unhealthy.

- **Builder selection:** normal `task setup` callers pass the active Docker-context builder (`desktop-linux` on Docker Desktop, `default` on plain Linux/CI), and may override the internal `BUILDX_BUILDER` selection. Self-hosted `task ci:pr` and `task ci:main` (also called by release) are the exceptions: `.github/scripts/with-healthy-buildkit.sh` selects the stable dedicated `${NOOK_PR_BUILDX_BUILDER:-nook-pr}` `docker-container` builder. Before each invocation it gives `buildx inspect --bootstrap` plus a minimal `FROM scratch` solve one shared hard `${NOOK_BUILDKIT_HEALTH_TIMEOUT_SECONDS:-60}` second deadline. A healthy daemon and its cache are reused; a missing, failed, or timed-out daemon has its entire probe process group terminated, its container/state removed, and a replacement bootstrapped before repository preflight and app solve phases begin.
...
- **BuildKit caching never crosses machines:** normal build commands reuse the selected Docker-context builder's local content store, while self-hosted delivery tasks reuse their dedicated health-checked builder on the same Docker host. Registry cache imports and exports remain forbidden because lazy cache blobs still create large, failure-prone network transfers.
...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rust/web/browser layers | Normal commands reuse the selected builder's local store; self-hosted PR, main, and release reuse a dedicated local builder after a bounded functional health check. An unhealthy builder is removed and recreated. Nothing is imported from or exported to a registry. |
