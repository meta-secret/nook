...
| Rust crate dependencies | **cargo-chef** release WASM cooks plus manifest-keyed dummy-root warm-ups for native nextest/clippy/coverage and the debug/test WASM artifacts consumed by `wasm-pack test --node`. Hosted Main publishes these secret-free layers for read-only PR restores; authorized local builds can additionally reuse compatible compiler outputs through direct TLS Redis. |
