...
Status: Active product authority with historical storage sections. Claims that
provider credentials or storage mode persist in `localStorage` describe
migration input only. Current app-scoped provider state is sealed by Rust/WASM
and persisted in IndexedDB as defined in
...
6. **Editing secrets:** The UI opens supported vault items in edit mode.
   - Saving calls the Rust-owned `replace_secret` operation through `nook-wasm`.
   - Replacement validates, encrypts, and persists the new record atomically.
   - Passkey private material remains non-editable.
7. **Deleting Secrets:**
...
- **`store_id`:** Logical secret-store identity (`store_{token}`). Same value on every provider replica. See [secret-store-identity.md](../../security/architecture/secret-store-identity.md).
