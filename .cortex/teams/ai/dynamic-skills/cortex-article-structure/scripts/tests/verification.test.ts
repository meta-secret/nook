...
export class CortexArticleStructureVerificationScenario {
  ...
  static expectVerified(
    ...
    expect(() =>
      CortexArticleResultVerifier.verifyCortexArticleStructureResult(request),
    ).not.toThrow();
  }
...
const EXPECTED_FINDINGS: CortexArticleFinding[] = [
  {
    ...
    file: '.cortex/exempt.md',
    line: 1,
    message: 'Article #Exempt empty article has no body content.',
  },
  {
    code: CortexArticleFindingCode.EmptyArticle,
...
test('independently accepts the audit result across every diagnostic branch', () => {
  expect(CortexArticleAudit.auditCortexArticleStructure(AUDIT_REQUEST)).toEqual(
    EXPECTED_FINDINGS,
  );
...
test('rejects missing, reordered, duplicated, and mutated findings', () => {
  const dense = EXPECTED_FINDINGS.at(2);
...
test('treats semantic separators and visible ordered lists as policy inputs', () => {
  ...
  expect(
    CortexArticleAudit.auditCortexArticleStructure(structuredRequest),
  ).toEqual([]);
  CortexArticleStructureVerificationScenario.expectVerified(
    verificationRequest,
  );
...
test('continues independent density verification below nested headings', () => {
  ...
  const result = CortexArticleStructureVerificationScenario.resultWith(
    CortexArticleAudit.auditCortexArticleStructure(auditRequest),
  );
  const verificationRequest: VerifyCortexArticleStructureResultRequest = {
    auditRequest,
    result,
