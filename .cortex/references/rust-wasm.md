...
- **Build mode:** `WASM_BUILD_MODE` defaults to `dev`, which runs release `wasm-pack` with `--no-opt` and stamps `.wasm-source-sha256` with `no-opt`. `WASM_BUILD_MODE=prod` runs the Binaryen `wasm-opt` pass and stamps `optimized`. Local `task check`, `task setup`, `task web:dev`, `task wasm:build`, and PR CI use dev mode unless explicitly overridden; main and release CI pass `WASM_BUILD_MODE=prod`.
...
- **CI** does not rely on defaults: PR CI calls Task with `WASM_BUILD_MODE=dev` to skip `wasm-opt`, while main and release call Task with `WASM_BUILD_MODE=prod` so deployed/released sealed images carry the optimized package.
