...
- Invoke wasm-pack from the **Rust workspace root** (`nook-app`) so chef-cached `target/` is reused:
  `cd nook-app && wasm-pack build nook-wasm --target web --out-dir ../nook-web/src/lib/nook-wasm --out-name nook_wasm`
- **Docker bake** runs wasm-pack once (release + `wasm-opt`) in `builder-wasm` and stamps `.wasm-source-sha256` beside the pkg with the source hash plus build mode. In-container `task _wasm:build` skips when that stamp matches `optimized` — do not rely on file mtimes (COPY makes sources look newer than the baked wasm pkg).
- **Fast local iteration:** `task wasm:build:fast` regenerates the web pkg with `wasm-pack --no-opt`, skipping the expensive Binaryen `wasm-opt` pass. It uses the existing `nook-web:local` image and bind-mounts the worktree; run `task setup` once first if that image does not exist. Use the fast task for browser/dev-loop checks; use `task wasm:build`, `task web:build`, or `task setup` before production/deploy validation so the sealed image carries the optimized artifact.
