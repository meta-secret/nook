...
- **Tests:** PR/main verification runs `task wasm:test`, which executes default wasm-bindgen tests in Node via `wasm-pack test --node --release nook-wasm`. Keeping the Node tests in release mode lets them reuse the release WASM dependency lineage instead of compiling a second debug graph. Browser-only IndexedDB tests are feature-gated behind `browser-wasm-tests` and run manually with `task wasm:test:browser`.
