...
4. **Settings (authenticated):** **Storage providers** lists providers and reconnect. **Onboard another device** generates a QR/link containing active provider credentials plus a selected vault password; users can choose an existing password or create one first. **Devices & access** lists enrolled browsers and approval-based join requests.
