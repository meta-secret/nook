...
- **Host artifact handoff.** `task setup` builds Rust/WASM and web dependencies in parallel, exports only generated WASM and coverage from a scratch target to a temporary host directory, then passes that directory as the web solve's named context. `builder-wasm` is never a parent or context of `nook-web`.
- **Cache is pull-always, push-main-only.** Rust uses `rust-<commit>` / `rust-buildcache`; web uses `web-<commit>` / `web-buildcache`. `cache-to` is gated on `TOOLCHAIN_PUSH` (main CI only), and the `toolchain-push` group publishes the two independent targets. PR CI and local development never push.
- **Dependency caches remain branch-local.** `builder-deps` runs cargo-chef and native/wasm warm-ups in the Rust lineage. `web-deps` runs `bun install --frozen-lockfile` in parallel, with its BuildKit cache mount at `/opt/nook/bun-install-cache`. Playwright Chromium is pre-installed only in `web-base` (and the manual Rust browser-test image).
- **Web dist built at image time.** `nook-app/nook-web/Dockerfile` runs `bun run build` (with the `VITE_BASE` arg), so `nook-app/nook-web/dist` is present in every container. Cloudflare (PR and main development) deploys it from inside the container; GitHub Pages release extracts it via `task docker:extract:dist`.
...
- **CI runners:** latency-critical `pr.yml` verification uses self-hosted `nook` runners with warm Docker/BuildKit caches. Long-running AI agents and all other CI use GitHub-hosted `ubuntu-latest` so background work cannot exhaust or block the Nook machine. Do not use Blacksmith or other third-party runner labels.
...
- **PR CI.** `pr.yml` runs **`task ci:pr`** — one container for format and verify ‖ build, without browser e2e, then in-container Cloudflare preview deploy. Toolchain push is **main only** (`ci:main:publish`). **`main.yml`** runs **`task ci:main:publish`** with full local-provider and extension e2e, then deploys the development artifact to Cloudflare Pages at `https://dev.nokey.sh`; failures invoke the `ci-fix` AI worker. **Nightly** runs sync-live (real provider APIs) and also invokes `ci-fix` on failure.
