...
- **PR CI parallelism.** `pr.yml` builds the toolchain once, then `task ci:pr` runs `ci:verify` and `web:build` in parallel — each in its own `docker run` on the same runner — then deploys the preview.
