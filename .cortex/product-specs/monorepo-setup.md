...
- **Source-in-image, no runtime bind mount.** The workspace source is `COPY`'d into the **nook-web image** (`nook-web/Dockerfile`), as late as possible so a source edit never busts cached layers above it; `task` runs that image directly. `target/` lives at the default in-tree path `/meta-secret/nook/target` (no `CARGO_TARGET_DIR` override, no `/opt`). The only mounted task is `task web:dev` (Vite hot-reload).
...
- **Web dist built at image time.** `nook-web/Dockerfile` runs `bun run build` (with the `VITE_BASE` arg), so `nook-web/dist` is present in every container. Cloudflare (PR) deploys it from inside the container; GitHub Pages (main) extracts it via `task docker:extract:dist`.
