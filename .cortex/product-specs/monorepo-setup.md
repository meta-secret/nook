...
- **Source-in-image, no runtime bind mount.** The workspace source is `COPY`'d into the **nook-web image** (`nook-app/nook-web/Dockerfile`), as late as possible so a source edit never busts cached layers above it; `task` runs that image directly. `nook-app/target/` lives at the default in-tree path `/meta-secret/nook/nook-app/target` (no `CARGO_TARGET_DIR` override, no `/opt`). The only mounted task is `task web:dev` (Vite hot-reload).
