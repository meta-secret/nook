...
- **PR BuildKit is persistent but health-bounded.** `task ci:pr` reuses one dedicated `docker-container` BuildKit daemon on each Docker host. Before building, a hard 60-second health probe must either confirm the daemon or force-kill its container/state and bootstrap a replacement. Never import or export these lineages through registry cache manifests. The Docker-host-only Redis `sccache` service remains the compiler-object fallback below persistent local BuildKit layers.
...
- **CI runners:** latency-critical `pr.yml`, main delivery, and production release use the persistent self-hosted `nook` runner. PR verification reuses its dedicated health-checked BuildKit builder; main and release may reuse the normal Docker-context builder, while all delivery paths can reuse compatible compiler objects through the host-only Redis `sccache`. Long-running AI agents and other background/scheduled CI use GitHub-hosted `ubuntu-latest` and compile Rust cold when necessary; they must not download a serialized remote target snapshot. Do not use Blacksmith or other third-party runner labels.
