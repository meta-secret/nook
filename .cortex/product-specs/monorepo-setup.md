...
- **Cache is pull-always, push-main-only.** Rust imports one `rust-buildcache` manifest, web imports one `web-buildcache` manifest, and browser e2e imports one `web-e2e-buildcache` manifest. `cache-to` is gated on `TOOLCHAIN_PUSH` (main CI only), and the `toolchain-push` group updates those independent caches without publishing duplicate commit-tagged images. PR CI and local development never push.
...
- **CI runners:** latency-critical `pr.yml`, main delivery, and production release use the persistent self-hosted `nook` runner so the multi-GB Rust target and browser layers survive the PR -> main -> release chain. Long-running AI agents and other background/scheduled CI use GitHub-hosted `ubuntu-latest` so they cannot exhaust or block the Nook machine. Do not use Blacksmith or other third-party runner labels.
