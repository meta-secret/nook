...
- **Web dist built at image time.** `nook-app/nook-web/Dockerfile` runs `bun run build` (with the `VITE_BASE` arg), so `nook-app/nook-web/dist` is present in every container. Cloudflare (PR) deploys it from inside the container; GitHub Pages (main and release) extracts it via `task docker:extract:dist`.
...
- **PR CI.** `pr.yml` runs **`task ci:pr`** — one container for format, verify ‖ build, full local-provider e2e, then in-container Cloudflare preview deploy. Toolchain push is **main only** (`ci:main:publish`). **`main.yml`** runs **`task ci:main:publish`** with full local-provider e2e, then builds/extracts `dist` for the development GitHub Pages deploy at `https://dev.nokey.sh`. **Nightly** runs sync-live (real provider APIs).
