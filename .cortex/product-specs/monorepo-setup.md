...
- **BuildKit state never crosses PR builds.** Each `task ci:pr` invocation creates and removes its own `docker-container` BuildKit daemon; normal non-PR build commands may still reuse the active Docker-context builder. Never import or export these lineages through registry cache manifests. The Docker-host-only Redis `sccache` service remains the persistent compiler-object optimization below disposable PR BuildKit state.
