...
- **Rust cache is runner-local.** Rust/WASM relies on the persistent self-hosted delivery runner's local BuildKit cache. Never import or export `target/` with a registry `mode=max` cache: it serializes every intermediate compiler mutation and makes cold restores transfer the entire historical layer chain. The compact web dependency and browser e2e lineages each import one registry manifest; main alone updates them through `toolchain-push`. PR CI and local development never push.
...
- **WASM capability leaves, not feature rebuilds.** The featureless `nook-wasm` bridge compiles once. Tiny crates under `nook-wasm/apps/` select Unified, Simple, Sentinel, extension, or migration identity and link isolated artifacts; do not reintroduce mutually exclusive app features on the shared bridge.
...
- **CI runners:** latency-critical `pr.yml`, main delivery, and production release use the persistent self-hosted `nook` runner so the multi-GB Rust target survives locally across the PR -> main -> release chain without registry transfer. Long-running AI agents and other background/scheduled CI use GitHub-hosted `ubuntu-latest` and compile Rust cold when necessary; they must not download a serialized remote target snapshot. Do not use Blacksmith or other third-party runner labels.
