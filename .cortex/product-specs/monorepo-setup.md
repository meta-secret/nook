...
- **PR CI parallelism.** `pr.yml` builds the toolchain once, then `ci:pr` runs format + wasm, Rust tests + web lint/test/build in parallel, then **local Playwright e2e** (~1 min; IndexedDB specs). **`main.yml`** prepares wasm once, then `ci:main:finish` fans out verify, web build, and **Playwright local ‖ sync-stub** as separate `docker:e2e:run` containers. **Nightly** runs sync-live (real provider APIs).
