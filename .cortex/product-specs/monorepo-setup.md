...
- **One featureless WASM package, not consumer rebuilds.** `nook-wasm` compiles and runs wasm-pack exactly once. Unified, Simple, Sentinel, extension, and migration consumers share that generated package; immutable Rust-owned application configuration and manager capability checks enforce the active realm. Do not reintroduce per-consumer wrapper crates, mutually exclusive app features, or duplicate generated artifacts.
