...
- **PR CI.** `pr.yml` runs **`task ci:pr`** — one container for format, verify ‖ build, full local-provider e2e, then in-container Cloudflare preview deploy. Toolchain push is **main only** (`ci:main:publish`). **`main.yml`** runs **`task ci:main:publish`** with full local-provider e2e, then deploys the development artifact to Cloudflare Pages at `https://dev.nokey.sh`. **Nightly** runs sync-live (real provider APIs).
