...
- **Dependency cache lives in the image.** `cargo-chef` pre-compiles Rust deps; clippy/test/build warm-up runs during `docker build`. `builder-wasm` runs `wasm-pack` there (same cache chain as the release wasm build). The final `toolchain` stage only copies `/opt/nook/target`, the crates.io registry, and `/opt/nook/nook-wasm-pkg` — no second wasm compile.
