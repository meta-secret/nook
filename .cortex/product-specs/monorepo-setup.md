...
- **Dependency cache lives in the image.** `builder-deps` runs `cargo chef cook` (native + wasm32) and `cargo fetch` once. **`builder-debug`** (native clippy, nextest `--no-run`, **llvm-cov `--no-report` + nextest `--no-run`** into `target/llvm-cov-target`, build) and **`builder-wasm`** (wasm clippy, release build, wasm-pack) fork from deps and BuildKit/bake can run them **in parallel**. The final **`toolchain`** stage copies the crates.io registry from deps, native `target/` from debug, wasm32 + pkg from wasm — no serial debug→wasm chain.
...
- **PR CI parallelism.** `pr.yml` builds the toolchain once, then `ci:pr` runs format + wasm, Rust tests + web lint/test/build in parallel, then **e2e-pr** (~1 min; IndexedDB specs). **`main.yml`** runs full stub **e2e** in one container after verify/build. **Nightly** runs sync-live (real provider APIs).
