...
- **PR CI parallelism.** `pr.yml` builds the toolchain once (`cargo chef cook`, clippy, `cargo test --no-run`, wasm warm-up in the Dockerfile), then `ci:pr` runs format + wasm in one container and Rust tests + web lint/test/build in parallel — **no second Rust compile in `docker run`**, but `cargo test -p nook-core` runs against the PR checkout. `task check` on main still runs the full gate in-container.
