...
- **All BuildKit caches are runner-local.** Rust/WASM, web dependencies, and browser layers rely on the persistent self-hosted delivery runner's local BuildKit content store. Never import or export these lineages through registry cache manifests: even a nominally compact cache can lazily pull large blobs, stall on registry sessions, and turn the cache into a network dependency. GitHub-hosted background jobs build cold and do not transfer serialized cache snapshots.
...
- **PR CI.** `pr.yml` runs **`task ci:pr`** — one container for format and verify ‖ build, without browser e2e, then in-container Cloudflare preview deploy. **`main.yml`** runs **`task ci:main`** with full local-provider and extension e2e, then deploys the development artifact to Cloudflare Pages at `https://dev.nokey.sh`; failures invoke the `ci-fix` AI worker. **Nightly** runs sync-live (real provider APIs) and also invokes `ci-fix` on failure.
