...
- **Source-in-image, no runtime bind mount.** The workspace source is `COPY`'d into the **nook-web image** (`docker/nook-web.Dockerfile`), as late as possible so a source edit never busts cached layers above it; `task` runs that image directly. `target/` lives at the default in-tree path `/meta-secret/nook/target` (no `CARGO_TARGET_DIR` override, no `/opt`). The only mounted task is `task web:dev` (Vite hot-reload).
...
- **Cache is pull-always, push-CI-only.** `cache-from` is wired for every build (local dev included), so a cold checkout pulls CI's warm layers instead of recompiling everything. `cache-to` is gated on `TOOLCHAIN_PUSH` (CI only): PRs push `:buildcache` layers (`toolchain-cache`, no `:latest` overwrite); main pushes the verified base image + cache (`toolchain-push`). Local never pushes; a missed local pull just falls back to a cold build. The registry is cache, never a build dependency.
- **Dependency cache lives in the base image.** `builder-deps` runs `cargo chef cook` (native + wasm32) and `cargo fetch` once. **`builder-debug`** (nextest `--no-run`, clippy, llvm-cov `show-env` + nextest `--no-run`) and **`builder-wasm`** (wasm clippy, release build, wasm-pack into `nook-web/src/lib/nook-wasm`) fork from deps and build **in parallel**. The **`toolchain`** stage copies the crates.io registry from deps and the warm `target/` from debug + wasm.
- **Web deps + Playwright in the base image.** `bun install --frozen-lockfile` and `playwright install --with-deps chromium` run in `toolchain-web` (layer cached while `package.json`/`bun.lock` unchanged). `BUN_INSTALL_CACHE_DIR=/opt/nook/bun-install-cache`, `PLAYWRIGHT_BROWSERS_PATH=/opt/nook/ms-playwright`.
- **Web dist built at image time.** `docker/nook-web.Dockerfile` runs `bun run build` (with the `VITE_BASE` arg), so `nook-web/dist` is present in every container. Cloudflare (PR) deploys it from inside the container; GitHub Pages (main) extracts it via `task docker:extract:dist`.
