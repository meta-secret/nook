# Development Core Knowledge Graph

## Relationships

- The [common agent contract](../AGENTS.md) routes software-domain work here.
- [Engineering team ownership](../architecture/team-ownership.md) defines the boundary.

## Document map

- [Development Core Team Agent Contract](AGENTS.md)
- [Authenticator Items](product-specs/authenticator-items.md)
- [Credit Card Items](product-specs/credit-card-items.md)
- [Multi-Device Decentralized Auth](product-specs/decentralized-auth.md)
- [Devices and Access](product-specs/devices-and-access.md)
- [File Attachments](product-specs/file-attachments.md)
- [Password Unlock and Device Join](product-specs/password-envelope.md)
- [Password Manager](product-specs/password-manager.md)
- [Secure Notes](product-specs/secure-notes.md)
- [SLIP-0039 Recovery](product-specs/slip39-recovery.md)
- [Auth Providers, Sync, and Login UX](design-docs/auth-providers.md)
- [Identity, App Keys, Passkeys, and Vault DEKs](design-docs/identity-vault-architecture.md)
- [Secret Store Identity](design-docs/secret-store-identity.md)
- [Sentinel Genesis](design-docs/sentinel-genesis.md)
- [Typed Newtypes](design-docs/typed-newtypes.md)
- [Unified Vault Architecture](design-docs/unified-vault.md)
- [Vault Architecture Modes](design-docs/vault-architecture-modes.md)
- [Vault Event Log](design-docs/vault-event-log.md)
- [Vault Schema Versioning](design-docs/vault-schema-versioning.md)
- [Vault Session and Lock](design-docs/vault-session-and-lock.md)
- [Rust Coding](dynamic-skills/rust-coding.md)
- [Rust Macro Minimization](dynamic-skills/rust-macro-minimization.md)
- [Rust and TypeScript Code Separation](dynamic-skills/rust-typescript-code-separation.md)
- [Rust and WASM Name Coherence](dynamic-skills/rust-wasm-name-coherence.md)
- [Rust and WASM Reference](references/rust-wasm.md)
