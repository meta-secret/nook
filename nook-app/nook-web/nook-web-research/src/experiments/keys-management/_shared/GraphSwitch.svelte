<script lang="ts">
  import { GraphId, graphs, type KeyGraph } from './key-graph'

  interface Props {
    graph: KeyGraph
    onGraph: (next: GraphId) => void
    light?: boolean
  }

  let { graph, onGraph, light = false }: Props = $props()
</script>
...
<div
  ...
  role="group"
  aria-label="Key graph"
>
  {#each graphs as option (option.id)}
    <button
      ...
      aria-pressed={option.id === graph.id}
      onclick={() => onGraph(option.id)}
    >
      {option.label}
    </button>
  {/each}
