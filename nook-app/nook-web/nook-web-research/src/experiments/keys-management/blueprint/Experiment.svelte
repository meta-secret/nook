...
<script lang="ts">
  ...
  interface Part {
    ...
    here: boolean
    /** Set on the one device key belonging to the browser drawn on this sheet. */
    local: boolean
    wires: string[]
    x: number
    y: number
  ...
  /** Sketched affordances: this is the one device key the person can act on. */
  const OWN_ACTIONS = ['Rename', 'Add passkey', 'Revoke']
  ...
  const mine = $derived(anchors[0])
  const externals = $derived(
    graph.devices.filter((device) => !isHere(graph, device)),
  )
  const minePasskeys = $derived(mine ? passkeysForDevice(graph, mine) : [])
  const mineVaults = $derived(mine ? vaultsForDevice(graph, mine.id) : [])
  const schedule = $derived(buildSchedule(parts))
  ...
  function buildParts(graph: KeyGraph): Part[] {
    ...
    const devices: Part[] = graph.devices.map((device, index) => ({
      ...
      state: isHere(graph, device) ? 'Here' : 'Other device',
      here: isHere(graph, device),
      local: isHere(graph, device),
      wires: vaultsForDevice(graph, device.id).map((vault) => vault.shortId),
      x: DEVICE_X,
  ...
  /** This browser's key is drawn in its own block, so the schedule lists the rest. */
  function buildSchedule(parts: Part[]): Schedule[] {
    return [
      {
        key: 'A',
        code: 'A',
        title: 'Passkeys',
        rows: parts.filter((part) => part.kind === NodeKind.Passkey),
      },
      {
        key: 'B',
        code: 'B',
        title: 'Device keys · other',
        rows: parts.filter(
          (part) => part.kind === NodeKind.Device && !part.local,
        ),
      },
      {
        key: 'C',
        code: 'C',
        title: 'Vaults',
        rows: parts.filter((part) => part.kind === NodeKind.Vault),
      },
    ]
  }
  ...
  function frameClass(part: Part): string {
    const doubled = part.local
      ? ' shadow-[0_0_0_3px_#061423,0_0_0_4px_#7fd4f5]'
      : ''
    if (chosen(part)) return `border-[#7fd4f5] bg-[#0d2a40]${doubled}`
    if (lit(part)) return `border-[#2f7fa8] bg-[#082033]${doubled}`
    return `border-[#123f60] bg-[#071a2b]${doubled}`
  }
...
<main class={`min-h-[100svh] bg-[#061423] text-[#cfe9f7] ${GRID}`}>
  ...
  <section class="mx-auto max-w-6xl px-4 pt-28 pb-16 sm:px-8 sm:pt-24 sm:pb-20">
    <div class="border border-[#1d5c86] bg-[#061423]/85">
      ...
      <section
        class="border-b border-[#1d5c86] bg-[#08243a] px-4 py-4 sm:px-5"
        aria-label="This browser"
      >
        <p
          class="font-mono text-[9px] tracking-[0.26em] text-[#4d8fb5] uppercase"
        >
          Title block · this browser
        </p>

        {#if mine}
          <div
            class="mt-2 flex flex-wrap items-end justify-between gap-x-6 gap-y-3"
          >
            <p class="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              <span class="font-mono text-[22px] text-[#7fd4f5]"
                >{mine.shortId}</span
              >
              <span class="text-[12px] text-[#cfe9f7]">{mine.platform}</span>
            </p>
            <div class="flex flex-wrap gap-1.5">
              {#each OWN_ACTIONS as action (action)}
                <button
                  type="button"
                  class="border border-[#1d5c86] px-2 py-1 font-mono text-[9px] tracking-[0.16em] text-[#9fd0e8] uppercase transition hover:border-[#7fd4f5] hover:text-[#7fd4f5] motion-reduce:transition-none"
                >
                  {action}
                </button>
              {/each}
          ...
          <div class="mt-3 grid gap-x-6 gap-y-3 sm:grid-cols-2">
            <div>
              <p
                class="font-mono text-[9px] tracking-[0.18em] text-[#4d8fb5] uppercase"
              >
                Unlocked by {minePasskeys.length}
              </p>
              <div class="mt-1.5 flex flex-wrap gap-1.5">
                {#each minePasskeys as passkey (passkey.id)}
                  <button
                    type="button"
                    aria-pressed={selected.kind === NodeKind.Passkey &&
                      selected.id === passkey.id}
                    class={`border px-1.5 py-1 font-mono text-[11px] transition motion-reduce:transition-none ${passkey.reach === Reach.Here ? 'border-[#2f7fa8] text-[#7fd4f5]' : 'border-dashed border-[#7a4437] text-[#e8886a]'}`}
                    onclick={() =>
                      (selected = { kind: NodeKind.Passkey, id: passkey.id })}
                  >
                    {passkey.shortId}
                  </button>
                {/each}
            ...
            <div>
              <p
                class="font-mono text-[9px] tracking-[0.18em] text-[#4d8fb5] uppercase"
              >
                Opens {mineVaults.length} of {graph.vaults.length}
              </p>
              <div class="mt-1.5 flex flex-wrap gap-1.5">
                {#each mineVaults as vault (vault.id)}
                  <button
                    type="button"
                    aria-pressed={selected.kind === NodeKind.Vault &&
                      selected.id === vault.id}
                    class="border border-[#2f7fa8] px-1.5 py-1 font-mono text-[11px] text-[#7fd4f5] transition hover:border-[#7fd4f5] motion-reduce:transition-none"
                    onclick={() =>
                      (selected = { kind: NodeKind.Vault, id: vault.id })}
                  >
                    {vault.shortId}
                  </button>
                {/each}
        ...
        {:else}
          <p class="mt-2 font-mono text-[18px] text-[#e8886a]">No device key</p>
          <p
            class="mt-1 font-mono text-[9px] tracking-[0.18em] text-[#4d8fb5] uppercase"
          >
            Opens 0 of {graph.vaults.length}
          </p>
        {/if}
      </section>
      ...
      <div class="grid lg:grid-cols-[1fr_19rem]">
        <div
          class="min-w-0 border-b border-[#123f60] lg:border-r lg:border-b-0"
        >
        ...
        <aside class="min-w-0 px-4 py-5 sm:px-5">
          ...
          {#each schedule as block (block.key)}
            <p
            ...
            >
              <span>{block.code} · {block.title}</span>
              <span>{block.rows.length}</span>
            </p>

            {#if block.rows.length === 0}
              <p
                class="mt-1.5 border border-dashed border-[#7a4437] px-2 py-1.5 font-mono text-[9px] tracking-[0.16em] text-[#e8886a] uppercase"
              >
                none
              </p>
            {/if}

            {#each block.rows as part (part.key)}
              <button
                type="button"
                aria-pressed={chosen(part)}
      ...
      >
        ...
        >
          ...
          {#each externals as device (device.id)}
            <p class="mt-1 flex items-baseline gap-2">
              <span class="text-[#9fd0e8]">{device.shortId}</span>
              <span class="tracking-[0.14em] text-[#4d8fb5] uppercase">
                {device.label}
              </span>
            </p>
          {/each}
          {#if externals.length === 0}
            <p class="mt-1 tracking-[0.14em] text-[#4d8fb5] uppercase">None</p>
          {/if}
        </div>
