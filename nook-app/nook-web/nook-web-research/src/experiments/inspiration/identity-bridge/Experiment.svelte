...
<main class="min-h-svh bg-[#08090a] text-[#f4f3f0]">
  ...
  <section class="page" aria-labelledby="bridge-title">
    <header>
      ...
      <p class="lede">
        Hardware proves this identity can act here. Vault grants decide what the
        identity may open.
      </p>
    </header>
          ...
          {#each selectedIdentity.devices as device (device.id)}
            <article class="device-row">
              <span class="device-icon"
                ><MonitorSmartphone class="size-4" /></span
              >
          ...
          {#each selectedGrants as grant (grant.id)}
            ...
            <article class="vault-card">
              <div class="vault-topline">
                ...
                <span class="grant-proof"
                  ><ShieldCheck class="size-3.5" /> Authorized</span
                >
              </div>
              <h2>{vault.label}</h2>
...
<style>
  ...
  .identity-menu button.active .identity-mark {
    border-color: #c9c8c4;
  }
  .identity-copy {
    min-width: 0;
  ...
  .vault-card {
    ...
    border-radius: 0.375rem;
    background: linear-gradient(145deg, #141517, #101113);
  }
  .vault-topline {
    display: flex;
    align-items: center;
  ...
  @media (width < 70rem) {
    ...
    .bridge {
      grid-template-columns: minmax(11rem, 13rem) 6rem minmax(15rem, 1fr);
    }
    .vault-stack {
