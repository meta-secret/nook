...
{#if data.portMode === BridgeGraphPortMode.Target || data.portMode === BridgeGraphPortMode.Both}
  <Handle
    ...
    position={data.flow === BridgeGraphFlow.Vertical
      ? Position.Top
      : Position.Left}
  />
{/if}
  ...
  >
    <header>
      ...
      <code
        ...
        {data.installations.length === 1 ? 'key' : 'keys'}</code
      >
    </header>
  ...
  >
    <header>
      ...
      {#if data.presentation === BridgeIdentityPresentation.Evidence}
        <span class="grant-label"
          ><ShieldCheck class="size-3.5" /> {data.grantRole}</span
        >
      {/if}
    </header>
...
<style>
  ...
  .installation-row > span:nth-child(2),
  ...
  .source-node {
    ...
    box-shadow:
      0 0 0 1px rgb(255 107 61 / 28%),
      0 0 2.25rem rgb(255 107 61 / 9%);
  }
  .hub-node header {
    padding-top: 1.1rem;
  }
