...
<main class="min-h-svh bg-[#08090a] text-[#f4f3f0]">
  ...
  <nav
    class="identity-menu"
    aria-label={selectedPerspective === BridgePerspective.Identities ? 'Identities' : 'Vaults'}
  >
        ...
        {#each identities as identity (identity.id)}
          ...
          <li>
            ...
            >
              ...
              <span class="identity-count"><Vault class="size-3" /> {vaultCount}</span>
            </button>
          </li>
        {/each}
      </ol>
        ...
        {#each vaults as vault (vault.id)}
          ...
          <li>
            ...
            >
              ...
              <span class="identity-count"><Fingerprint class="size-3" /> {identityCount}</span>
            </button>
          </li>
        {/each}
      </ol>
  ...
  <section class="page" aria-labelledby="bridge-title">
    {#if selectedPerspective === BridgePerspective.Identities}
      <header class="page-header">
        ...
        <h1 id="bridge-title">
          {selectedIdentity.label} connects its devices to {grantsForIdentity(selectedIdentityId).length}
          {grantsForIdentity(selectedIdentityId).length === 1 ? 'vault' : 'vaults'}.
        </h1>
        <p class="lede">
          Device keys establish where this distributed identity can act. Vault grants define what it
          may open.
        </p>
      </header>
    {:else}
      <header class="page-header">
        ...
        <h1 id="bridge-title">
          ...
          {selectedVaultGrants.length === 1 ? 'identity' : 'identities'} can open {selectedVault.label}.
        </h1>
        <p class="lede">
          The vault grant names who may open it. Each identity carries evidence of the devices where
          it can act.
        </p>
      </header>
    {/if}
...
<style>
  ...
  .perspective-switch button::before { height: 1.25rem; }
  .perspective-switch button.active { background: linear-gradient(90deg, #17181a 0%, transparent 100%); color: #f4f3f0; }
  .perspective-switch button.active::before { height: 2rem; background: #ff6b3d; }
  .perspective-switch button:focus-visible,
  .identity-menu ol button:focus-visible { outline: 0; box-shadow: inset 0 -1px #8b8b88; }
  ...
  .identity-menu ol button::before { left: -0.75rem; height: 1.5rem; background: #4a4a48; }
  .identity-menu ol button.active { background: linear-gradient(90deg, #1b1c1e 0%, transparent 100%); color: #f4f3f0; }
  .identity-menu ol button.active::before { height: 3.5rem; background: #ff6b3d; }
  ...
  .identity-menu ol button.active .identity-mark { border-color: #c9c8c4; }
  ...
  .identity-count { display: flex; align-items: center; gap: 0.25rem; color: #6d6d6a; font-family: ui-monospace, monospace; font-size: 0.625rem; }
  ...
  .page-header,
  .graph-shell { max-width: 82rem; margin-right: auto; }
  .eyebrow { margin: 0; color: #ff6b3d; font-size: 0.625rem; }
  h1 { max-width: 66rem; margin: 0.75rem 0 0; font-size: clamp(2.35rem, 4vw, 3.75rem); line-height: 1.02; font-weight: 500; letter-spacing: -0.045em; }
  .lede { max-width: 46rem; margin: 0.8rem 0 0; color: #777774; font-size: 0.8125rem; line-height: 1.55; }
  .graph-shell { margin-top: 1.7rem; }
  ...
  @media (width < 48rem) {
    .identity-menu { position: relative; top: auto; left: auto; width: auto; min-height: 0; padding: 5.5rem 1.25rem 0; transform: none; }
    .perspective-heading { margin-left: 0; }
    .perspective-switch { width: 100%; margin: 0 0 1.35rem; }
    .perspective-switch button { min-height: 2.75rem; justify-content: center; padding: 0.5rem 0.75rem; }
    .identity-menu ol,
    ...
    .identity-menu ol button::before { top: 50%; bottom: auto; left: 0; width: 1px; height: 1.5rem; transform: translateY(-50%); }
    .identity-menu ol button.active::before { width: 1px; height: 3rem; }
    .identity-mark { width: 2.375rem; height: 2.375rem; }
    .identity-copy { width: auto; text-align: left; }
    .identity-copy strong { font-size: 0.8125rem; }
    .identity-copy small { display: block; }
    .identity-count { display: flex; }
    .page { min-height: auto; padding: 2rem 0 4rem; }
    .page-header { padding: 0 1.25rem; }
    h1 { max-width: 22rem; font-size: clamp(2.25rem, 10.5vw, 2.75rem); line-height: 1.04; }
    .lede { max-width: 22rem; }
    .graph-shell { margin-top: 1.4rem; }
  }
</style>
