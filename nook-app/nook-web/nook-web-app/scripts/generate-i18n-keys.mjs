...
const bootstrapMessagesOutput = resolve(
  repositoryRoot,
  'nook-app/nook-web/nook-web-shared/src/generated/bootstrap-messages.ts',
)
...
function bootstrapMessagesSource(catalogs) {
  const fields = {
    ...
    unavailable: 'errors.engine_unavailable',
  }
  return [
    ...
    ...Object.entries(catalogs).flatMap(([locale, catalog]) => [
      ...
      ...Object.entries(fields).map(
        ([name, key]) =>
          `    ${name}: ${typescriptString(catalogValue(catalog, key))},`,
      ),
      '  },',
    ]),
    '} as const',
...
const russianCatalog = await readCatalog('ru')
const englishKeys = flattenCatalog(englishCatalog).sort()
const russianKeys = flattenCatalog(russianCatalog).sort()
compareCatalogKeys(englishKeys, russianKeys)
...
const generatedBootstrapMessages = bootstrapMessagesSource({
  en: englishCatalog,
  ru: russianCatalog,
})
if (checkOnly) {
  await verifyOutput(typescriptOutput, generatedTypescript)
  await verifyOutput(bootstrapMessagesOutput, generatedBootstrapMessages)
  ...
  await mkdir(dirname(bootstrapMessagesOutput), { recursive: true })
  ...
  await writeFile(bootstrapMessagesOutput, generatedBootstrapMessages)
