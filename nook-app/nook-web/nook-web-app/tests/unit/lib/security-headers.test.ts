...
describe('vault app security headers', () => {
  test('CSP allows Google Identity Services and Apple CloudKit scripts', () => {
    ...
    expect(csp).toContain(`script-src 'self' 'wasm-unsafe-eval' ${GOOGLE_GIS_SCRIPT_SRC} ${APPLE_CLOUDKIT_SCRIPT_SRC}`)
    expect(csp).toContain(`frame-src ${GOOGLE_GIS_FRAME_SRC}`)
    expect(csp).toContain(`style-src 'self' 'unsafe-inline' ${GOOGLE_GIS_STYLE_SRC}`)
