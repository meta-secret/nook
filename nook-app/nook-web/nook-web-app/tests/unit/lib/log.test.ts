...
describe('isIgnoredErrorSource', () => {
  it('ignores browser extension origins', () => {
    expect(
      browserLogRuntime.isIgnoredErrorSource(
        'chrome-extension://abc/bootstrap-autofill-overlay.js',
      ),
    ).toBe(true)
    expect(
      browserLogRuntime.isIgnoredErrorSource('moz-extension://abc/script.js'),
    ).toBe(true)
  })
  ...
  it('keeps app script sources', () => {
    expect(
      browserLogRuntime.isIgnoredErrorSource(
        'https://app.example/assets/index.js',
      ),
    ).toBe(false)
  })
})
...
describe('sanitizeLogUrl', () => {
  it('removes query strings and hashes', () => {
    expect(
      browserLogRuntime.sanitizeLogUrl(
      ...
      fc.property(
        ...
        (rawUrl, querySecret, fragmentSecret) => {
          ...
          const sanitized = new URL(
            browserLogRuntime.sanitizeLogUrl(url.toString()),
          )
