...
describe('classified login activation', () => {
  ...
  test('does not activate a Continue control whose class is destructive', () => {
    ...
    didSubmit(wholeDocumentPasswordFormSubmission)
    expect(activated).toBe(false)
  })
  ...
  test.each([
    ...
    (formId, control, allowed) => {
      ...
      expect(didSubmit(wholeDocumentPasswordFormSubmission)).toBe(allowed)
    },
  )
  ...
  test('does not fill a form whose method token is a whitespace-padded post', () => {
    document.body.innerHTML = `
      <form method=" post " id="login" action="/auth/login">
        <input autocomplete="username" />
        <input type="password" autocomplete="current-password" />
