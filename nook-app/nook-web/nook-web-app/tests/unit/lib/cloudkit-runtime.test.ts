...
import { cloudKitRuntime, type CloudKitUserIdentity } from '$lib/auth/icloud/cloudkit-runtime'
...
class CloudKitTransportFixture {
  ...
  async setUpAuth(): Promise<CloudKitUserIdentity | undefined> { return this.identity }
  async fetchCurrentUserIdentity(): Promise<CloudKitUserIdentity | undefined> { return this.identity }
  async whenUserSignsIn() { return this.identity }
  install(): void {
...
describe('CloudKit transport identity adapter', () => {
  test('normalizes both transport methods with their owning receivers', async () => {
    ...
    const expected = { kind: CloudKitIdentityKind.SignedIn, identity: transport.identity }
    expect(await container.setUpAuth({ grabAuthToken: true, persist: false })).toEqual(expected)
  ...
  test('normalizes empty identity responses to signed out', async () => {
    ...
    const container = cloudKitRuntime.getDefaultCloudKitContainer()
    expect(await container.setUpAuth({ grabAuthToken: true, persist: false })).toEqual({ kind: CloudKitIdentityKind.SignedOut })
    expect(await container.fetchCurrentUserIdentity()).toEqual({ kind: CloudKitIdentityKind.SignedOut })
  })
})
