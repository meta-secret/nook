...
test(
  'keeps the create workflow usable after locked-login Sentinel status errors',
  async ({ page }) => {
    await page.goto('/app/')
    ...
    await page.evaluate(
      async (statuses: SentinelStatusCycle) => {
        ...
        const originalStatusReader = manager.sentinel_unlock_status.bind(
          manager,
        )
    ...
    await expect(
      page.getByTestId('login-vault-create-workflow'),
    ).toBeVisible()
