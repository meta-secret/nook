...
type AuthenticationDirectSubmitHandler = (form: HTMLFormElement) => boolean;
...
const AuthenticationSubmissionBridge = class {
  ...
  observeAuthenticationDirectSubmits(
    handler: AuthenticationDirectSubmitHandler,
  ): () => void {
    const state = this.isolatedBridgeState();
  ...
  installIsolatedAuthenticationDirectSubmitBridge(): () => void {
    const receive = (event: Event) => {
      if (!(event.target instanceof HTMLFormElement)) return;
      const handler = this.isolatedBridgeState().handler;
      if (handler && !handler(event.target)) event.preventDefault();
    };
    this.browser.window.addEventListener(DIRECT_SUBMIT_EVENT, receive, true);
    return () => {
      this.browser.window.removeEventListener(
        DIRECT_SUBMIT_EVENT,
        receive,
        true,
      );
  ...
  installPageAuthenticationDirectSubmitBridge(): () => void {
