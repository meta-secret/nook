  ...
  async recoverDeviceProtectionWithPasskey() {
    ...
    try {
      ...
      if (isPasskeyCeremonyNotAllowedError(error)) {
        ...
        this.errorMsg = this.t('device_protection.passkey_recovery_not_allowed')
        return
      }
      if (isPasskeyUnavailableError(error)) {
        vaultLog.warn(
  ...
  async unlockWithExtensionDeviceIdentity(
    ...
    try {
      ...
      await this.continueInitializationAfterDeviceUnlock({
        ...
        forceVaultUnlock: true,
      })
      if (!this.isAuthenticated) {
        throw new Error(this.t('extension.unlock.handoff_failed'))
      }
      this.deviceProtectionStatus = 'unlocked'
      return true
    } catch (error) {
