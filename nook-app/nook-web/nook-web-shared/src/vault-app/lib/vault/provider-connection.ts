    ...
    try {
      const discovery = (async () => {
        if (setupType === 'local-folder') {
          ...
          if (handle.kind !== LocalFolderHandleKind.Selected) return storageOk('')
      ...
      const storeId = await new VaultDiscoveryTimeout({ timeoutMs: 30_000 }).waitFor(
        { operation: discovery, releaseLateValue: () => {} },
      )
