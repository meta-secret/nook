...
export function remoteEventProviderArgs(
  ...
  if (provider?.type === LOCAL_FOLDER_PROVIDER_TYPE) return undefined;
  ...
  return state.hasRemoteCredentials() ? state.wasmStorageArgs() : undefined;
}
...
export function dismissLocalFolderMultipleVaultsIssue(
  ...
  state.localFolderMultipleVaultsIssue = undefined;
}
...
export async function manualSync(state: SyncActionsContext) {
  ...
  // A fresh browser has neither a connected provider nor remote credentials.
  // There is no vault to synchronize yet, and initializing device-dependent
  // sync in that state asks the WASM manager to encrypt before vault crypto
  // exists. Keep the device roster projection empty until a vault is connected.
  if (state.syncProviders.length === 0 && !state.hasRemoteCredentials()) {
    state.pendingJoins = [];
    state.vaultMembers = [];
    return;
  }
