  ...
  async issueEnrollmentCode({
    ...
    try {
      ...
      const verification = await state.enqueueStorage(async () => {
        ...
        try {
          return storageOk(
            admitted.value.verify_vault_password(entryId, password),
          );
        } catch {
      ...
      if (selectedProvider.type === "local-folder") {
        return storageErr(
          new EnrollmentIssueFailure(
            EnrollmentIssueRejection.LocalFolderProvider,
          ),
        );
      }
      ...
      if (usesSharedProviderGrant) {
        if (usesSharedICloud) {
          ...
          let grant;
          try {
            grant = await prepare_shared_storage_grant(
      ...
      try {
        payload =
          vaultName.kind === CatalogVaultLabelKind.Present
            ...
            : NookEnrollmentIssueInput.unnamed(
                provider,
                entryId,
                isoTimestamp(),
              );
