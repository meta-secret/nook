...
import { JoinEnrollmentState, NookExistingVaultProviderReadiness } from '$app-wasm'
import {
...
export class ExistingVaultImportLifecycle {
  ...
  remember(storeId: string): void {
    ...
    if (preparation.kind === NookExistingVaultProviderReadiness.MissingLocalFolder) {
      this.vault.errorMsg = this.vault.t(I18N_KEYS.AuthStorageLocalFolderChooseErr)
      return
    }
  ...
  async resume(): Promise<void> {
    ...
    if (existingLocalVault) {
      ...
      if (
        ...
        this.vault.errorMsg = this.vault.t(I18N_KEYS.ErrorsVaultSelectionFailed)
        return
      }
    } else {
