...
export class PasswordEnrollmentActions {
  ...
  private applySavedEnrollmentProvider({
    selection,
  }: SavedEnrollmentProviderApplication): Result<void, StorageOperationFailure> {
    const state = this.state
    if (
    ...
    if (configuration.kind === LocalFolderProviderConfigurationKind.Missing) {
      return storageErr(
        new StorageOperationFailure(StorageOperationFailureKind.LocalFolderRequired),
      )
    }
    state.storageMode = provider.type
    state.clearLoginSetup()
        ...
        try {
          ...
          if (enrollmentProvider.type === GITHUB_PROVIDER_TYPE) {
            ...
            if (
              ...
              const tokens = await googleOAuthSession.requestGoogleDriveSharedAccess(
            ...
            if (sharedProvider.kind === 'authorizationRequired') {
              state.errorMsg = state.t(I18N_KEYS.ErrorsSharedProviderAccessRequired)
              return
            }
            ...
            let oauthFile: ReturnType<typeof enrollmentProvider.oauth_configuration>
            try {
            ...
            const loadedProviders2 = await state.loadProviders(providerLoadOptions2)
            if (loadedProviders2.isErr()) {
              state.errorMsg = state.t(loadedProviders2.error.translationKey)
              return
            }
            const hasLocalPasswordEntries = await this.localVaultHasPasswordEntries()
            let selection: SavedEnrollmentProvider = {
              kind: SavedEnrollmentProviderKind.Local,
            }
            if (hasLocalPasswordEntries.isErr()) {
              state.errorMsg = state.t(hasLocalPasswordEntries.error.translationKey)
              return
            }
          ...
          const loadedProviders3 = await state.loadProviders(providerLoadOptions3)
