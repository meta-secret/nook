...
export class VaultSecretActions {
  ...
  async handleApplePasswordsImport({
    exportBytes,
  }: ApplePasswordsVaultImport): Promise<SecretOperationResult<NookImportResult>> {
    const state = this.state
    ...
    >[0] = {
      importFromManager: async (manager) => {
        try {
          return storageOk(await manager.import_apple_passwords_export(exportBytes))
        } catch (failure) {
          return storageErr(new NativeVaultStorageFailure(failure))
        }
      },
  ...
  async handleChromePasswordsImport({
    csv,
  }: ChromePasswordsVaultImport): Promise<SecretOperationResult<NookImportResult>> {
    const state = this.state
  ...
  > {
    ...
    if (
      ...
      state.selectedPasswordEntry.kind === PasswordEntrySelectionKind.NotSelected
    ) {
      for (const entry of state.passwordEntries) state.selectPasswordEntry(entry.id)
    }
    return storageOk(undefined)
  }

  async refreshSecretsFromSession(): Promise<Result<void, StorageOperationFailure>> {
    const state = this.state
    if (!state.hasManager) {
    ...
    const loadSecretPageArgs2: Parameters<VaultSecretActions['loadSecretPage']>[0] =
      {
  ...
  applyConnectedSecretPage({ page, query }: ConnectedSecretPageApplication): void {
    const state = this.state
  ...
  async decryptSecret({
    id,
  }: SecretDecryption): Promise<Result<NookSecretRecord, StorageOperationFailure>> {
    const state = this.state
  ...
  > {
    ...
    const result = await state.enqueueStorage(async () => {
      ...
      try {
        return storageOk(
          await admittedManager.value.current_authenticator_code(id, unixSeconds),
        )
      } catch (nativeFailure) {
        return storageErr(new NativeVaultStorageFailure(nativeFailure))
      }
