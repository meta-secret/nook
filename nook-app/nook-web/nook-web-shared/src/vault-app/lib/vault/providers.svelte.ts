...
export class VaultProviderActions {
  ...
  wasmStorageArgs(): NookStorageConnectArgs {
    ...
    if (
      ...
      return draft_oauth_storage_args($state.snapshot(state.oauthFileDraft.config))
    }
  ...
  connectStorageArgs(): NookStorageConnectArgs {
    ...
    if (this.shouldUseJoinProviderForConnect()) {
      return new StorageProviderPresentation(
        $state.snapshot(new ProviderSelectionActions(state).syncProviders()[0]!),
      ).storageArgs()
    }
  ...
  async handleRemoteVaultAssessStatus({
    ...
    switch (decision) {
      ...
      case RemoteVaultAssessDecision.PromptMissingRemote:
        state.remoteVaultRecoveryState = RemoteVaultRecoveryState.PromptMissingOnly
        return true
  ...
  > {
    ...
    const promoted = await state.enqueueStorage(async () => {
      ...
      if (manager.isErr()) return storageErr(manager.error)
      try {
        const snapshot = await manager.value.ensure_local_auth_provider_snapshot(
  ...
  beginProviderSetup({ request }: ProviderSetup) {
    ...
    if (type === 'oauth-file') {
      ...
      const configureOauthFileArgs: Parameters<typeof state.configureOauthFile>[0] =
        {
          preset,
  ...
  cancelProviderSetup() {
    const state = this.state
    this.resetICloudSignInState()
    if (state.addProviderOpen && state.loginSetup.kind === LoginSetupKind.Active) {
      const setupType = state.loginSetup.providerType
      state.clearLoginSetup()
      state.githubPat = ''
      state.githubRepo =
        setupType === 'oauth-file' ? DEFAULT_DRIVE_BACKUP_NAME : DEFAULT_GITHUB_REPO
      state.clearLocalFolder()
      state.clearExistingVaultRecoverySummary()
      state.errorMsg = ''
      return
  ...
  async ensureProviderSaved(): Promise<Result<void, StorageOperationFailure>> {
    ...
    try {
      if (outcome.state === NookProviderSaveOutcomeState.Duplicate) {
        return storageErr(
          new StorageOperationFailure(StorageOperationFailureKind.DuplicateProvider),
        )
      }
