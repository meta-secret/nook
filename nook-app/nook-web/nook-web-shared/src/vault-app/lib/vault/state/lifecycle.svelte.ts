...
import { EnrollmentLocationKind, enrollmentBrowser } from '$lib/enrollment/code'
import {
...
export class VaultLifecycleState extends VaultStateSlices {
  ...
  get successDismissScheduled(): boolean {
    return this.successDismissSchedule.kind === SuccessDismissScheduleKind.Scheduled
  }
  ...
  cancelSuccessDismissTimer(): void {
    if (this.successDismissSchedule.kind === SuccessDismissScheduleKind.Scheduled) {
  ...
  stopIdleSessionTracker(): void {
    ...
    if (active.kind === VaultIdleSessionStartKind.Tracking) active.session.stop()
  }
