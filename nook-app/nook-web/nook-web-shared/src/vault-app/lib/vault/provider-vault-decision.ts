...
export class ProviderVaultEvidenceReader {
  ...
  async execute(): Promise<Result<ReadyProviderVaultEvidence, VaultStorageFailure>> {
    const { manager, providerStoreId } = this.request
    let projection: NookProviderVaultDecisionProjection
    try {
      projection = await manager.provider_vault_decision_request(providerStoreId)
    } catch (failure) {
      return err(new NativeVaultStorageFailure(failure))
    }
    try {
      return ok(new ReadyProviderVaultEvidence(projection, projection.identities))
    } catch (failure) {
      projection.free()
      return err(new NativeVaultStorageFailure(failure))
    }
