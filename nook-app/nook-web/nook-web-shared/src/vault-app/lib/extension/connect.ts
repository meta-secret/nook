...
class ExtensionConnectionBrowser {
  ...
  private pairingDeliveryFromResponse(response: unknown): ExtensionPairingDelivery {
    if (
      response &&
  ...
  async openInstalledExtension(): Promise<boolean> {
    ...
    const sendExtensionMessageArgs: Parameters<typeof this.sendExtensionMessage>[0] =
      {
  ...
  private async discoverPairedExtensionIdentityOnce(vaultStoreId: string): Promise<
    | { kind: ExtensionMessageDeliveryKind.Unavailable }
    | {
  ...
  > {
    ...
    if (status.status !== ExtensionPairedVaultIdentityStatusMessageStatus.Unlocked) {
      if (
        status.status ===
  ...
  async discoverPairedExtensionIdentity(
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const result = await this.discoverPairedExtensionIdentityOnce(vaultStoreId)
      ...
      try {
        runtime.sendMessage?.(request.extensionRuntimeId, message, (response) => {
          ...
          if (
            ...
            resolve(
              ok({ envelope: response.envelope, nextNonce: response.nextNonce }),
            )
            return
          }
          resolve(
  ...
  > {
    if (request.source === ExtensionIdentityRequestSource.PairedVault) {
      ...
      try {
        ...
        try {
          admission = Reflect.apply(admit_companion_handoff_response, this.browser, [
...
export const extensionConnectionBrowser = new ExtensionConnectionBrowser(globalThis)
