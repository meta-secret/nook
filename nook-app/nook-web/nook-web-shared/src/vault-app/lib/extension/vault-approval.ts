...
export class ExtensionVaultApproval {
  private readonly managerAtStart: Result<NookVaultManager, VaultStorageFailure>
  ...
  > {
    ...
    const storeId = await vault.enqueueStorage(() => {
      const current = this.admitManager()
      if (current.isErr()) return err(current.error)
      if (activeVault.kind === ActiveVaultKind.Open) return ok(activeVault.storeId)
      try {
    ...
    if (request.scopes.includes(ExtensionConnectScope.SyncProviderCredentials)) {
