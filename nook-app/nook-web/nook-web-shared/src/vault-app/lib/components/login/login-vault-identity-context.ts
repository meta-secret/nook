...
export class LoginVaultIdentityReader {
  ...
  async execute(): Promise<Result<LoginVaultIdentityContext, VaultStorageFailure>> {
    ...
    try {
      ...
      if (kind === NookSelectedVaultIdentityContextKind.Empty) return ok({ kind })
      const identities: LoginVaultLinkedIdentity[] = []
      for (let index = 0; index < snapshot.length; index += 1) {
        const identity = new LinkedLoginIdentity(snapshot.identity(index)).read()
...
class LinkedLoginIdentity {
  ...
  read(): Result<LoginVaultLinkedIdentity, VaultStorageFailure> {
    try {
      return ok({ identityId: this.identity.identityId, label: this.identity.label })
    } catch (failure) {
      return err(new NativeVaultStorageFailure(failure))
    } finally {
      this.identity.free()
