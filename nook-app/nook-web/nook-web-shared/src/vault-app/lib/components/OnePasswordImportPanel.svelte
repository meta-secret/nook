<script lang="ts">
  ...
  async function importFile() {
    ...
    try {
      archive = new Uint8Array(await selectedFile.arrayBuffer())
      result = await onImport(archive)
    } catch (cause: unknown) {
  ...
  <Card class="gap-0 border-border/60 bg-card py-0">
    <CardContent class="space-y-4 p-4 sm:p-5">
      <div class="flex items-start gap-3">
        ...
        <div class="space-y-1 text-sm">
          <p class="font-medium text-foreground">
            {vault.t('onepassword_import.export_hint_title')}
          </p>
          <p class="text-muted-foreground">
            {vault.t('onepassword_import.export_hint')}
          </p>
        </div>
      </div>

      <label class="block space-y-2 text-sm font-medium text-foreground">
        <span>{vault.t('onepassword_import.file_label')}</span>
        <input
          type="file"
          accept=".1pux,application/zip,application/x-zip-compressed"
          data-testid="onepassword-pux-file"
          onchange={selectFile}
      ...
      <p class="text-xs text-muted-foreground">
        {vault.t('onepassword_import.supported_types')}
      </p>

      <Button
        data-testid="onepassword-import-submit"
        disabled={!selectedFile || isSaving}
        onclick={() => void importFile()}
      >
        <Upload class="size-4" />
        {isSaving
          ? vault.t('onepassword_import.importing')
          : vault.t('onepassword_import.import')}
      </Button>

      {#if error}
        <p class="text-sm text-destructive" data-testid="onepassword-import-error">
          {error}
        </p>
      {/if}
      ...
      {#if result}
        <div
          ...
          data-testid="onepassword-import-result"
        >
          <p class="font-medium">
            {vault.t('onepassword_import.result_imported', {
              count: String(result.imported),
            })}
          </p>
          <p class="mt-1 text-xs text-muted-foreground">
            {vault.t('onepassword_import.result_skipped', {
