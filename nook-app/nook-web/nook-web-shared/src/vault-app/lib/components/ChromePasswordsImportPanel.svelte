<script lang="ts">
  ...
  function selectFile(event: Event) {
    selectedFile =
      (event.currentTarget as HTMLInputElement).files?.[0] ?? undefined;
    result = undefined;
    error = "";
  }
...
<div class="space-y-4" data-testid="chrome-passwords-import-panel">
  {#if !embedded}
    <div>
      <h2 class="text-lg font-semibold text-foreground">
        {vault.t("chrome_passwords_import.title")}
      </h2>
      <p class="mt-1 text-sm text-muted-foreground">
        {vault.t("chrome_passwords_import.description")}
      </p>
    </div>
  {/if}
  ...
  <Card class="gap-0 border-border/60 bg-card py-0">
    <CardContent class="space-y-4 p-4 sm:p-5">
      <div class="flex items-start gap-3">
        ...
        <div class="space-y-1 text-sm">
          <p class="font-medium text-foreground">
            {vault.t("chrome_passwords_import.export_hint_title")}
          </p>
          <p class="text-muted-foreground">
            {vault.t("chrome_passwords_import.export_hint")}
          </p>
        </div>
      </div>

      <label class="block space-y-2 text-sm font-medium text-foreground">
        <span>{vault.t("chrome_passwords_import.file_label")}</span>
        <input
          type="file"
          accept=".csv,text/csv"
          data-testid="chrome-passwords-csv-file"
          onchange={selectFile}
      ...
      <p class="text-xs text-muted-foreground">
        {vault.t("chrome_passwords_import.supported_types")}
      </p>

      <Button
        data-testid="chrome-passwords-import-submit"
        disabled={!selectedFile || isSaving}
        onclick={() => void importFile()}
      >
        <Upload class="size-4" />
        {isSaving
          ? vault.t("chrome_passwords_import.importing")
          : vault.t("chrome_passwords_import.import")}
      </Button>

      {#if error}
        <p
          class="text-sm text-destructive"
          data-testid="chrome-passwords-import-error"
        >
          {error}
        </p>
      {/if}
      ...
      {#if result}
        <div
          ...
          data-testid="chrome-passwords-import-result"
        >
          <p class="font-medium">
            {vault.t("chrome_passwords_import.result_imported", {
              count: String(result.imported),
            })}
          </p>
          <p class="mt-1 text-xs text-muted-foreground">
            {vault.t("chrome_passwords_import.result_skipped", {
