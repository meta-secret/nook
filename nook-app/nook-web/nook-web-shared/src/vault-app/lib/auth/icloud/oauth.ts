...
class ICloudOAuthSession {
  ...
  resetICloudAuthStateForTests(): void {
    this.cloudKitInitialization = { kind: CloudKitInitializationKind.NotStarted }
  ...
  initICloudAuth(): Promise<Result<void, OAuthFailure>> {
    if (this.cloudKitInitialization.kind === CloudKitInitializationKind.Initializing)
  ...
  private setUpCloudKitAuth(
    ...
    this.cloudKitAuthSetup = { kind: CloudKitAuthSetupKind.Initializing, completion }
    return completion
  }
  ...
  private clickCloudKitSignInButton(): Result<void, OAuthFailure> {
    try {
      ...
      if (!control) return err(new OAuthFailure(OAuthFailureKind.ControlUnavailable))
      control.click()
      return ok(undefined)
    } catch {
  ...
  private normalizedICloudShortGuid(value: string): Result<string, OAuthFailure> {
    const trimmed = value.trim()
    if (!trimmed) return err(new OAuthFailure(OAuthFailureKind.SharedLinkRequired))
    ...
    try {
      const candidate = new URL(trimmed).pathname.split('/').filter(Boolean).at(-1)
  ...
  private requireCloudKitRecordInfo(response: CloudKitRecordInfosResponse): Result<
    {
  ...
  private async sharedContainer(): Promise<Result<CloudKitContainer, OAuthFailure>> {
    const initialized = await this.initICloudAuth()
    if (initialized.isErr()) return err(initialized.error)
    try {
      await initNookWasm()
  ...
  async createICloudSharedVault(
    ...
    if (!database) return err(new OAuthFailure(OAuthFailureKind.SharedCreation))
  ...
  private async waitForCloudKitSignIn({
    ...
    const signIn = (async (): Promise<Result<CloudKitIdentity, OAuthFailure>> => {
      try {
