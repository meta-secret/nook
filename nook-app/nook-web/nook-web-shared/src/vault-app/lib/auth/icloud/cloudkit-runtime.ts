...
class CloudKitRuntime {
  ...
  removeTokenListener(listener: (token: string) => void): void {
  ...
  private webAuthTokenListeners = new Set<(token: string) => void>();
  ...
  getDefaultCloudKitContainer(): CloudKitContainer {
  ...
  loadCloudKitScript(): Promise<void> {
    return new Promise(
      // eslint-disable-next-line max-params -- Host API owns this positional callback signature.
      (resolve, reject) => {
        if (window.CloudKit) {
          log.info(
            "CloudKit JS already loaded " +
              JSON.stringify(this.currentBrowserDiagnostics()),
          );
          resolve();
          return;
        }
