...
enum AppWasmStartupKind { NotStarted = 'not-started', Initializing = 'initializing' }
type AppWasmStartup =
  | { kind: AppWasmStartupKind.NotStarted }
  | { kind: AppWasmStartupKind.Initializing; completion: Promise<ApplicationStartup> }

/** Construction is possible only after the browser module has initialized. */
export class ReadyVaultApplication {
  private constructor() {}
  static async initialize(application: VaultApplication): Promise<ApplicationStartup> {
    try {
      await initNookWasm()
  ...
  createManager(): Result<NookVaultManager, VaultEngineFailure> {
    try { return ok(new NookVaultManager()) }
    catch { return err(VaultEngineFailure.ManagerCreation) }
  }
}
class VaultApplicationRuntime {
  ...
  ensureAppWasm(application: VaultApplication): Promise<ApplicationStartup> {
    if (this.startup.kind === AppWasmStartupKind.Initializing) return this.startup.completion
...
enum StartupRequestState { Pending = "pending", Expired = "expired" }
...
export class VaultManagerStartup {
  ...
  private async create(): Promise<Result<NookVaultManager, VaultEngineFailure>> {
    const ready = await vaultApplicationRuntime.ensureAppWasm(this.application)
    if (ready.isErr()) return err(ready.error)
    if (this.state === StartupRequestState.Expired) return err(VaultEngineFailure.Timeout)
