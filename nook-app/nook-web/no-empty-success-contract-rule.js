...
function repositoryRelativePath(filename) {
  return relative(import.meta.dirname, resolve(filename))
    .split(sep)
    .join("/");
}
...
function isNeverthrowSymbol(symbol, expectedName, checker) {
  ...
  return declarations.some((declaration) => {
    const pathSegments = resolve(declaration.getSourceFile().fileName).split(
      sep,
    );
  ...
  if (type.isUnionOrIntersection()) {
    if (
      type.types.some((part) =>
        containsEmptyNeverthrowSuccess(part, seen, checker),
      )
    ) {
      return true;
    }
...
function hasEmptySuccessValue(type) {
  ...
  return (
    type.isUnion() && type.types.some((part) => hasEmptySuccessValue(part))
  );
}
...
/** @param {AstNode} node */
function isNestedPropertyPayload(node) {
  let current = node.parent;
  while (current) {
