...
test.describe('Browser 2FA enrollment', () => {
  ...
  test('discards a staged enrollment when the extension session locks', async ({
    ...
    try {
      ...
      // The enrollment watch polls every 250 ms. Remaining empty across four
      // cycles proves that the pre-lock ceremony does not resume after unlock.
      await enrollPage.waitForTimeout(1_000)
      await expect(otpInput).toHaveValue('')
      expect(await listExtensionAuthenticators(paired.context)).toEqual([])
    } finally {
      await paired.context.close()
