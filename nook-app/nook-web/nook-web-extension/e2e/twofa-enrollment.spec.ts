...
class TwoFactorEnrollmentScenario {
  async listExtensionAuthenticators(
    ...
    const grants = this.parsedExtensionPairingSessionGrants(
  ...
  async attemptMalformedBackupCodeReplace(
          ...
          (resolve) => {
            ...
            databaseRequest.onerror = () => {
              resolve({
                ...
                message: ((
                  ...[v = 'Failed to open extension event storage.']
  ...
  private parsedExtensionPairingSessionGrants(
    ...
    for (const [key, value] of entries) {
      ...
      const parsed = this.extensionPairingSessionGrant(value)
...
const twoFactorEnrollmentScenario = new TwoFactorEnrollmentScenario()
...
test.describe('Browser 2FA enrollment', () => {
  ...
  test('cancels QR preview without vault write', async ({
    ...
    try {
      ...
      expect(
        await twoFactorEnrollmentScenario.listExtensionAuthenticators(
          paired.context,
        ),
      ).toEqual([])
    } finally {
  ...
  test('saves a QR authenticator only after confirmation and fills through the picker', async ({
    ...
    try {
      ...
      ).toEqual([{ issuer: 'Mock Auth', account: 'alice-2fa@nook.test' }])

      await expect
        .poll(
          async () =>
            twoFactorEnrollmentScenario.listExtensionAuthenticators(
              paired.context,
            ),
          {
            timeout: 15_000,
  ...
  test('reviews backup codes with replace semantics and no automatic save', async ({
    ...
    try {
      ...
      const malformedReplaceResult =
        await twoFactorEnrollmentScenario.attemptMalformedBackupCodeReplace(
          malformedReplaceArgs,
        )
