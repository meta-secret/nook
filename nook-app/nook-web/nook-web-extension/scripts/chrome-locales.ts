...
type ExtensionChromeLocaleBuildRequest = {
  appCommonLocalesRoot: string
  distDir: string
}

const chromeLocaleJsonReplacer = (_key: string, value: unknown): unknown =>
  value
...
export async function buildChromeLocales({
  appCommonLocalesRoot,
  distDir,
}: ExtensionChromeLocaleBuildRequest): Promise<void> {
  await Promise.all(
    ['en', 'ru'].map(async (locale) => {
      ...
      await writeFile(
        ...
        `${JSON.stringify(messages, chromeLocaleJsonReplacer, 2)}\n`,
      )
    }),
  )
}
