...
export { AccountPickerPageTarget } from './account-picker-page-target'
import { AccountPickerPageTarget } from './account-picker-page-target'
...
class AccountPickerSessions {
  ...
  async authenticatorAccounts({
    grants,
    query,
  }: AuthenticatorAccountsArgs): Promise<
    Result<WebsiteAuthenticatorOption[], ExtensionSessionTransportFailure>
  > {
  ...
  private async loginAccountListForOrigin({
    ...
    for (const grant of grants) {
      ...
      const delivery = await sendMessage(request)
      if (delivery.isErr()) {
