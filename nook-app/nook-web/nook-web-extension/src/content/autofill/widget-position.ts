...
type AuthenticationWidgetPositionContext = {
  readonly widgetState: typeof widgetState
}
class AuthenticationWidgetPosition {
  constructor(private readonly ui: AuthenticationWidgetPositionContext) {}
  ...
  applyWidgetPosition({ host, position }: ApplyWidgetPositionArgs): void {
  ...
  attachPointerDrag({
    ...
    handle.addEventListener('pointermove', (event) => {
      ...
      const nookTypedArgs0_0: Parameters<typeof this.clampWidgetPosition>[0] = {
        left: originLeft + dx,
        top: originTop + dy,
        width: host.offsetWidth,
        height: host.offsetHeight,
      }
      const position = this.clampWidgetPosition(nookTypedArgs0_0)
      this.ui.widgetState.setPosition(position)
      const nookTypedArgs0_1: Parameters<typeof this.applyWidgetPosition>[0] = {
        host,
        position,
      }
      this.applyWidgetPosition(nookTypedArgs0_1)
    })
