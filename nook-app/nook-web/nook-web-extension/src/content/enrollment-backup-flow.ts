...
class EnrollmentBackupInteraction {
  ...
  private unavailableMessage(host: BackupEnrollmentHost): string {
    return host.translatedMessage(BROWSER_MESSAGE_KEYS.WidgetConnectVault)
  }

  private lockedBackupMessage(host: BackupEnrollmentHost): string {
    return host.translatedMessage(BROWSER_MESSAGE_KEYS.WidgetEnrollUnlock)
  }

  private mergeBackupCandidates({
    existing,
    incoming,
  ...
  private showBackupModeChooser({
      ...
      void host
        ...
        .then((delivery) => {
          if (
            ...
            const nookTypedArgs0_47: Parameters<
              typeof enrollmentFlowRenderer.setHostDescription
            >[0] = {
              host,
            ...
            enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_47)
          } else if (
            delivery.kind === RuntimeMessageDeliveryKind.Delivered &&
            delivery.response.kind ===
            ...
            const nookTypedArgs0_48: Parameters<
              typeof enrollmentFlowRenderer.setHostDescription
            >[0] = {
              host,
              text: this.lockedBackupMessage(host),
            }
            enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_48)
          } else {
            const nookTypedArgs0_49: Parameters<
              typeof enrollmentFlowRenderer.setHostDescription
            >[0] = {
              host,
              text: host.translatedMessage(
            ...
            enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_49)
          }
        })
        .finally(() => {
    ...
    const nookTypedArgs0_51: Parameters<
    ...
    const replaceButton =
      enrollmentFlowRenderer.createSecondaryButton(nookTypedArgs0_51)
    const nookTypedArgs0_52: Parameters<
      typeof enrollmentFlowRenderer.createSecondaryButton
    >[0] = {
      host,
    ...
    const mergeButton =
      enrollmentFlowRenderer.createSecondaryButton(nookTypedArgs0_52)
    const nookTypedArgs0_53: Parameters<
      typeof enrollmentFlowRenderer.appendButtonRow
    >[0] = {
      container: section,
      buttons: [replaceButton, mergeButton],
    }
    enrollmentFlowRenderer.appendButtonRow(nookTypedArgs0_53)

    const nookTypedArgs1_6: Parameters<
    ...
    const cancelButton =
      enrollmentFlowRenderer.createTextButton(nookTypedArgs1_6)
    section.append(cancelButton)
  }

  private showBackupAuthenticatorChooser({
    host,
    section,
    accounts,
    codes,
    ...
    section.replaceChildren()
    const nookTypedArgs0_56: Parameters<
      typeof enrollmentFlowRenderer.setHostDescription
    >[0] = {
      host,
      text: host.translatedMessage(
    ...
    enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_56)
    const list = this.browser.document.createElement('div')
    ...
    ;[...accounts.entries()].forEach(([index, account]) => {
      ...
      >[0] = {
        ...
        substitution: new SavedOptionOrdinal(index).label,
      }
      button.textContent =
        host.translatedMessageWithSubstitution(substitutionArgs)
      button.addEventListener('click', (event) => {
        if (!new AuthenticationGesture(event).trusted || host.isBusy()) return
        const nookTypedArgs0_57: Parameters<
        ...
        this.showBackupModeChooser(nookTypedArgs0_57)
      })
      list.append(button)
    })
    section.append(list)

    const nookTypedArgs1_7: Parameters<
      typeof enrollmentFlowRenderer.createTextButton
    >[0] = {
      host,
      labelKey: BROWSER_MESSAGE_KEYS.WidgetBackupCancel,
    ...
    const cancelButton =
      enrollmentFlowRenderer.createTextButton(nookTypedArgs1_7)
    section.append(cancelButton)
  }

  private async continueBackupWithAuthenticatorOptions({
    host,
    section,
    codes,
  }: ContinueBackupWithAuthenticatorOptionsArgs): Promise<void> {
    const nookTypedArgs0_60: Parameters<
      typeof enrollmentFlowRenderer.setHostDescription
    >[0] = {
      host,
      text: host.translatedMessage(BROWSER_MESSAGE_KEYS.WidgetBackupWorking),
    }
    enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_60)
    host.setBusy(true)
    ...
    try {
      ...
      >[0] = {
        ...
        payload: { origin: this.browser.location.origin },
      }
      const delivery =
        await host.sendAuthenticatorOptionsRuntimeMessage(message)

      if (delivery.kind === RuntimeMessageDeliveryKind.Unavailable) {
        const nookTypedArgs0_61: Parameters<
          typeof enrollmentFlowRenderer.setHostDescription
        >[0] = {
          host,
          text: host.translatedMessage(BROWSER_MESSAGE_KEYS.WidgetBackupFailed),
        }
        enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_61)
        host.returnToActions()
        recoveryCopyObservation.clearBackupCodeCandidates(codes)
        return
      }
      ...
      if (response.kind === AuthenticatorOptionsResponseKind.Locked) {
        const nookTypedArgs0_63: Parameters<
          typeof enrollmentFlowRenderer.setHostDescription
        >[0] = {
          host,
          text: this.lockedBackupMessage(host),
        }
        enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_63)
        host.returnToActions()
        recoveryCopyObservation.clearBackupCodeCandidates(codes)
        return
      }
      ...
      if (response.kind === AuthenticatorOptionsResponseKind.Unavailable) {
        const nookTypedArgs0_65: Parameters<
          typeof enrollmentFlowRenderer.setHostDescription
        >[0] = {
          host,
          text: this.unavailableMessage(host),
        }
        enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_65)
        host.returnToActions()
        recoveryCopyObservation.clearBackupCodeCandidates(codes)
        return
      }
      ...
      if (
        ...
        const nookTypedArgs0_66: Parameters<
          typeof enrollmentFlowRenderer.setHostDescription
        >[0] = {
          host,
          text: host.translatedMessage(BROWSER_MESSAGE_KEYS.WidgetBackupFailed),
        }
        enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_66)
        host.returnToActions()
        recoveryCopyObservation.clearBackupCodeCandidates(codes)
        return
      }
      ...
      if (accounts.length === 0) {
        const nookTypedArgs0_67: Parameters<
          typeof enrollmentFlowRenderer.setHostDescription
        >[0] = {
          host,
          text: host.translatedMessage(BROWSER_MESSAGE_KEYS.WidgetBackupFailed),
        }
        enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_67)
        host.returnToActions()
        recoveryCopyObservation.clearBackupCodeCandidates(codes)
        return
      }
      ...
      const nookTypedArgs0_70: Parameters<
        typeof this.showBackupAuthenticatorChooser
      >[0] = { host, section, accounts, codes }
      this.showBackupAuthenticatorChooser(nookTypedArgs0_70)
    } finally {
      host.setBusy(false)
    }
  }

  private collectSelectedBackupCodes(list: HTMLElement): string[] {
  ...
  private showBackupReview({
    ...
    const nookTypedArgs0_71: Parameters<
      typeof enrollmentFlowRenderer.setHostDescription
    >[0] = {
      host,
      text: host.translatedMessage(BROWSER_MESSAGE_KEYS.WidgetBackupReview),
    }
    enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_71)
    ...
    const renderCodeRows = (): void => {
      ...
      codes.forEach((code) => {
        const row = this.browser.document.createElement('label')
        ...
        const checkbox = this.browser.document.createElement('input')
        checkbox.type = 'checkbox'
        checkbox.checked = true

        const text = this.browser.document.createElement('span')
        text.textContent = code

        const removeButton = this.browser.document.createElement('button')
    ...
    const nookTypedArgs1_8: Parameters<
    ...
    >[0] = {
      ...
      onClick: (event) => {
        if (!new AuthenticationGesture(event).trusted || host.isBusy()) return
        const selected = this.collectSelectedBackupCodes(list)
        if (selected.length === 0) {
          const nookTypedArgs0_73: Parameters<
            typeof enrollmentFlowRenderer.setHostDescription
          >[0] = {
            host,
            text: host.translatedMessage(
              BROWSER_MESSAGE_KEYS.WidgetBackupEmpty,
            ),
          }
          enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_73)
          return
        }
        const nookTypedArgs0_74: Parameters<
          typeof this.continueBackupWithAuthenticatorOptions
        >[0] = {
          host,
          section,
          codes: selected,
        }
        void this.continueBackupWithAuthenticatorOptions(nookTypedArgs0_74)
      },
    }
    const confirmButton =
      enrollmentFlowRenderer.createPrimaryButton(nookTypedArgs1_8)

    const nookTypedArgs1_9: Parameters<
      typeof enrollmentFlowRenderer.createTextButton
    >[0] = {
      host,
      labelKey: BROWSER_MESSAGE_KEYS.WidgetBackupCancel,
    ...
    const cancelButton =
      enrollmentFlowRenderer.createTextButton(nookTypedArgs1_9)

    const nookTypedArgs0_77: Parameters<
      typeof enrollmentFlowRenderer.appendButtonRow
    >[0] = {
      container: section,
      buttons: [confirmButton, cancelButton],
    }
    enrollmentFlowRenderer.appendButtonRow(nookTypedArgs0_77)
  }

  async startBackupEnrollment({
    host,
    section,
    ...
    const nookTypedArgs0_78: Parameters<
      typeof enrollmentFlowRenderer.setHostDescription
    >[0] = {
      host,
      text: host.translatedMessage(BROWSER_MESSAGE_KEYS.WidgetBackupWorking),
    }
    enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_78)
    host.setBusy(true)
    section.replaceChildren()
    ...
    try {
      ...
      if (codes.length === 0) {
        ...
        enrollmentFlowRenderer.setHostDescription(nookTypedArgs0_79)
        host.returnToActions()
        return
      }
      const nookTypedArgs0_81: Parameters<typeof this.showBackupReview>[0] = {
        host,
        section,
        codes,
      }
...
export const enrollmentBackupInteraction = new EnrollmentBackupInteraction(
  globalThis,
)
