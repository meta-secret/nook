//! BIP-39 mnemonic validation (word membership + checksum).

use crate::SeedPhraseSecret;

use crate::errors::{ValidationError, ValidationResult};
use bip39::{Language, Mnemonic};

/// A supported BIP-39 mnemonic word count inferred from normalized input.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Bip39MnemonicWordCount(u32);

impl Bip39MnemonicWordCount {
    pub const WORDS_12: Self = Self(12);
    pub const WORDS_24: Self = Self(24);
}

impl From<Bip39MnemonicWordCount> for u32 {
    fn from(value: Bip39MnemonicWordCount) -> Self {
        value.0
    }
}

/// Maximum number of matching BIP-39 word suggestions to return.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Bip39WordSuggestionLimit(usize);

impl From<usize> for Bip39WordSuggestionLimit {
    fn from(value: usize) -> Self {
        Self(value)
    }
}

impl From<Bip39WordSuggestionLimit> for usize {
    fn from(value: Bip39WordSuggestionLimit) -> Self {
        value.0
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Bip39WordSequenceExpectedCount(usize);

impl From<usize> for Bip39WordSequenceExpectedCount {
    fn from(value: usize) -> Self {
        Self(value)
    }
}

impl From<Bip39WordSequenceExpectedCount> for usize {
    fn from(value: Bip39WordSequenceExpectedCount) -> Self {
        value.0
    }
}

/// Validates a normalized English BIP-39 mnemonic (12 or 24 words).
/// Named values required by SeedPhraseSecret::suggest_bip39_words.
pub struct Bip39WordSuggestions<'a> {
    pub prefix: &'a str,
    pub limit: Bip39WordSuggestionLimit,
}

/// Named values required by SeedPhraseSecret::is_bip39_word_sequence_valid.
pub struct Bip39WordSequence<'a> {
    pub text: &'a str,
    pub expected_word_count: Bip39WordSequenceExpectedCount,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
#[error("mnemonic word count is unsupported")]
pub struct UnsupportedMnemonicWordCount;

impl SeedPhraseSecret {
    pub fn validate_bip39_mnemonic(mnemonic: &str) -> ValidationResult<()> {
        let normalized = mnemonic.trim();
        if normalized.is_empty() {
            return Err(ValidationError::Bip39Empty);
        }

        Mnemonic::parse_in_normalized(Language::English, normalized)
            .map(|_| ())
            .map_err(|_| ValidationError::Bip39Invalid)
    }
}

impl SeedPhraseSecret {
    #[must_use]
    pub fn bip39_english_wordlist() -> Vec<&'static str> {
        Language::English.word_list().to_vec()
    }
}

impl SeedPhraseSecret {
    #[must_use]
    pub fn is_known_bip39_word(word: &str) -> bool {
        let normalized = word.trim().to_lowercase();
        if normalized.is_empty() {
            return false;
        }
        Language::English.find_word(&normalized).is_some()
    }
}

impl SeedPhraseSecret {
    #[must_use]
    pub fn suggest_bip39_words(request: Bip39WordSuggestions<'_>) -> Vec<&'static str> {
        let Bip39WordSuggestions { prefix, limit } = request;
        let limit = usize::from(limit);
        let needle = prefix.trim().to_lowercase();
        if needle.is_empty() || limit == 0 {
            return Vec::new();
        }
        Language::English
            .words_by_prefix(&needle)
            .iter()
            .copied()
            .take(limit)
            .collect()
    }
}

impl SeedPhraseSecret {
    #[must_use]
    pub fn is_bip39_word_sequence_valid(request: Bip39WordSequence<'_>) -> bool {
        let Bip39WordSequence {
            text,
            expected_word_count,
        } = request;
        let words = SeedPhraseSecret::parse_bip39_words(text);
        words.len() == usize::from(expected_word_count)
            && words
                .iter()
                .all(|word| SeedPhraseSecret::is_known_bip39_word(word))
    }
}

impl SeedPhraseSecret {
    #[must_use]
    pub fn parse_bip39_words(text: &str) -> Vec<String> {
        text.split_whitespace()
            .map(str::trim)
            .filter(|word| !word.is_empty())
            .map(str::to_lowercase)
            .collect()
    }
}

impl SeedPhraseSecret {
    #[must_use]
    pub fn join_bip39_words(words: &[String]) -> String {
        words
            .iter()
            .map(|word| word.trim().to_lowercase())
            .filter(|word| !word.is_empty())
            .collect::<Vec<_>>()
            .join(" ")
    }
}

impl SeedPhraseSecret {
    #[must_use]
    pub fn infer_bip39_mnemonic_length(
        text: &str,
    ) -> Result<Bip39MnemonicWordCount, UnsupportedMnemonicWordCount> {
        match SeedPhraseSecret::parse_bip39_words(text).len() {
            12 => Ok(Bip39MnemonicWordCount::WORDS_12),
            24 => Ok(Bip39MnemonicWordCount::WORDS_24),
            _ => Err(UnsupportedMnemonicWordCount),
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use bip39::Mnemonic;

    const VALID_12: &str = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";

    #[test]
    fn accepts_standard_bip39_test_vectors() -> anyhow::Result<()> {
        assert!(SeedPhraseSecret::validate_bip39_mnemonic(VALID_12).is_ok());
        let mnemonic_24 = Mnemonic::from_entropy(&[0u8; 32])?;
        assert!(SeedPhraseSecret::validate_bip39_mnemonic(&mnemonic_24.to_string()).is_ok());
        Ok(())
    }

    #[test]
    fn rejects_unknown_words() {
        assert!(SeedPhraseSecret::validate_bip39_mnemonic("notaword notaword notaword notaword notaword notaword notaword notaword notaword notaword notaword notaword").is_err());
    }

    #[test]
    fn rejects_valid_words_with_bad_checksum() {
        assert!(
            SeedPhraseSecret::validate_bip39_mnemonic(
                "able able able able able able able able able able able able"
            )
            .is_err()
        );
    }

    #[test]
    fn rejects_wrong_word_count() {
        assert!(SeedPhraseSecret::validate_bip39_mnemonic("abandon abandon abandon").is_err());
    }

    #[test]
    fn rejects_empty_mnemonic() {
        assert!(SeedPhraseSecret::validate_bip39_mnemonic("   ").is_err());
    }

    #[test]
    fn exposes_bundled_english_wordlist() {
        let words = SeedPhraseSecret::bip39_english_wordlist();
        assert_eq!(words.len(), 2048);
        assert_eq!(words.first().copied(), Some("abandon"));
        assert_eq!(words.last().copied(), Some("zoo"));
    }

    #[test]
    fn suggests_words_by_prefix() {
        assert_eq!(
            SeedPhraseSecret::suggest_bip39_words(Bip39WordSuggestions {
                prefix: "ab",
                limit: 4.into()
            }),
            vec!["abandon", "ability", "able", "about"]
        );
        assert_eq!(
            SeedPhraseSecret::suggest_bip39_words(Bip39WordSuggestions {
                prefix: "zoo",
                limit: 8.into()
            }),
            vec!["zoo"]
        );
        assert!(
            SeedPhraseSecret::suggest_bip39_words(Bip39WordSuggestions {
                prefix: "missing",
                limit: 8.into()
            })
            .is_empty()
        );
    }

    #[test]
    fn validates_word_sequence_membership_without_checksum() {
        assert!(SeedPhraseSecret::is_bip39_word_sequence_valid(
            Bip39WordSequence {
                text: "abandon ability able about above absent absorb abstract absurd abuse access accident",
                expected_word_count: 12.into()
            }
        ));
        assert!(!SeedPhraseSecret::is_bip39_word_sequence_valid(
            Bip39WordSequence {
                text: "abandon notaword",
                expected_word_count: 12.into()
            }
        ));
        assert!(!SeedPhraseSecret::is_bip39_word_sequence_valid(
            Bip39WordSequence {
                text: "abandon ability",
                expected_word_count: 12.into()
            }
        ));
    }

    #[test]
    fn normalizes_and_joins_mnemonic_words() {
        assert_eq!(
            SeedPhraseSecret::parse_bip39_words("  Abandon   ability\nable "),
            vec!["abandon", "ability", "able"]
        );
        assert_eq!(
            SeedPhraseSecret::join_bip39_words(&[
                " abandon ".to_owned(),
                "ABILITY".to_owned(),
                String::new(),
                "able".to_owned(),
            ]),
            "abandon ability able"
        );
    }

    #[test]
    fn infers_supported_mnemonic_lengths() -> anyhow::Result<()> {
        let twelve_word_length = SeedPhraseSecret::infer_bip39_mnemonic_length(
            "abandon ability able about above absent absorb abstract absurd abuse access accident",
        )?;
        assert_eq!(u32::from(twelve_word_length), 12);
        let twenty_four_word_length = SeedPhraseSecret::infer_bip39_mnemonic_length(
            "abandon ability able about above absent absorb abstract absurd abuse access accident \
             account accuse achieve acid acoustic acquire across act action actor actress actual",
        )?;
        assert_eq!(u32::from(twenty_four_word_length), 24);
        assert_eq!(
            SeedPhraseSecret::infer_bip39_mnemonic_length("abandon ability"),
            Err(super::UnsupportedMnemonicWordCount)
        );
        Ok(())
    }
}
