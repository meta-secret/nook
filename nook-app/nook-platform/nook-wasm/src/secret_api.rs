...
impl NookSecretTypeFilter {
    pub(crate) const fn to_core(self) -> nook_core::SecretTypeFilter {
        match self {
            Self::All => SecretTypeFilter::All,
            Self::Login => SecretTypeFilter::Only(SecretType::Login),
            Self::ApiKey => SecretTypeFilter::Only(SecretType::ApiKey),
            Self::SeedPhrase => SecretTypeFilter::Only(SecretType::SeedPhrase),
            Self::SecureNote => SecretTypeFilter::Only(SecretType::SecureNote),
            Self::Passkey => SecretTypeFilter::Only(SecretType::Passkey),
            Self::Authenticator => SecretTypeFilter::Only(SecretType::Authenticator),
            Self::CreditCard => SecretTypeFilter::Only(SecretType::CreditCard),
            Self::FileAttachment => SecretTypeFilter::Only(SecretType::FileAttachment),
        }
    }
}
...
impl NookSecretListItem {
    ...
    pub fn website_url(&self) -> String {
        match &self.item.data {
            ...
            | SecretListItemData::Authenticator { website_url, .. } => website_url.clone(),
            _ => String::new(),
        }
    }
    ...
    pub fn username(&self) -> String {
        match &self.item.data {
            SecretListItemData::Login { username, .. } => username.clone(),
            _ => String::new(),
        }
    }
    ...
    pub fn expires_at(&self) -> String {
        match &self.item.data {
            SecretListItemData::ApiKey { expires_at, .. } => expires_at.clone(),
            _ => String::new(),
        }
    }
    ...
    pub fn last4(&self) -> String {
        match &self.item.data {
            SecretListItemData::CreditCard { last4, .. } => last4.clone(),
            _ => String::new(),
        }
    }
    ...
    pub fn file_name(&self) -> String {
        match &self.item.data {
            SecretListItemData::FileAttachment { file_name, .. } => file_name.clone(),
            _ => String::new(),
        }
    }
    ...
    pub fn mime_type(&self) -> String {
        match &self.item.data {
            SecretListItemData::FileAttachment { mime_type, .. } => mime_type.clone(),
            _ => String::new(),
        }
    }
    ...
    pub fn passkey_user_name(&self) -> String {
        match &self.item.data {
            SecretListItemData::Passkey { user_name, .. } => user_name.clone(),
            _ => String::new(),
        }
    }
    ...
    pub fn account(&self) -> String {
        match &self.item.data {
            SecretListItemData::Authenticator { account, .. } => account.clone(),
            _ => String::new(),
        }
    }
...
pub fn current_code_from_otpauth_uri(
    ...
    if !(millis.is_finite() && millis >= 0.0) {
        return Err(NookError::from(ValidationError::AuthenticatorSecretInvalid).into());
    }
...
mod wasm_tests {
    ...
    fn passkey_list_item_exports_only_rp_and_account_metadata() {
        let item = NookSecretListItem::from_core(
            nook_core::SecretListItem {
                ...
                data: SecretListItemData::Passkey {
    ...
    fn credit_card_list_and_detail_keep_distinct_secret_boundaries() -> anyhow::Result<()> {
        ...
        let record = nook_core::SecretRecord {
            ...
            data: SecretValue::CreditCard(card),
        };
