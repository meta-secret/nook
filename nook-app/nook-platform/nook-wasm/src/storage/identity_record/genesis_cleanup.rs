...
async fn publish_staged_identity(
    ...
    let directory_id = serde_wasm_bindgen::to_value(IDENTITY_DIRECTORY_KEY)
        .map_err(|error| NookError::IndexedDb(format!("Genesis identity key error: {error:?}")))?;
    let current = store
        .get(directory_id.clone())
        .await
        .map_err(|error| NookError::IndexedDb(format!("Genesis identity read error: {error:?}")))?;
    let current = current
    ...
    let encoded = serde_json::to_string(&directory)
        .map_err(|error| NookError::IndexedDb(format!("Genesis identity encode error: {error}")))?;
    ...
    store
        ...
        .await
        .map_err(|error| NookError::IndexedDb(format!("Genesis signing write error: {error:?}")))?;
    Ok(())
}
    ...
    if let Some(current) = current.filter(|value| !value.is_undefined() && !value.is_null()) {
            ...
            && pending.created_at == completed.created_at
        {
            if pending.is_staged() {
                publish_staged_identity(&store, &completion, &mut pending).await?;
            }
