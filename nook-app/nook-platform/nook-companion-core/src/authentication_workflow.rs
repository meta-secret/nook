...
mod tests {
    ...
    fn classifies_authenticator_setup_and_verify_enrollment() -> anyhow::Result<()> {
        ...
        let setup_with_backup_codes = AuthenticationPageObservation {
            enrollment_evidence: AuthenticationEnrollmentEvidence::AuthenticatorSetupAndBackupCodes,
            ..observation()
        };
        let snapshot = classify_authentication_workflow(setup_with_backup_codes).snapshot()?;
        assert_eq!(snapshot.stage, AuthenticationWorkflowStage::Recovery);
        assert_eq!(
            snapshot.action,
            AuthenticationWorkflowAction::SaveBackupCodes
