...
impl ExtensionPairingState {
    pub fn migrate_legacy_json(
        ...
        let records: HashMap<String, LegacyExtensionPairingRecord> = serde_json::from_str(value)
        ...
        for (key, record) in records {
            let grant = match record {
                ...
                LegacyExtensionPairingRecord::Grant(grant) if key == selected_key => {
                    StoredExtensionPairingGrant {
                        ...
                        vault_store_id: StoreId::parse(&grant.vault_store_id)
                            .map_err(|_| ExtensionPairingStateError::InvalidLegacyState)?,
                        vault_name: grant.vault_name,
                        approved_at:
                            ExtensionPairingApprovalEpochMilliseconds::from_legacy_date_to_iso_string(
                                &grant.approved_at,
                            )
                            .map_err(|_| ExtensionPairingStateError::InvalidLegacyState)?,
                        scopes: grant.scopes,
            ...
            if key != StoredExtensionPairingGrant::storage_key_for(&grant.vault_store_id) {
                return Err(ExtensionPairingStateError::InvalidLegacyState);
            }
            if key == selected_key {
        ...
        let migrated_vault_names: HashSet<_> = entries
            ...
            .collect();
        let mut ready_setup = ExtensionReadySetup::from_grant(&selected);
...
mod tests {
    ...
    fn rejects_malformed_legacy_pairing_state_json() {
        assert_eq!(
            ExtensionPairingState::migrate_legacy_json("{"),
    ...
    fn rejects_ambiguous_legacy_pairing_grants_for_the_selected_vault() -> anyhow::Result<()> {
        ...
        records.insert(
            format!(
                "{}{}",
                super::EXTENSION_GRANT_KEY_PREFIX,
                Fixture::OTHER_STORE
            ),
