...
impl WebsiteAuthenticatorOption {
    fn from_wire(
...
impl AuthenticatorOptionsResponse {
    pub fn from_wire(
        ...
        match wire {
            AuthenticatorOptionsResponseWire::Available(
                AuthenticatorOptionsAvailableWire::Ready { ok: true, accounts },
            ) => Ok(AuthenticatorOptionsResponse::Ready {
                kind: AuthenticatorOptionsResponseKind::Ready,
                accounts: accounts
                    .into_iter()
                    .map(WebsiteAuthenticatorOption::from_wire)
...
mod tests {
    ...
    fn decodes_each_authenticator_options_variant() {
        let ready =
            AuthenticatorOptionsResponseWire::Available(AuthenticatorOptionsAvailableWire::Ready {
                ok: true,
                accounts: vec![Fixture::account("vault", "secret", "Nook")],
            });
        assert!(matches!(
            AuthenticatorOptionsResponse::from_wire(ready),
        ...
        for (wire, expected) in [
            ...
            let decoded = AuthenticatorOptionsResponse::from_wire(
        ...
        assert!(matches!(
            AuthenticatorOptionsResponse::from_wire(AuthenticatorOptionsResponseWire::Rejected(
    ...
    fn rejects_blank_authenticator_account_identity() {
        let mut blank_vault_name = Fixture::account("vault", "secret", "Nook");
        blank_vault_name.vault_name = " ".to_owned();
        for malformed in [
            blank_vault_name,
            Fixture::account(" ", "secret", "Nook"),
            Fixture::account("vault", " ", "Nook"),
            Fixture::account("vault", "secret", " "),
        ] {
            ...
            assert_eq!(
                AuthenticatorOptionsResponse::from_wire(wire),
                Err(AuthenticatorOptionsResponseDecodeError)
            );
        }
    }
