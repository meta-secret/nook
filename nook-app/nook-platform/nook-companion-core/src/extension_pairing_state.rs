...
pub fn migrate_legacy_pairing_state_json(
    ...
    let mut matching = records.iter().filter_map(|(key, record)| match record {
        ...
        LegacyExtensionPairingRecord::Grant(_) | LegacyExtensionPairingRecord::Setup(_) => None,
    });
    let Some((key, grant)) = matching.next() else {
        return Err(ExtensionPairingStateError::InvalidLegacyState);
    };
    if matching.next().is_some() {
        return Err(ExtensionPairingStateError::InvalidLegacyState);
    }
    let stored = StoredExtensionPairingGrant {
        vault_type: grant.vault_type,
        device_id: grant.device_id.clone(),
        device_public_key: grant.device_public_key.clone(),
        device_signing_public_key: grant.device_signing_public_key.clone(),
        device_label: grant.device_label.clone(),
        vault_store_id: grant.vault_store_id.clone(),
        vault_name: grant.vault_name.clone(),
    ...
    if setup.status != ExtensionReadySetupStatus::Ready
        || !non_empty(&setup.device_label)
        || setup.paired_vaults.is_empty()
        || setup.sync_provider_count != stored.sync_provider_count
        || key != &grant_storage_key(&stored.vault_store_id)
    {
        return Err(ExtensionPairingStateError::InvalidLegacyState);
    }
    let state = state_for_grant(&stored, true);
    state.validate()?;
    Ok(state)
}
