...
const telemetry = (job, overrides = {}) => ({
  ...
  sccache: { report_count: 1, baked_runtime_mode: "READ_WRITE", runtime_mode: "READ_WRITE", runtime_mode_source: "runtime_secret", client_side: true, counter_reliability: "backend_incomplete", publication_status: "counters_observed", compile_requests: 10, requests_executed: 10, cache_hits: 8, cache_misses: 2, cache_errors: 0, cache_write_errors: 0, cache_writes: 2, compile_failures: 0, hit_rate_percent: 80, measurement: "sum_of_zero_based_run_snapshots", fallback: { state: "active", reason: "" }, snapshots: [] },
  buildkit: {
...
void test("does not invent a regression for cold, tiny, or handoff-only work", () => {
  const model = new PrCacheHealth({ minimumBuildkitHitRate: 20, minimumCompletedSteps: 20 }).evaluate({
    ...
    telemetry: [
      telemetry("wasm", { buildkit: { build_record_count: 0, completed_steps: 0, cached_steps: 0, cache_export: { attempts: 0, completed: 0, bytes: 0, duration_ms: 0, incomplete_failures: 0 }, measurement: "buildx_target_record_steps" } }),
...
void test("uses explicit per-consumer results without inferring matrix timeouts", () => {
  ...
  const failed = new PrCacheHealth().evaluate({
    ...
    telemetry: [],
    consumerResults: new Map([["extension-e2e", "functional_failure"]]),
  });
  ...
  assert.ok(cancelled.warnings.includes("full-e2e-shard:consumer_result_missing"));
  ...
  const timedOut = new PrCacheHealth().evaluate({
    ...
    telemetry: [],
    consumerResults: new Map([["full-e2e-shard-1", "timed_out"]]),
  });
  assert.equal(timedOut.gate.verdict, "fail");
  assert.ok(timedOut.gate.reasons.includes("full-e2e-shard-1:consumer_timed_out"));

  const successDespiteAggregateFailure = new PrCacheHealth().evaluate({
    jobs: [{ id: "full-e2e-shard-1", result: "failure", buildExpected: false, readOnly: true, consumer: true }],
    telemetry: [],
...
void test("records a legitimate cold build without applying the warm threshold", () => {
  ...
  assert.ok(model.warnings.includes("rust:sccache_zero_hits_diagnostic"));
});

void test("reports zero hits diagnostically while cache write errors remain terminal", () => {
...
void test("reads browser consumer results recursively", () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nook-cache-consumer-"));
  const nestedDirectory = path.join(directory, "artifact");
  fs.mkdirSync(nestedDirectory, { recursive: true });
  fs.writeFileSync(path.join(nestedDirectory, "ui-demo.result"), "ui-demo success\n");
  fs.writeFileSync(path.join(nestedDirectory, "shard.result"), "full-e2e-shard-1 timed_out\n");
  try {
    const results = PrCacheHealth.readConsumerResults(directory);
    assert.deepEqual([...results].sort(), [["full-e2e-shard-1", "timed_out"], ["ui-demo", "success"]]);
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
});
...
void test("PR workflow covers every BuildKit-producing job without another build", () => {
  ...
  assert.doesNotMatch(workflow, /Mark browser cache consumer (?:started|completed)/);
  assert.match(workflow, /timeout --kill-after=30s 420s task _web:test:ui-demo/);
  assert.equal(workflow.match(/timeout --kill-after=30s 10200s task/g)?.length, 3);
  assert.match(workflow, /full-e2e-shard-\$\{\{ matrix\.shard \}\}\.result/);
  assert.equal(workflow.match(/NOOK_CACHE_CONSUMER_STATUS/g)?.length, 6);
  assert.match(workflow, /nook-cache-consumer\/ui-demo\.result/);
  assert.match(workflow, /nook-cache-consumer\/extension-e2e\.result/);
