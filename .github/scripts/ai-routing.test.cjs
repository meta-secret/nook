...
const PolicyJobInventoryState = Object.freeze({
  Complete: 'complete',
...
const policyJobInventory = (pages) => {
  ...
  for (const page of pages) {
    if (!Number.isSafeInteger(page.totalCount) || page.totalCount <= 0) {
      return { state: PolicyJobInventoryState.Invalid }
    }
    if (expectedCount !== 0 && page.totalCount !== expectedCount) {
      return { expectedCount, receivedCount, state: PolicyJobInventoryState.CountMismatch }
    }
    expectedCount = page.totalCount
    receivedCount += page.jobs
  }
  return receivedCount === expectedCount
    ? { expectedCount, receivedCount, state: PolicyJobInventoryState.Complete }
    : { expectedCount, receivedCount, state: PolicyJobInventoryState.CountMismatch }
}
...
test('repository policy frontier rejects cross-page job-count drift', () => {
  assert.deepEqual(
    policyJobInventory([{ totalCount: 150, jobs: 100 }, { totalCount: 100, jobs: 0 }]),
    { expectedCount: 150, receivedCount: 100, state: PolicyJobInventoryState.CountMismatch },
  )
  assert.deepEqual(
    policyJobInventory([{ totalCount: 150, jobs: 100 }, { totalCount: 150, jobs: 50 }]),
    { expectedCount: 150, receivedCount: 150, state: PolicyJobInventoryState.Complete },
  )
  assert.deepEqual(policyJobInventory([{ jobs: 100 }]), {
    state: PolicyJobInventoryState.Invalid,
  })
})
