#!/usr/bin/env bash
# Build nook-web/dist for e2e preview when missing or stale.
...
if [[ "${E2E_FORCE_BUILD:-}" == "1" ]]; then
  ...
  if [[ ! -f "$INDEX" ]]; then
    ...
    nook-web/src \
    nook-web/index.html \
    nook-web/vite.config.ts \
    nook-web/svelte.config.js \
    nook-web/src/lib/nook-wasm/nook_wasm_bg.wasm \
    -newer "$INDEX" \
