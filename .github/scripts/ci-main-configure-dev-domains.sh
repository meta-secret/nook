...
if [ -n "$zone_id" ] && ! api_json -X POST \
  ...
  echo "::warning::Cloudflare cache purge was unavailable; using exact-commit probes"
fi
...
if [ "$site_ok" != true ] || [ "$simple_ok" != true ] || [ "$sentinel_ok" != true ]; then
  echo "::error::Development domains failed verification (site=$root_status/$site_status/$simple_status/$sentinel_status/$app_status/$extension_status, simple=$simple_root_status/$simple_extension_status, sentinel=$sentinel_root_status/$sentinel_extension_status)"
  exit 1
fi
