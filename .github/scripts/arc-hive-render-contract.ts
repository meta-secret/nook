...
class ArcHiveRenderContractAssertCpuUnconstrained {
...
export class ArcHiveRenderContractAssertHiveRenderContract {
  constructor(private readonly request: HiveRenderContractInput) {}
  async execute(): Promise<void> {
    const input = this.request;

    const renderedDirectory = mkdtempSync(
      join(tmpdir(), "nook-arc-hive-values-"),
    );
    try {
      ...
      if (hiveValues.minRunners !== 0 || hiveValues.maxRunners !== 10) {
        throw new Error(
          "Hive ARC must scale from zero through ten fresh runners",
        );
      }
      ...
      if (sidecars.has("container-runtime") || sidecars.has("buildkit")) {
        throw new Error(
          "Hive ARC must use the persistent BuildKit service only",
        );
      }
      ...
      for (const container of hivePod.initContainers) {
        new ArcHiveRenderContractAssertCpuUnconstrained(container).execute();
      }
      ...
      if (
        ...
        throw new Error(
          "Hive ARC runner must not target the general Podman API",
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Hive ARC test runtime must retain its memory envelope",
        );
      }
