...
test("worktree decoding excludes prunable development worktrees", () => {
  ...
  expect(new DevelopmentWorktreeSelection().select(decoded.value).isOk()).toBe(true);
});
...
test("selection refuses competing usable dev worktrees", () => {
  ...
  const records = [
    new WorktreeRecord("/tmp/a", headA.value, { kind: WorktreeBranchKind.Branch, name: main.value }, false),
    new WorktreeRecord("/tmp/b", headB.value, { kind: WorktreeBranchKind.Branch, name: main.value }, false),
  ];
