...
- [Bun 1.3.14](https://bun.sh/) for host-run Loom commands
...
must remain outside the tracked repository. Use `task loom:cortex-audit` to
verify canonical links, catalog coverage, and the absence of tracked harness
skill mirrors.
...
`task skills:verify` for both pinned projects. Use the exact
`task skills:tools-list` entrypoint to discover the strict-YAML host's closed
provider-neutral catalog; provider action invocation remains a follow-up.
